(() => {
  ...
  function renderSnapshot(){
    ...
    const cards=[
      ...
      ['DAILY PICKS',daily?String(daily):'—','qualified picks · odds 1.25–1.50'],
  ...
  function marketPreviewCard(row,key){
    ...
    let meta='',badge='',mainValue='—',confidenceClass='low',pickLabel=key==='prime'?'Prime Pick':key==='value'?'Value Pick':key==='ace'?'Ace / DF Pick':key==='sg'?'Set / Game Pick':'Daily Pick';
    if(projectionOnly){
      ...
      meta=[Number.isFinite(odds)?`Odds ${odds.toFixed(2)}`:'',Number.isFinite(edge)?`Model edge ${edge>0?'+':''}${(edge*(Math.abs(edge)<=1?100:1)).toFixed(1)} pp`:'',Number.isFinite(ev)?`EV ${ev>0?'+':''}${(ev*(Math.abs(ev)<=1?100:1)).toFixed(1)}%`:''].filter(Boolean).join(' · ');
  ...
  const routeMeta={
    ...
    prime:['MATCH WINNER','Prime Picks','Qualified Match Winner picks from the main 78/80/5 pool with odds above 1.50.'],
    top_daily:['LOWER RISK','Daily Picks','Qualified Match Winner picks from the same main pool with odds 1.25–1.50.'],
    value:['CLOSE MARKET','Value Picks','Close-odds matches where BlinQ has a clearer model winner; edge is shown, not used as a filter.'],
  ...
  function resultCategoryLabel(value){return ({all:'All published',prime:'Prime',top_daily:'Daily',value:'Value',ace:'Aces',double_faults:'Double Faults',sets:'Sets',games:'Games'})[value]||String(value||'').replaceAll('_',' ');}
  function resultPublication(row,category='all'){
    const pubs=issuedMarketPublications(row);
    const filtered=['prime','top_daily','value','ace','double_faults','sets','games'].includes(category)?pubs.filter(p=>p.section===category):pubs;
  ...
  function filteredResults(){
    ...
    return (state.feed.results||[]).filter(row=>{
      ...
      if(['prime','top_daily','value','ace','double_faults','sets','games'].includes(category)&&!resultTags(row).includes(category))return false;
  ...
  function localResultMetrics(rows,category){
    ...
    const categoryIsBet=['prime','top_daily','value','ace','double_faults','sets','games'].includes(category);const wins=categoryIsBet?betWins:coreWins;const sample=categoryIsBet?bets.length:rows.length;
  ...
  function renderRoute(route){
    ...
    if(route==='prime'){const r=state.ui?.market_rules?.prime||{};body=`<div class="route-sub route-rules"><strong>Prime rule</strong><span>Main quality pool ${Math.round(Number(r.min_win_probability||.78)*100)}%+ · depth ${Math.round(Number(r.min_data_depth||.80)*100)}%+ · surface ${Number(r.min_surface_matches||5)}+/player · odds &gt; ${Number(r.min_odds_exclusive||1.50).toFixed(2)} · edge shown only.</span></div>${genericTable(primeTableRows(),'prime')}`;}
    else if(route==='top_daily'){const r=state.ui?.market_rules?.top_daily||{};const day=state.feed?.market_selection?.odds_report?.betting_day||'current';body=`<div class="route-sub route-rules"><strong>Daily rule</strong><span>BlinQ betting day ${escapeHtml(day)} · same 78/80/5 quality pool · odds ${Number(r.min_odds||1.25).toFixed(2)}–${Number(r.max_odds||1.50).toFixed(2)} · ranked by win probability · no artificial quota.</span></div>${genericTable(marketRows('top_daily'),'top_daily')}`;}
    else if(route==='value'){const r=state.ui?.market_rules?.value||{};body=`<div class="route-sub route-rules"><strong>Value rule</strong><span>Close market only · de-vig player probability gap ≤ ${Math.round(Number(r.max_implied_probability_gap||.15)*100)} pp · model winner ${Math.round(Number(r.min_probability||.60)*100)}%+ · edge/EV displayed, never used as a hard filter.</span></div>${genericTable(marketRows('value'),'value')}`;}
