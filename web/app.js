(() => {
  ...
  function renderAdminUserEditor(user){
    ...
    return `<form id="adminUserForm" class="admin-user-editor admin-user-editor-v6522"><div class="admin-user-editor-head"><span class="avatar">${escapeHtml(initials(user.name||user.email||'U'))}</span><div><small>${escapeHtml(user.id)}</small><h3>${escapeHtml(user.name||user.email||'Používateľ')}</h3><p>${escapeHtml(user.email||'')}${user.telegram_nick?` · ✈ ${escapeHtml(user.telegram_nick)}`:''}</p></div><span class="admin-current-access">${escapeHtml(user.plan_label||user.plan||'BEZ PLÁNU')} · ${escapeHtml(user.status||'—')}</span></div><div class="admin-form-section"><div class="admin-form-section-title"><strong>Členský prístup</strong><span>Vyber úroveň a platnosť. GOAT je najvyššia úroveň.</span></div><div class="admin-form-grid"><label>Plán<select id="adminUserPlan">${planSelectOptions(selectedPlan,true)}</select><small class="field-hint" id="adminPlanTerm">${escapeHtml(selectedPlan?planTermLabel(selectedPlan):'Bez plateného plánu')}</small></label><label>Stav<select id="adminUserStatus">${statusSelectOptions(user.status||'expired')}</select></label><label class="span-2">Platnosť do<input id="adminUserExpires" type="datetime-local" value="${escapeHtml(userDateValue(user.expires_at))}" ${user.status==='lifetime'?'disabled':''}></label></div><div class="admin-account-plan-shortcuts">${membershipHierarchy.filter(id=>state.ui?.plans?.[id]?.enabled!==false).map(id=>`<button type="button" class="admin-plan-shortcut${id==='goat'?' top-tier':''}" data-admin-user-plan="${escapeHtml(id)}"><b>${escapeHtml(state.ui?.plans?.[id]?.label||id.toUpperCase())}</b><small>${escapeHtml(planTermLabel(id))}${id==='goat'?' · TOP':''}</small></button>`).join('')}</div></div><details class="admin-advanced"><summary>Rola a interná referencia</summary><div class="admin-form-grid"><label>Rola<select id="adminUserRole" ${self?'disabled':''}><option value="user"${user.role!=='admin'?' selected':''}>USER</option><option value="admin"${user.role==='admin'?' selected':''}>ADMIN</option></select></label><label>Platba / manuálna referencia<input id="adminPaymentReference" maxlength="120" value="${escapeHtml(user.payment_reference||'')}" placeholder="order, note, transaction id..."></label></div></details><div class="admin-user-savebar"><div>${user.telegram_nick?`<span>Telegram: ✈ ${escapeHtml(user.telegram_nick)}</span>`:''}<span>Vytvorený ${escapeHtml(fmtDate(user.created_at))}</span><span>Posledné prihlásenie ${escapeHtml(fmtDate(user.last_sign_in_at))}</span></div><p id="adminUserMessage" class="form-message"></p><button class="btn btn-primary" type="submit">Použiť zmeny účtu</button></div>${self?'<small class="admin-muted admin-self-note">Tvoja vlastná ADMIN rola je chránená pred náhodným odstránením.</small>':''}</form>`;
  }
  function renderAdminAccounts(){
    const users=Array.isArray(state.adminUsers)?state.adminUsers:[];
    const list=state.adminUsersLoading?'<div class="state-card">Načítavam účty…</div>':users.length?users.map(user=>`<button type="button" class="admin-user-row${state.adminSelectedUser?.id===user.id?' selected':''}" data-admin-user="${escapeHtml(user.id)}" data-search="${escapeHtml(`${user.email||''} ${user.name||''} ${user.telegram_nick||''} ${user.plan||''}`.toLowerCase())}"><span class="avatar">${escapeHtml(initials(user.name||user.email||'U'))}</span><span><strong>${escapeHtml(user.name||'Člen')}</strong><small>${escapeHtml(user.email||'')}</small></span><b>${escapeHtml(user.plan_label||user.plan||'—')}</b><em>${escapeHtml(user.status||'—')}</em></button>`).join(''):'<div class="admin-user-empty">Nie sú načítané žiadne účty. Správa účtov nie je v tomto runtime nakonfigurovaná.</div>';
    return `<section class="admin-ux-section"><div class="admin-ux-heading"><div><small>ČLENOVIA</small><h2>Účty</h2><p>Zmeny účtu sa aplikujú priamo na vybraného používateľa a nevyžadujú publikovanie konfigurácie.</p></div></div>${state.adminUsersError?`<div class="admin-note admin-note-error"><strong>API účtov nie je dostupné</strong><span>${escapeHtml(state.adminUsersError)}</span></div>`:''}<div class="admin-accounts-toolbar"><label class="search-box"><span>⌕</span><input id="adminUserSearch" type="search" placeholder="Hľadať podľa mena, e-mailu alebo Telegramu…"></label><button class="btn btn-ghost" type="button" data-admin-action="refresh-users">↻ Obnoviť používateľov</button></div><div class="admin-accounts-grid"><div class="admin-user-list">${list}</div>${renderAdminUserEditor(state.adminSelectedUser)}</div></section>`;
  }
  ...
  function advertiserOptions(selected=''){
    ...
    return `<option value="">Nepriradené</option>`+rows.map(([id,a])=>`<option value="${escapeHtml(id)}"${id===selected?' selected':''}>${escapeHtml(a.name||id)}</option>`).join('');
  }
