(() => {
  ...
  const dashboardPageSectionKeys=['overview','results','btts'];
  const dashboardSectionKeys=[...dashboardPageSectionKeys.slice(0,1),...dashboardPickSectionKeys,...dashboardPageSectionKeys.slice(1)];
  const dashboardSectionFallback={
    overview:{label:'Overview',panel_id:'',sidebar_element:'SIDEBAR_OVERVIEW',sidebar_enabled:true,dashboard_enabled:false,dashboard_order:8,preview_limit:'ALL'},
    ...
    btts:{label:'BTTS Bonus',panel_id:'bttsBonusPanel',sidebar_element:'SIDEBAR_BTTS',sidebar_enabled:true,dashboard_enabled:false,dashboard_order:9,preview_limit:5},
  };
  ...
  async function loadUiConfig() {
    ...
    applyV6525NavigationAndOverviewDefaults();
  ...
  function applyV6525NavigationAndOverviewDefaults(){
    if(!state.ui)return;
    const navOrder={SIDEBAR_PREDICTIONS:1,SIDEBAR_PRIME:2,SIDEBAR_TOP_DAILY:3,SIDEBAR_ACE:4,SIDEBAR_VALUE:5,SIDEBAR_DOUBLES:6,SIDEBAR_SG:7,SIDEBAR_RESULTS:8,SIDEBAR_OVERVIEW:9,SIDEBAR_BTTS:10};
    Object.entries(navOrder).forEach(([id,order])=>{const item=state.ui?.elements?.[id];if(item)item.order=order;});
    state.ui.dashboard=state.ui.dashboard||{};state.ui.dashboard.sections=state.ui.dashboard.sections||{};
    const cfg=state.ui.dashboard.sections.overview=state.ui.dashboard.sections.overview||clone(dashboardSectionFallback.overview);
    cfg.sidebar_element='SIDEBAR_OVERVIEW';cfg.sidebar_enabled=cfg.sidebar_enabled!==false;cfg.dashboard_enabled=false;cfg.dashboard_order=8;cfg.preview_limit='ALL';cfg.plans=cfg.plans||{};
    accessContexts.forEach(plan=>{const source=cfg.plans[plan]||cfg.plans.rookie||{};cfg.plans[plan]={visible_picks:source.visible_picks??'ALL',blur_remaining:source.blur_remaining===true,see_all:source.see_all!==false,order:8};});
    if(cfg.plans.rookie)cfg.plans.trial=clone(cfg.plans.rookie);
  }
  ...
  function uiIconSvg(id){
    const icons={
      predictions:'<path d="M3.5 11.2 12 4l8.5 7.2"/><path d="M5.5 10v9.5h13V10"/><path d="M9.5 19.5v-6h5v6"/>',
      prime:'<path d="m12 3 2.7 5.7 6.3.8-4.6 4.4 1.2 6.2L12 17l-5.6 3.1 1.2-6.2L3 9.5l6.3-.8L12 3Z"/>',
      top_daily:'<path d="M4 19V11"/><path d="M10 19V5"/><path d="M16 19v-8"/><path d="M22 19V8"/>',
      value:'<path d="m12 3 8 7-8 11-8-11 8-7Z"/><path d="m4 10 8 3 8-3"/>',
      doubles:'<circle cx="9" cy="8" r="3"/><circle cx="17" cy="9" r="2.5"/><path d="M3.5 20c.7-4 2.7-6 5.5-6s4.8 2 5.5 6"/><path d="M14.5 15c2.8 0 4.8 1.7 5.5 5"/>',
      ace:'<path d="M12.6 2.8c.7 3-1.6 4.3-2.4 6.3-.7 1.7-.1 3.2 1.5 4.1-.1-2.1 1.4-3.4 3-4.4 2.1 2 3.3 4.2 3.1 6.7-.2 3.5-2.9 6-6.4 6-3.8 0-6.7-2.8-6.7-6.6 0-3.4 2.1-6.1 4.5-8.2.2 1.5.8 2.5 1.8 3.1.8-2.7.6-4.7 1.6-7Z"/>',
      sg:'<rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><rect x="14" y="14" width="6" height="6" rx="1"/>',
      results:'<path d="M8 4h8v3c0 4-1.7 7-4 8-2.3-1-4-4-4-8V4Z"/><path d="M8 6H5v1c0 2.4 1.3 4 3.5 4.5"/><path d="M16 6h3v1c0 2.4-1.3 4-3.5 4.5"/><path d="M12 15v4"/><path d="M8.5 20h7"/>',
      overview:'<rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10h18M8 5v14M15 5v14"/>',
      btts:'<circle cx="12" cy="12" r="9"/><path d="m12 7 3 2.2-1.1 3.5h-3.8L9 9.2 12 7Z"/><path d="m7.2 5.6 1.8 3.6M16.8 5.6 15 9.2M5 13l5.1-.3M19 13l-5.1-.3M8 19l2.1-6.3M16 19l-2.1-6.3"/>',
      admin:'<circle cx="12" cy="12" r="3"/><path d="M19 12a7 7 0 0 0-.1-1l2-1.5-2-3.4-2.4 1a7 7 0 0 0-1.8-1L14.4 3h-4.8l-.4 3.1a7 7 0 0 0-1.8 1l-2.4-1-2 3.4L5 11a7 7 0 0 0 0 2l-2 1.5 2 3.4 2.4-1a7 7 0 0 0 1.8 1l.4 3.1h4.8l.4-3.1a7 7 0 0 0 1.8-1l2.4 1 2-3.4-2-1.5a7 7 0 0 0 0-1Z"/>',
    };
    const body=icons[id];
    return body?`<svg aria-hidden="true" viewBox="0 0 24 24">${body}</svg>`:'';
  }
  function decorateSectionIcons(){
    const map={predictionsPanel:'prime',topDailyPanel:'top_daily',acePanel:'ace',valuePanel:'value',doublesPanel:'doubles',sgPanel:'sg'};
    Object.entries(map).forEach(([panelId,key])=>{const span=$(panelId)?.querySelector('.section-kicker>span');if(span&&!span.dataset.iconReady){span.innerHTML=uiIconSvg(key);span.dataset.iconReady='1';span.setAttribute('aria-hidden','true');}});
  }
  function emptyStateCard(key,message){
    const label=publicText(dashboardSectionConfig(key).label||key);
    return `<div class="state-card market-empty"><span class="empty-state-icon" aria-hidden="true">${uiIconSvg(key)}</span><strong>${escapeHtml(label)}</strong><small>${escapeHtml(message)}</small></div>`;
  }
  ...
  function renderNavigationGroup(items, containerId) {
    ...
    [...(items||[])].filter(x=>x.enabled!==false).sort((a,b)=>Number(a.order||0)-Number(b.order||0)).forEach(item=>{
      ...
      a.innerHTML=`<span class="nav-icon" aria-hidden="true">${uiIconSvg(item.id)||escapeHtml(item.icon||'•')}</span><span>${escapeHtml(item.label||item.id)}${item.beta?' <em class="nav-beta">BETA</em>':''}</span>`;
      host.appendChild(a);
    });
  }
  ...
  function lockedPickCard(key,index){
    ...
    const planShort=String(label||plan).replace(/^BlinQ\s+/i,'');
    return `<article class="prediction-card dashboard-locked-card" data-upgrade-plan="${escapeHtml(plan)}" data-upgrade-section="${escapeHtml(section.label||key)}"><div class="dashboard-locked-meta"><span>${escapeHtml(publicText(section.label||key))}</span><span>🔒 ${escapeHtml(planShort)}</span></div><div class="locked-pick-copy"><b aria-hidden="true">↗</b><strong>${escapeHtml(publicText('Unlock this pick'))}</strong><small>${escapeHtml(lcopy(`Available with ${label}.`,`Dostupné od úrovne ${label}.`,`Dostupné od úrovně ${label}.`))}</small><button type="button" class="btn btn-primary">${escapeHtml(publicText('Upgrade to unlock →'))}</button></div><div class="dashboard-locked-foot">${escapeHtml(lcopy('Preview protected by your current plan','Náhľad chránený aktuálnou úrovňou','Náhled chráněný aktuální úrovní'))}</div></article>`;
  }
  ...
  function renderNavigation(){
    const navIcons={overview:'▦',prime:'✦',top_daily:'★',value:'◇',doubles:'◈',ace:'♠',sg:'▥',results:'✓',btts:'⚽'};
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    if(projectionOnly){
      ...
      else{mainValue=`${projection.toFixed(1)}<small> proj.</small>`;metrics=[[lcopy('Opp. proj.','Súper proj.','Proj. soupeře'),Number.isFinite(opponentProjection)?opponentProjection.toFixed(1):'—'],[lcopy('Gap','Rozdiel','Rozdíl'),Number.isFinite(projectionGap)?`${projectionGap>=0?'+':''}${projectionGap.toFixed(1)}`:'—'],[lcopy('Confidence','Istota','Jistota'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];pickLabel=lcopy('Ace / DF projection','Projekcia es / dvojchýb','Projekce es / dvojchyb');}
  ...
  function renderMarketSection(key,hostId,emptyText){
    ...
    host.innerHTML=visible.length?visible.map((row,localIndex)=>{const absoluteIndex=start+localIndex;const locked=ent.blur_remaining!==false&&absoluteIndex>=unlocked;return marketPreviewCard(row,key,absoluteIndex,locked,true)}).join(''):emptyStateCard(key,emptyText);
  ...
  function renderPredictions(){
    ...
    $('matchCount').textContent=allRows.length;const primeSeeCount=$('primeSeeAllCardCount');if(primeSeeCount)primeSeeCount.textContent=publicText(`${allRows.length} published`);
    ...
    if(!visible.length)grid.innerHTML=emptyStateCard('prime',publicText('No Prime Picks available yet. This board updates automatically when new picks qualify.'));
  ...
  function overviewDateLabel(key){
    if(!key)return '—';const d=new Date(`${key}T12:00:00`);if(Number.isNaN(d.getTime()))return key;
    return new Intl.DateTimeFormat(locale==='cz'?'cs-CZ':locale==='sk'?'sk-SK':'en-GB',{weekday:'short',day:'2-digit',month:'2-digit',year:'numeric'}).format(d);
  }
  function overviewCategoryMap(){
    const defs=[['prime','Prime'],['top_daily',lcopy('Top Bet','Top tip','Top tip')],['value','Value'],['doubles',lcopy('Doubles','Štvorhra','Čtyřhra')],['ace',lcopy('Aces','Esá','Esa')],['sg',lcopy('S/G','Sety / hry','Sety / hry')]];
    const map=new Map();
    defs.forEach(([key,label])=>marketRows(key).forEach(row=>{const id=String(row?.event_id||row?.id||'');if(!id)return;const current=map.get(id)||[];if(!current.some(x=>x.key===key))current.push({key,label});map.set(id,current);}));
    return map;
  }
  function overviewBaseRows(){
    const categories=overviewCategoryMap();
    return (state.feed?.upcoming||[]).map(normalize).map(m=>({...m,categories:categories.get(String(m.id||''))||[]}));
  }
  function overviewAvailableDates(rows=overviewBaseRows()){
    return [...new Set(rows.map(row=>overviewDateKey(row.date)).filter(Boolean))].sort();
  }
  function overviewEnsureDate(rows){
    const dates=overviewAvailableDates(rows);if(!dates.length){state.overviewFilters.date='';return dates;}
    if(!dates.includes(state.overviewFilters.date)){const today=overviewDateKey(new Date());state.overviewFilters.date=dates.includes(today)?today:dates[0];}
    return dates;
  }
  function overviewFilteredRows(){
    const rows=overviewBaseRows(),dates=overviewEnsureDate(rows),f=state.overviewFilters||{};
    let list=rows.filter(row=>!f.date||overviewDateKey(row.date)===f.date);
    if(f.tour)list=list.filter(row=>row.tour===f.tour);
    if(f.surface)list=list.filter(row=>String(row.surface||'').toLowerCase()===String(f.surface).toLowerCase());
    if(f.category&&f.category!=='all')list=list.filter(row=>row.categories.some(x=>x.key===f.category));
    const q=String(f.query||'').trim().toLowerCase();if(q)list=list.filter(row=>`${row.p1} ${row.p2} ${row.tournament} ${row.round} ${row.pick}`.toLowerCase().includes(q));
    const sort=String(f.sort||'time');
    list.sort((a,b)=>{
      if(sort==='confidence')return (b.probability||0)-(a.probability||0)||new Date(a.date)-new Date(b.date);
      if(sort==='odds')return (Number.isFinite(b.odds)?b.odds:-1)-(Number.isFinite(a.odds)?a.odds:-1)||new Date(a.date)-new Date(b.date);
      if(sort==='ev')return (Number.isFinite(b.expectedValue)?b.expectedValue:-999)-(Number.isFinite(a.expectedValue)?a.expectedValue:-999)||new Date(a.date)-new Date(b.date);
      return new Date(a.date)-new Date(b.date)||String(a.tournament).localeCompare(String(b.tournament));
    });
    return {rows:list,dates,all:rows};
  }
  function overviewDataFacts(m){
    const q=m.quality||{},p1=q.player1||{},p2=q.player2||{};
    const depth=Number(m.dataDepth),s1=Number(p1.surface_matches),s2=Number(p2.surface_matches),history1=Number(p1.history_matches),history2=Number(p2.history_matches);
    const facts=[];
    if(Number.isFinite(depth))facts.push(`${lcopy('Depth','Hĺbka','Hloubka')} ${Math.round((depth<=1?depth*100:depth))}%`);
    if(Number.isFinite(s1)&&Number.isFinite(s2))facts.push(`${lcopy('Surface','Povrch','Povrch')} ${Math.min(s1,s2)}+`);
    else if(Number.isFinite(history1)&&Number.isFinite(history2))facts.push(`${lcopy('History','História','Historie')} ${Math.min(history1,history2)}+`);
    if(m.raw?.stats_available===true)facts.push(lcopy('Stats ✓','Štatistiky ✓','Statistiky ✓'));
    return facts.length?facts.join(' · '):'—';
  }
  function overviewCategoryBadges(categories){
    if(!categories?.length)return `<span class="overview-badge overview-badge-neutral">${escapeHtml(lcopy('Model','Model','Model'))}</span>`;
    return categories.map(item=>`<span class="overview-badge overview-badge-${escapeHtml(item.key)}">${escapeHtml(item.label)}</span>`).join('');
  }
  function overviewSelectOptions(values,selected,allLabel){
    return `<option value="">${escapeHtml(allLabel)}</option>${[...values].filter(Boolean).sort().map(v=>`<option value="${escapeHtml(v)}"${String(v)===String(selected)?' selected':''}>${escapeHtml(String(v).replaceAll('_',' '))}</option>`).join('')}`;
  }
  function overviewLockedRow(m,index){
    const plan=firstUnlockPlan('overview',index,false),label=upgradePlanLabel(plan).replace(/^BlinQ\s+/i,'');
    return `<tr class="overview-row-locked" data-upgrade-plan="${escapeHtml(plan)}" data-upgrade-section="Overview"><td data-label="${escapeHtml(lcopy('Time','Čas','Čas'))}" class="overview-time"><strong>${escapeHtml(fmtTime(m.date))}</strong><small>${escapeHtml(m.tour||'')}</small></td><td data-label="${escapeHtml(lcopy('Match','Zápas','Zápas'))}" class="overview-match"><strong>${escapeHtml(m.p1)}</strong><span>••• <i>vs</i> •••</span><strong>${escapeHtml(m.p2)}</strong><small>${escapeHtml(lcopy('Prediction locked','Predikcia zamknutá','Predikce zamčená'))}</small></td><td data-label="${escapeHtml(lcopy('Tournament','Turnaj','Turnaj'))}" class="overview-tournament"><strong>${escapeHtml(m.tournament)}</strong><small>${escapeHtml(m.round||'—')}</small></td><td data-label="${escapeHtml(lcopy('Surface','Povrch','Povrch'))}"><span class="overview-surface">•••</span></td><td data-label="${escapeHtml(lcopy('Prediction','Predikcia','Predikce'))}" class="overview-prediction overview-masked"><strong>••••••</strong><span class="confidence low">LOCKED</span></td><td data-label="${escapeHtml(lcopy('Odds','Kurz','Kurz'))}" class="overview-number overview-masked"><strong>•••</strong></td><td data-label="${escapeHtml(lcopy('Edge / EV','Výhoda / EV','Výhoda / EV'))}" class="overview-edge overview-masked"><strong>•••</strong><span>•••</span></td><td data-label="${escapeHtml(lcopy('Data','Dáta','Data'))}" class="overview-data overview-masked">••••••</td><td data-label="${escapeHtml(lcopy('Type','Typ','Typ'))}" class="overview-tags"><span class="overview-badge overview-badge-neutral">${escapeHtml(lcopy('Locked','Zamknuté','Zamčeno'))}</span></td><td class="overview-action"><button type="button" class="overview-detail-btn locked" data-upgrade-plan="${escapeHtml(plan)}" data-upgrade-section="Overview">${escapeHtml(lcopy('Unlock','Odomknúť','Odemknout'))} ${escapeHtml(label)} →</button></td></tr>`;
  }
  function renderOverviewPage(){
    const data=overviewFilteredRows(),f=state.overviewFilters||{},rows=data.rows;
    const dayRows=data.all.filter(row=>!f.date||overviewDateKey(row.date)===f.date),tours=new Set(dayRows.map(row=>row.tour)),surfaces=new Set(dayRows.map(row=>row.surface));
    const dates=data.dates.map(key=>`<option value="${escapeHtml(key)}"${key===f.date?' selected':''}>${escapeHtml(overviewDateLabel(key))}</option>`).join('');
    const categoryOptions=[['all',lcopy('All matches','Všetky zápasy','Všechny zápasy')],['prime','Prime'],['top_daily',lcopy('Top Bets','Top tipy','Top tipy')],['value','Value'],['ace',lcopy('Aces','Esá','Esa')],['doubles',lcopy('Doubles','Štvorhra','Čtyřhra')],['sg',lcopy('Sets / Games','Sety / hry','Sety / hry')]].map(([value,label])=>`<option value="${value}"${f.category===value?' selected':''}>${escapeHtml(label)}</option>`).join('');
    const sortOptions=[['time',lcopy('Time','Čas','Čas')],['confidence',lcopy('Prediction confidence','Istota predikcie','Jistota predikce')],['odds',lcopy('Odds','Kurz','Kurz')],['ev','EV']].map(([value,label])=>`<option value="${value}"${f.sort===value?' selected':''}>${escapeHtml(label)}</option>`).join('');
    const ent=dashboardPlanEntitlement('overview'),unlocked=visiblePickCount('overview',rows.length),detailAllowed=ent.see_all!==false;
    const tableRows=rows.map((m,index)=>{
      if(ent.blur_remaining!==false&&index>=unlocked)return overviewLockedRow(m,index);
      const p1Pct=Number.isFinite(m.p1Prob)?pct(m.p1Prob):'—',p2Pct=Number.isFinite(m.p2Prob)?pct(m.p2Prob):'—';
      const odds=Number.isFinite(m.odds)&&m.odds>0?m.odds.toFixed(2):'—';
      const edge=Number.isFinite(m.edge)?`${m.edge>=0?'+':''}${(m.edge*(Math.abs(m.edge)<=1?100:1)).toFixed(1)} pp`:'—';
      const ev=Number.isFinite(m.expectedValue)?`${m.expectedValue>=0?'+':''}${(m.expectedValue*(Math.abs(m.expectedValue)<=1?100:1)).toFixed(1)}%`:'—';
      const evTone=Number.isFinite(m.expectedValue)?(m.expectedValue>0?' positive':m.expectedValue<0?' negative':''):'';
      const surface=String(m.surface||'—').replaceAll('_',' ');
      const detail=detailAllowed?`<button type="button" class="overview-detail-btn" data-overview-detail="${index}">${escapeHtml(lcopy('Detail','Detail','Detail'))} →</button>`:`<button type="button" class="overview-detail-btn locked" data-upgrade-plan="${escapeHtml(firstUnlockPlan('overview',0,true))}" data-upgrade-section="Overview detail">${escapeHtml(lcopy('Unlock detail','Odomknúť detail','Odemknout detail'))} →</button>`;
      return `<tr data-overview-row="${index}"><td data-label="${escapeHtml(lcopy('Time','Čas','Čas'))}" class="overview-time"><strong>${escapeHtml(fmtTime(m.date))}</strong><small>${escapeHtml(m.tour||'')}</small></td><td data-label="${escapeHtml(lcopy('Match','Zápas','Zápas'))}" class="overview-match"><strong>${escapeHtml(m.p1)}</strong><span>${escapeHtml(p1Pct)} <i>vs</i> ${escapeHtml(p2Pct)}</span><strong>${escapeHtml(m.p2)}</strong><small>${escapeHtml(playerMetaLabel(m.p1Rank,m.p1Country))} · ${escapeHtml(playerMetaLabel(m.p2Rank,m.p2Country))}</small></td><td data-label="${escapeHtml(lcopy('Tournament','Turnaj','Turnaj'))}" class="overview-tournament"><strong>${escapeHtml(m.tournament)}</strong><small>${escapeHtml(m.round||'—')}</small></td><td data-label="${escapeHtml(lcopy('Surface','Povrch','Povrch'))}"><span class="overview-surface">${escapeHtml(surface)}</span></td><td data-label="${escapeHtml(lcopy('Prediction','Predikcia','Predikce'))}" class="overview-prediction"><strong>${escapeHtml(m.pick)}</strong><span class="confidence ${escapeHtml(m.confidence)}">${escapeHtml(pct(m.probability))}</span></td><td data-label="${escapeHtml(lcopy('Odds','Kurz','Kurz'))}" class="overview-number"><strong>${escapeHtml(odds)}</strong></td><td data-label="${escapeHtml(lcopy('Edge / EV','Výhoda / EV','Výhoda / EV'))}" class="overview-edge"><strong>${escapeHtml(edge)}</strong><span class="${evTone.trim()}">${escapeHtml(ev)}</span></td><td data-label="${escapeHtml(lcopy('Data','Dáta','Data'))}" class="overview-data">${escapeHtml(overviewDataFacts(m))}</td><td data-label="${escapeHtml(lcopy('Type','Typ','Typ'))}" class="overview-tags">${overviewCategoryBadges(m.categories)}</td><td class="overview-action">${detail}</td></tr>`;
    }).join('');
    const empty=`<div class="overview-empty"><span>⌕</span><strong>${escapeHtml(lcopy('No matches match these filters.','Pre zvolené filtre nie sú dostupné žiadne zápasy.','Pro zvolené filtry nejsou dostupné žádné zápasy.'))}</strong><small>${escapeHtml(lcopy('Try another day or remove a filter.','Skús iný deň alebo zruš niektorý filter.','Zkus jiný den nebo zruš některý filtr.'))}</small></div>`;
    return `<section class="overview-page"><div class="overview-summary"><div><span class="overview-summary-icon">▦</span><div><strong>${rows.length}</strong><small>${escapeHtml(lcopy('matches shown','zobrazených zápasov','zobrazených zápasů'))}</small></div></div><div><span>${escapeHtml(lcopy('Selected day','Vybraný deň','Vybraný den'))}</span><strong>${escapeHtml(overviewDateLabel(f.date))}</strong></div><p>${escapeHtml(lcopy('Model probabilities are point-in-time predictions. Odds and EV appear only where a verified market snapshot is available.','Modelové pravdepodobnosti sú point-in-time predikcie. Kurz a EV sa zobrazia iba tam, kde máme overený trhový snapshot.','Modelové pravděpodobnosti jsou point-in-time predikce. Kurz a EV se zobrazí pouze tam, kde máme ověřený tržní snapshot.'))}</p></div><div class="overview-toolbar"><label><span>${escapeHtml(lcopy('Day','Deň','Den'))}</span><select id="overviewDate">${dates}</select></label><label><span>${escapeHtml(lcopy('Tour','Okruh','Okruh'))}</span><select id="overviewTour">${overviewSelectOptions(tours,f.tour,lcopy('All tours','Všetky okruhy','Všechny okruhy'))}</select></label><label><span>${escapeHtml(lcopy('Surface','Povrch','Povrch'))}</span><select id="overviewSurface">${overviewSelectOptions(surfaces,f.surface,lcopy('All surfaces','Všetky povrchy','Všechny povrchy'))}</select></label><label><span>${escapeHtml(lcopy('Type','Typ','Typ'))}</span><select id="overviewCategory">${categoryOptions}</select></label><label><span>${escapeHtml(lcopy('Sort','Zoradiť','Seřadit'))}</span><select id="overviewSort">${sortOptions}</select></label><label class="overview-search"><span>${escapeHtml(lcopy('Search','Hľadať','Hledat'))}</span><input id="overviewSearch" type="search" value="${escapeHtml(f.query||'')}" placeholder="${escapeHtml(lcopy('Player or tournament…','Hráč alebo turnaj…','Hráč nebo turnaj…'))}"></label></div>${rows.length?`<div class="overview-table-shell"><table class="overview-table"><thead><tr><th>${escapeHtml(lcopy('Time','Čas','Čas'))}</th><th>${escapeHtml(lcopy('Match','Zápas','Zápas'))}</th><th>${escapeHtml(lcopy('Tournament','Turnaj','Turnaj'))}</th><th>${escapeHtml(lcopy('Surface','Povrch','Povrch'))}</th><th>${escapeHtml(lcopy('Prediction','Predikcia','Predikce'))}</th><th>${escapeHtml(lcopy('Odds','Kurz','Kurz'))}</th><th>${escapeHtml(lcopy('Edge / EV','Výhoda / EV','Výhoda / EV'))}</th><th>${escapeHtml(lcopy('Data','Dáta','Data'))}</th><th>${escapeHtml(lcopy('Type','Typ','Typ'))}</th><th></th></tr></thead><tbody>${tableRows}</tbody></table></div>`:empty}</section>`;
  }

  function wireOverview(){
    const bind=(id,key)=>{const el=$(id);if(!el)return;el.onchange=()=>{state.overviewFilters[key]=el.value;renderRoute('overview');};};
    bind('overviewDate','date');bind('overviewTour','tour');bind('overviewSurface','surface');bind('overviewCategory','category');bind('overviewSort','sort');
    const search=$('overviewSearch');if(search){let timer;search.oninput=()=>{clearTimeout(timer);const value=search.value;timer=setTimeout(()=>{state.overviewFilters.query=value;renderRoute('overview');},160);};}
    $('routePanel')?.querySelectorAll('[data-overview-detail]').forEach(button=>{button.onclick=()=>{const {rows}=overviewFilteredRows(),row=rows[Number(button.dataset.overviewDetail)];if(row)openMatch(row);};});
  }

  const routeMetaEn={
    predictions:['TENNIS INTELLIGENCE','Dashboard','Prime Picks, daily selections, market picks and current BlinQ intelligence.'],overview:['ALL MATCHES','Overview','All available matches for the selected day with odds, model prediction and core data.'],prime:['MATCH WINNER','Prime Picks','Accuracy-first Match Winner picks; preferred odds 1.20–1.50, without a hard odds band.'],top_daily:['CONFIDENCE FIRST','Top Bets','Strongest remaining Match Winner picks, ranked by model probability and data quality.'],value:['VALUE EDGE','Value Picks','Higher-priced model-vs-market opportunities with stricter edge and EV guardrails.'],doubles:['DOUBLES','Doubles','Separate doubles model and team-pair intelligence.'],ace:['ACES + DOUBLE FAULTS','Ace Picks','Top Aces and Double Faults market selections.'],sg:['SETS + GAMES','S/G Picks','Top Sets and Games market selections.'],results:['SETTLED PICKS','Results','Settled selections, hit rate, ROI, units and related performance statistics.'],btts:['FOOTBALL · BETA','BTTS Bonus','Both Teams To Score is prepared as a separate BlinQ beta module.'],account:['BLINQ MEMBERS','Account','Profile, security, membership and access.'],admin:['BLINQ CONTROL','Admin Control Center','Internal model performance, backtests, publishing, access and workspace configuration.'],how_blinq_works:['LEARN','How BlinQ Works','How the BlinQ workflow turns point-in-time tennis data into probabilities.'],methodology:['LEARN','Methodology','The principles used to keep predictions point-in-time and auditable.'],model_data:['LEARN','Model & Data','What the published feed exposes about data and model state.'],faq:['LEARN','FAQ','Common questions about probabilities, results and model output.'],responsible_use:['LEARN','Responsible Use','Use probabilities as information, never as guarantees.']
  };
  const routeMetaSk={
    predictions:['TENISOVÁ ANALYTIKA','Prehľad','Prime tipy, denné výbery, trhové tipy a aktuálna BlinQ analytika.'],overview:['VŠETKY ZÁPASY','Overview','Prehľad všetkých zápasov + kurzy + predikcia + základné dáta, ktoré máme.'],prime:['VÍŤAZ ZÁPASU','Prime tipy','Tipy na víťaza zápasu orientované na presnosť; preferovaný kurz 1,20–1,50 bez pevného kurzového pásma.'],top_daily:['ISTOTA NA PRVOM MIESTE','Top tipy','Najsilnejšie zostávajúce tipy na víťaza zápasu zoradené podľa pravdepodobnosti modelu a kvality dát.'],value:['VALUE VÝHODA','Value tipy','Príležitosti s vyšším kurzom, kde model nachádza výhodu oproti trhu a používa prísnejšie EV pravidlá.'],doubles:['ŠTVORHRA','Štvorhra','Samostatný model štvorhry a inteligencia dvojíc/tímov.'],ace:['ESÁ + DVOJCHYBY','Esá','Najlepšie projekcie pre esá a dvojchyby.'],sg:['SETY + HRY','Sety / hry','Najlepšie projekcie pre sety a počet hier.'],results:['VYHODNOTENÉ TIPY','Výsledky','Vyhodnotené tipy, úspešnosť, ROI, jednotky a súvisiace štatistiky výkonu.'],btts:['FUTBAL · BETA','BTTS Bonus','Both Teams To Score je pripravený ako samostatný beta modul BlinQ.'],account:['BLINQ ČLENSTVO','Účet','Profil, zabezpečenie, členstvo a prístup.'],how_blinq_works:['INFO','Ako funguje BlinQ','Ako BlinQ mení point-in-time tenisové dáta na pravdepodobnosti.'],methodology:['INFO','Metodika','Princípy, ktoré udržujú predikcie point-in-time a auditovateľné.'],model_data:['INFO','Model a dáta','Čo publikovaný feed ukazuje o dátach a stave modelu.'],faq:['INFO','FAQ','Najčastejšie otázky o pravdepodobnostiach, výsledkoch a výstupe modelu.'],responsible_use:['INFO','Zodpovedné používanie','Pravdepodobnosti používaj ako informáciu, nikdy nie ako záruku.']
  };
  const routeMetaCz={
    predictions:['TENISOVÁ ANALYTIKA','Přehled','Prime tipy, denní výběry, tržní tipy a aktuální BlinQ analytika.'],overview:['VŠECHNY ZÁPASY','Overview','Přehled všech zápasů + kurzy + predikce + základní data, která máme.'],prime:['VÍTĚZ ZÁPASU','Prime tipy','Tipy na vítěze zápasu orientované na přesnost; preferovaný kurz 1,20–1,50 bez pevného kurzového pásma.'],top_daily:['JISTOTA NA PRVNÍM MÍSTĚ','Top tipy','Nejsilnější zbývající tipy na vítěze zápasu seřazené podle pravděpodobnosti modelu a kvality dat.'],value:['VALUE VÝHODA','Value tipy','Příležitosti s vyšším kurzem, kde model nachází výhodu oproti trhu a používá přísnější EV pravidla.'],doubles:['ČTYŘHRA','Čtyřhra','Samostatný model čtyřhry a inteligence dvojic/týmů.'],ace:['ESA + DVOJCHYBY','Esa','Nejlepší projekce pro esa a dvojchyby.'],sg:['SETY + HRY','Sety / hry','Nejlepší projekce pro sety a počet her.'],results:['VYHODNOCENÉ TIPY','Výsledky','Vyhodnocené tipy, úspěšnost, ROI, jednotky a související statistiky výkonu.'],btts:['FOTBAL · BETA','BTTS Bonus','Both Teams To Score je připraven jako samostatný beta modul BlinQ.'],account:['BLINQ ČLENSTVÍ','Účet','Profil, zabezpečení, členství a přístup.'],how_blinq_works:['INFO','Jak funguje BlinQ','Jak BlinQ mění point-in-time tenisová data na pravděpodobnosti.'],methodology:['INFO','Metodika','Principy, které udržují predikce point-in-time a auditovatelné.'],model_data:['INFO','Model a data','Co publikovaný feed ukazuje o datech a stavu modelu.'],faq:['INFO','FAQ','Nejčastější otázky o pravděpodobnostech, výsledcích a výstupu modelu.'],responsible_use:['INFO','Zodpovědné používání','Pravděpodobnosti používej jako informaci, nikdy ne jako záruku.']
  };
  const routeMeta=locale==='sk'?{...routeMetaEn,...routeMetaSk}:locale==='cz'?{...routeMetaEn,...routeMetaCz}:routeMetaEn;

  function syncAdminPreviewBar(){
    const bar=$('adminPreviewBar'),label=$('adminPreviewLabel');if(!bar)return;
    const active=Boolean(isAdminAccount()&&state.previewPlan);bar.hidden=!active;
    if(label)label.textContent=active?`ADMIN PREVIEW · Viewing as ${accessLabel(state.previewPlan)}`:'';
    document.documentElement.classList.toggle('admin-preview-active',active);
  }
  function setRoute(route,push=true){ if(!routeMeta[route]) route='predictions'; if(route==='admin'&&!isAdminAccount()) route='predictions'; const section=dashboardSectionKeys.includes(route)?dashboardSectionConfig(route):null; if(section&&route!=='predictions'&&elementAccess(section.sidebar_element)!=='active'&&state.route!=='admin'){showUpgradePrompt(firstUnlockPlan(route,0,true),section.label||route);route='predictions';} if(route==='admin') state.previewPlan=null; const routeChanged=state.route!==route;state.route=route;syncAdminPreviewBar();if(routeChanged)state.page=0;const meta=routeMeta[route]; const overview=route==='predictions'; const routeHeading=$('routeHeading'); if(routeHeading) routeHeading.hidden=overview; $('pageEyebrow').textContent=meta[0]; $('pageTitle').textContent=meta[1]; $('pageSubtitle').textContent=meta[2]; const topbar=document.querySelector('.dashboard-topbar'); if(topbar) topbar.classList.toggle('overview-mode',overview); $('predictionsView').hidden=!overview; $('routePanel').hidden=overview; renderNavigation(); if(overview){renderPredictions();renderMarketSections();renderDashboardResultsPreview();applyAccessStates();} else renderRoute(route); if(push&&location.hash!==`#${route}`) history.pushState(null,'',`#${route}`); window.BlinqUI.routeChanged(route,push); updateLanguageLinks(); document.title=`${meta[1]} · BlinQ`; if(route!=='admin')translatePublicDom(document.body); }
  ...
  function adminOverviewRuleRow(plan){
    const key='overview',cfg=dashboardSectionConfig(key),ent=cfg.plans?.[plan]||cfg.plans?.rookie||{},routeAccess=adminSectionRouteAccess(key,plan);
    return `<div class="admin-section-rule-row compact" data-dashboard-plan-row="${escapeHtml(plan)}"><b>${escapeHtml(state.ui?.plans?.[plan]?.label||plan.toUpperCase())}${plan==='goat'?'<small>TOP</small>':''}</b><label><span>Page access</span><select data-dashboard-route-access>${adminSectionAccessOptions(routeAccess)}</select></label><label><span>Visible matches</span><select data-dashboard-matrix-field="visible_picks">${adminVisiblePickOptions(ent.visible_picks)}</select></label><label class="admin-toggle-line"><input type="checkbox" data-dashboard-matrix-field="blur_remaining" ${ent.blur_remaining===true?'checked':''}><span>Blur remaining</span></label><label class="admin-toggle-line"><input type="checkbox" data-dashboard-matrix-field="see_all" ${ent.see_all!==false?'checked':''}><span>Match detail</span></label></div>`;
  }
  function renderAdminOverviewCard(){
    const key='overview',cfg=dashboardSectionConfig(key);
    return `<article class="admin-section-card admin-overview-card" data-dashboard-section="overview"><div class="admin-section-card-head"><div><small>FULL MATCH TABLE</small><h3>Overview</h3><p>All matches, odds, prediction and core data. Configure access exactly like the pick sections.</p></div><div class="admin-section-card-switches"><span>GLOBAL</span><label><input type="checkbox" data-dashboard-field="sidebar_enabled" ${cfg.sidebar_enabled!==false?'checked':''}> Top nav</label></div></div>${adminSectionPresetButtons(key)}${adminOverviewRuleRow(state.adminPlan)}<details class="admin-all-levels"><summary>All membership levels</summary><div class="admin-all-level-rules">${accessContexts.map(adminOverviewRuleRow).join('')}</div></details></article>`;
  }
  ...
  function wireAdmin(){
    ...
    host.onclick=async event=>{
      ...
      const sectionPreset=event.target.closest('[data-section-preset]');if(sectionPreset){const key=sectionPreset.dataset.sectionKey,plan=state.adminPlan,cfg=key?state.ui?.dashboard?.sections?.[key]:null,id=cfg?.sidebar_element,item=id?elements()?.[id]:null;if(cfg&&item&&plan){cfg.plans=cfg.plans||{};cfg.plans[plan]=cfg.plans[plan]||{};item.access=item.access||{};const ent=cfg.plans[plan],preset=sectionPreset.dataset.sectionPreset;if(preset==='full'){item.access[plan]='active';ent.visible_picks='ALL';ent.blur_remaining=false;ent.see_all=true;}else if(preset==='teaser'){item.access[plan]='active';ent.visible_picks=1;ent.blur_remaining=true;ent.see_all=true;}else if(preset==='blurred'){item.access[plan]=key==='overview'?'active':'locked';ent.visible_picks=0;ent.blur_remaining=true;ent.see_all=false;}else if(preset==='hidden'){item.access[plan]='hidden';ent.visible_picks=0;ent.blur_remaining=true;ent.see_all=false;}if(plan==='rookie'){item.access.trial=item.access[plan];cfg.plans.trial=clone(ent);}state.dashboardVisibility=null;renderAllUiContent();rerenderAdmin();showStatus(`${cfg.label||key} · ${accessLabel(plan)} preset applied.`);}return;}
      ...
      else if(action==='clear-preview'){state.previewPlan=null;renderAllUiContent();syncAdminPreviewBar();if(state.route==='admin')rerenderAdmin();else setRoute(state.route,false);showStatus('Admin preview disabled — full ADMIN access restored.');}
  ...
  function primeDetailCard(m,index=0){
    ...
    const metricHtml=metrics.map(([label,value])=>{const raw=String(value);const tone=raw.trim().startsWith('+')?' metric-positive':raw.trim().startsWith('-')?' metric-negative':'';return `<span class="card-metric${tone}"><small>${escapeHtml(label)}</small><strong>${escapeHtml(raw)}</strong></span>`}).join('');
    return `<article class="prediction-card featured detail-pick-card match-card-v3"><div class="card-meta match-card-meta"><span class="tour">${escapeHtml(m.tour)} ${escapeHtml(m.tournament)}</span><span class="time">${escapeHtml(fmtTime(m.date))}</span><span class="surface">${escapeHtml(String(m.surface||'').replaceAll('_',' ').toUpperCase())}</span></div><div class="players-row match-players-row"><div class="player">${avatar(photo1,m.p1)}<strong class="player-name">${escapeHtml(m.p1)}</strong></div><div class="vs match-vs">VS</div><div class="player">${avatar(photo2,m.p2)}<strong class="player-name">${escapeHtml(m.p2)}</strong></div></div><div class="pick-row match-pick-row"><div class="pick-copy"><small>${escapeHtml(lcopy('BlinQ pick','Náš tip','Náš tip'))}</small><strong class="pick-name">${escapeHtml(m.pick)}</strong></div><div class="pick-score"><small class="probability-label">Model</small><div class="probability">${pct(m.probability)}</div><span class="confidence ${escapeHtml(m.confidence)}">${escapeHtml(confidenceLabel(m.confidence))}</span></div></div><div class="card-metrics-bar match-kpi-bar">${metricHtml}</div></article>`;
  }
  ...
  function renderRoute(route){
    ...
    if(route==='overview') body=renderOverviewPage();
    else if(route==='prime'){
    ...
    host.innerHTML=`<div class="route-card route-card-clean">${body}</div>`; if(route!=='admin')translatePublicDom(host); window.BlinqUI.prepareRoute(host); if(route==='overview')wireOverview(); if(route==='results')wireResultsFilters(); if(route==='account')wireAccountPage(); applyAccessStates(host);
  }
  ...
  function setupEvents(){
    ...
    $('headerLogoutButton').onclick=signOutCurrentSession;const previewExit=$('adminPreviewExit');if(previewExit)previewExit.onclick=()=>{state.previewPlan=null;renderAllUiContent();syncAdminPreviewBar();setRoute(state.route,false);showStatus('ADMIN preview ended. Full admin access restored.');};$('upgradeDialogClose').onclick=()=>$('upgradeDialog').close();$('upgradeDialog').addEventListener('click',e=>{if(e.target===$('upgradeDialog'))$('upgradeDialog').close()});
    document.addEventListener('click',e=>{
      ...
      const upgradeTarget=e.target.closest('[data-upgrade-plan]');if(upgradeTarget&&state.route!=='admin'){e.preventDefault();e.stopPropagation();if(isAdminAccount()&&!state.previewPlan){renderAllUiContent();setRoute(state.route,false);showStatus('ADMIN access restored. Upgrade gating is ignored for administrators.');return;}showUpgradePrompt(upgradeTarget.dataset.upgradePlan||'pro',upgradeTarget.dataset.upgradeSection||'this content');return;}
