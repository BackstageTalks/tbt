(() => {
  ...
  async function loadUiConfig() {
    ...
    try {
      state.uiSource = await getJSON('/ui-config.json?v=7360&p=24');
    } catch {
      state.uiSource = {schema:2,navigation:{learn:[]},plans:{},elements:{},admin:{draft_storage_key:'blinq_admin_ui_config_v1'}};
    }
    try {
      const telegramConfig = await getJSON('/config/telegram-groups.json?v=7360&p=24');
  ...
  function setAdminPlanDefaults(planId, force=false){
    ...
    if(plan.lifetime||planIsUnlimited(planId,plan)||planId==='goat'){expiry.value='';expiry.disabled=true;return;}
  ...
  function adminAddDaysToExpiry(days){
    ...
    if(plan.lifetime||planIsUnlimited(planId,plan)){setAdminPlanDefaults(planId,true);showStatus(planIsUnlimited(planId,plan)?'ROOKIE je bez časového obmedzenia.':'Tento level nemá expiráciu.');return;}
  ...
  function adminDefaultPlanDays(){
    const planId=String($('adminUserPlan')?.value||'');const plan=state.ui?.plans?.[planId]||{};return plan.lifetime||planIsUnlimited(planId,plan)||planId==='goat'?0:Math.max(1,Number(plan.duration_days)||30);
  }
  ...
  function wireAdmin(){
    ...
    host.onclick=async event=>{
      ...
      else if(action==='extend-default-term'){const plan=String($('adminUserPlan')?.value||'');if(!plan){showStatus('Najprv vyber level.');return;}const cfg=state.ui?.plans?.[plan]||{};if(plan==='goat'||planIsUnlimited(plan,cfg)){setAdminPlanDefaults(plan,true);showStatus(planIsUnlimited(plan,cfg)?'ROOKIE je bez časového obmedzenia.':'GOAT má doživotnú platnosť.');return;}adminAddDaysToExpiry(adminDefaultPlanDays());}
