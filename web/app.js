(() => {
  ...
  const dashboardCardsPerPanel = () => 1; // v6.5.12: dashboard is a lightweight one-pick preview; See more opens 3–5 picks.
  ...
  // v6.5.12: the public site is Slovak-first. Czech and English are optional public mutations.
  ...
  const PUBLIC_TRANSLATIONS = {
    ...
    cz: {
      ...
      'Your account':'Tvůj účet','YOUR ACCOUNT':'TVŮJ ÚČET','CURRENT ACCESS':'AKTUÁLNÍ PŘÍSTUP','Email verified':'E-mail ověřen','Email not verified':'E-mail není ověřen','Telegram nick':'Telegram nick','Avatar style':'Typ avatara','Default':'Výchozí','Male':'Muž','Female':'Žena','Save profile':'Uložit profil','Reset password':'Obnovit heslo','BLINQ MEMBERSHIP':'BLINQ ČLENSTVÍ','Available plans':'Dostupné plány','Current plan':'Aktuální plán','CURRENT PLAN':'AKTUÁLNÍ PLÁN','ONLINE PURCHASE NOT OPEN YET':'ONLINE NÁKUP ZATÍM NENÍ DOSTUPNÝ','INVITE ONLY':'JEN NA POZVÁNKU','Invite only':'Jen na pozvánku','Request invite':'Požádat o pozvánku','Lifetime access':'Doživotní přístup','Active':'Aktivní','No active access':'Bez aktivního přístupu','Rookie Trial':'Rookie zkušební verze','Lifetime':'Doživotně','Active membership':'Aktivní členství','Expired':'Expirované','Access':'Přístup','Member since':'Člen od','Security':'Zabezpečení','Verified email':'Ověřený e-mail','Verification required':'Vyžaduje se ověření',
  ...
  function renderNavigation(){
    ...
    const main=primaryNav.map(([id,label,icon],index)=>{
      ...
      return {id,label:publicText(label),icon,order:index+1,enabled:section?section.sidebar_enabled!==false:true,beta:id==='btts',href:id==='btts'?'#btts':`#${id}`,element_id:item?.id||''};
    });
  ...
  function bannerHtml(item, sidebar=false, index=0, layoutCount=1){
    const rawContent=resolvedBannerContent(item,index),c={...rawContent,eyebrow:publicText(rawContent.eyebrow||''),headline:publicText(rawContent.headline||''),accent_text:publicText(rawContent.accent_text||''),text:publicText(rawContent.text||''),button_text:publicText(rawContent.button_text||'')},route=c.route||'',href=safeLink(c.link,route?`#${route}`:'#account'),external=isExternalLink(href); const theme=String(c.theme||'violet').replace(/[^a-z0-9_-]/gi,'');
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    let meta='',badge='',mainValue='—',confidenceClass='low',pickLabel=publicText(key==='prime'?'Prime Pick':key==='value'?'Value Pick':key==='ace'?'Ace / DF Pick':key==='sg'?'Set / Game Pick':'Top Bet');
    if(projectionOnly){
      ...
      if(key==='sg'&&unit==='probability'){mainValue=`${pct(projection)}<small> proj.</small>`;meta=[marketLabel,Number.isFinite(reference)?`Baseline ${pct(reference)}`:'',Number.isFinite(projectionGap)?`Gap ${(projectionGap*100).toFixed(1)} pp`:'',Number.isFinite(projectionConfidence)?`Score ${Math.round(projectionConfidence*100)}/100`:'',sampleText,'Projection only · no odds'].filter(Boolean).join(' · ');pickLabel=publicText('Sets Projection');}
      else if(key==='sg'&&unit==='games'){mainValue=`${projection.toFixed(1)}<small> games</small>`;meta=[marketLabel,Number.isFinite(reference)?`Baseline ${reference.toFixed(1)}`:'',Number.isFinite(projectionGap)?`Δ ${projectionGap.toFixed(1)} games`:'',Number.isFinite(projectionConfidence)?`Score ${Math.round(projectionConfidence*100)}/100`:'',sampleText,'Projection only · no odds'].filter(Boolean).join(' · ');pickLabel=publicText('Games Projection');}
      else{meta=[marketLabel,Number.isFinite(opponentProjection)?`Opponent proj. ${opponentProjection.toFixed(1)}`:'',Number.isFinite(projectionGap)?`Gap +${projectionGap.toFixed(1)}`:'',Number.isFinite(projectionConfidence)?`Score ${Math.round(projectionConfidence*100)}/100`:'',sampleText,'Projection only · no odds'].filter(Boolean).join(' · ');mainValue=`${projection.toFixed(1)}<small> proj.</small>`;pickLabel=publicText('Ace / DF Projection');}
      badge=publicText('PROJECTION');confidenceClass=Number.isFinite(projectionConfidence)?confidenceBand(projectionConfidence):'medium';
    }else{
  ...
  function renderCard(m, slotIndex=0){
    ...
    const bettingRow=card.querySelector('.betting-row');if(bettingRow&&Number.isFinite(m.odds)&&m.odds>1){bettingRow.hidden=false;card.querySelector('.betting-odds').textContent=`${publicText('Odds')} ${m.odds.toFixed(2)}`;card.querySelector('.betting-edge').textContent=Number.isFinite(m.edge)?`${publicText('Model edge')} ${m.edge>=0?'+':''}${(m.edge*100).toFixed(1)} pp`:`${publicText('Model edge')} —`;card.querySelector('.betting-ev').textContent=Number.isFinite(m.expectedValue)?`EV ${m.expectedValue>=0?'+':''}${(m.expectedValue*100).toFixed(1)}%`:'EV —';}
  ...
  function primeDetailCard(m,index=0){
    ...
    return `<article class="prediction-card featured detail-pick-card"><div class="card-meta"><span class="tour">${escapeHtml(m.tour)} ${escapeHtml(m.tournament)}</span><span class="time">${escapeHtml(fmtTime(m.date))}</span><span class="surface">${escapeHtml(String(m.surface||'').replaceAll('_',' ').toUpperCase())}</span></div><div class="players-row"><div class="player">${avatar(photo1,m.p1)}<strong class="player-name">${escapeHtml(m.p1)}</strong></div><div class="vs">VS</div><div class="player">${avatar(photo2,m.p2)}<strong class="player-name">${escapeHtml(m.p2)}</strong></div></div><div class="pick-row"><div><small>BlinQ Pick</small><strong class="pick-name">${escapeHtml(m.pick)}</strong></div><div class="probability">${pct(m.probability)}</div><span class="confidence ${escapeHtml(m.confidence)}">${escapeHtml(confidenceLabel(m.confidence))}</span></div><div class="market-card-meta">${escapeHtml(meta)}</div></article>`;
  }
