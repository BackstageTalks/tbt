(() => {
  ...
  function renderAllUiContent(){ if(state.bannerObserver){state.bannerObserver.disconnect();state.bannerObserver=null;}state.bannerTimers=new WeakMap();renderNavigation(); wireDashboardSearch(); applyManagedPageBackground(); renderHeaderSlots(); renderHeroBanner(); renderBanners(); renderSidebarPromos(); renderVipRail(); renderFooterConfig(); renderMarketSections(); renderDashboardResultsPreview(); renderDashboardComposition(); renderDashboardKpis(); if(state.railMatch)renderMatchRail();else renderDashboardRightRailDefault(); refreshTopPlanCta(); updateLanguageLinks(); applyAccessStates(); renderInsightBell(); translatePublicDom(document.body); }
  ...
  function renderInsightBell(){
    ...
    const eligible=Boolean(account.is_admin||String(account.role||'').toLowerCase()==='admin'||(['active','lifetime'].includes(status)&&['elite','legend','goat'].includes(plan)));
  ...
  function renderDailyHub(){
    ...
    $('dailyHubTabs').innerHTML=tabs.map(tab=>{
      ...
      return `<button type="button" role="tab" aria-selected="${tab===state.dailyHubTab?'true':'false'}" class="daily-hub-tab${tab===state.dailyHubTab?' active':''}${locked?' is-locked':''}${coming?' is-coming':''}" data-daily-hub-tab="${tab}"><span>${escapeHtml(dailyHubTabLabel(tab))}</span>${count}</button>`;
    }).join('');
  ...
  function renderAdminBanners(){
    ...
    const bg=safePhotoUrl(c.site_background_url||'')||String(state.presentationConfig?.theme?.background?.image||'/assets/custom/site-background.webp');
    ...
    return `<section class="admin-ux-section lean-admin-banners" data-simple-banner="HERO_BANNER_1">
      ...
      <div class="admin-form-section"><div class="admin-form-section-title"><strong>Grafické podklady</strong><span>Jasné rozmery, žiadne kampane ani ďalšie sloty.</span></div><div class="admin-form-grid">
        ...
        <label class="span-2">Pozadie stránky · 1920×1080<input data-simple-banner-field="site_background_url" value="${escapeHtml(c.site_background_url||bg)}" placeholder="/assets/custom/site-background.webp"></label>
  ...
  function renderAdminInsights(){
    ...
    const premiumLevels=['elite','legend','goat'];
    const levelChecks=premiumLevels.map(level=>`<label class="admin-insight-level"><input type="checkbox" name="insight_level" value="${level}" ${levels.includes(level)||!item.id?'checked':''}><span>${escapeHtml(String(state.ui?.plans?.[level]?.label||level).replace(/^BlinQ\s+/i,''))}</span></label>`).join('');
    ...
    return `<section class="admin-ux-section admin-insights-section"><div class="admin-ux-heading"><div><small>SPRÁVY & LIVE</small><h2>Premium Info a LIVE signály</h2><p>Jednosmerný feed od ELITE vyššie. Premium Info píšeš ručne; LIVE je pripravený pre automatické Comeback Radar signály.</p></div><button type="button" class="btn btn-ghost" data-admin-action="insight-new">Nová správa</button></div>${state.adminInsightsError?`<div class="admin-runtime-note is-error"><strong>Feed nie je dostupný</strong><span>${escapeHtml(state.adminInsightsError)}</span></div>`:''}<div class="admin-insights-grid"><form id="adminInsightForm" class="admin-insight-composer"><div class="admin-insight-composer-head"><div><small>${item.id?'UPRAVIŤ':'NOVÁ SPRÁVA'}</small><h3>${item.id?escapeHtml(item.title):'Napíš Premium Info alebo LIVE upozornenie'}</h3></div><span>ELITE+</span></div><label>Nadpis<input id="adminInsightTitle" maxlength="140" required value="${escapeHtml(item.title||'')}"></label><label>Správa<textarea id="adminInsightBody" maxlength="4000" required rows="7">${escapeHtml(item.body||'')}</textarea></label><div class="admin-form-grid"><label>Typ<select id="adminInsightType"><option value="vip"${String(item.type||'vip')==='vip'?' selected':''}>Premium Info</option><option value="alert"${item.type==='alert'?' selected':''}>LIVE</option></select></label><label>Priorita<select id="adminInsightPriority"><option value="normal"${item.priority==='normal'?' selected':''}>Normal</option><option value="important"${item.priority==='important'?' selected':''}>Important</option><option value="critical"${item.priority==='critical'?' selected':''}>Critical</option></select></label></div><fieldset class="admin-insight-audience"><legend>Viditeľné pre</legend><div>${levelChecks}</div><small>Rookie a PRO Premium/LIVE feed nedostávajú.</small></fieldset><div class="admin-form-grid"><label>Event ID / zápas (voliteľné)<input id="adminInsightMatchId" value="${escapeHtml(item.match_id||'')}"></label><label>Text odkazu<input id="adminInsightLinkLabel" maxlength="80" value="${escapeHtml(item.link_label||'')}"></label><label class="span-2">Odkaz (voliteľné)<input id="adminInsightLink" value="${escapeHtml(item.link||'')}"></label><label>Aktívne od<input id="adminInsightFrom" type="datetime-local" value="${escapeHtml(adminDatetimeValue(item.active_from))}"></label><label>Aktívne do<input id="adminInsightUntil" type="datetime-local" value="${escapeHtml(adminDatetimeValue(item.active_until))}"></label></div><div class="admin-insight-flags"><label><input id="adminInsightActive" type="checkbox" ${item.active!==false?'checked':''}> Aktívna</label><label><input id="adminInsightPinned" type="checkbox" ${item.pinned?'checked':''}> Pripnúť hore</label></div><div class="admin-insight-actions"><button class="btn btn-primary" type="submit">${item.id?'Uložiť':'Publikovať'}</button>${item.id?'<button class="btn btn-ghost" type="button" data-admin-action="insight-new">Zrušiť</button>':''}<span id="adminInsightMessage"></span></div></form><div class="admin-insight-list"><div class="admin-subsection-heading"><div><strong>Publikované</strong><span>${state.adminInsightsLoading?'Načítavam…':`${list.length} správ`}</span></div></div>${state.adminInsightsLoading?'<div class="admin-note"><strong>Načítavam…</strong></div>':rows||'<div class="admin-note"><strong>Zatiaľ žiadne správy</strong><span>Premium Info sa zobrazí v zvončeku ELITE+ používateľov.</span></div>'}</div></div></section>`;
  }
  ...
  function renderAdminRoute(){
    const tabs=[['accounts','Účty','Level · platnosť'],['banners','Bannery','Hero · background'],['insights','Správy & LIVE','Premium Info · LIVE']];
    ...
    const info={accounts:['Účty','Správa levelu, platnosti a používateľského prístupu.'],banners:['Bannery','Jeden hero banner, mobilný podklad, pozadie a editovateľný HTML text.'],insights:['Správy & LIVE','Premium Info a LIVE signály pre ELITE, LEGEND a GOAT.']}[state.adminTab];
    let contextual='';
    ...
    return `<div class="admin-console admin-console-v685 admin-console-v687 lean-admin-console"><aside class="admin-side-nav"><div class="admin-side-brand"><span>BLINQ CONTROL</span><strong>Admin</strong><small>3 veci. Nič navyše.</small></div><nav class="admin-tabs admin-tabs-v685" aria-label="Admin navigácia"><div class="admin-nav-group"><span>SPRÁVA</span>${nav}</div></nav><div class="admin-side-foot"><a href="#predictions" data-route="predictions"><svg viewBox="0 0 20 20"><path d="M4 10h12M9 5l-5 5 5 5"></path></svg><span>Späť na web</span></a></div></aside><section class="admin-workarea"><header class="admin-workarea-head admin-control-toolbar"><div><small>ADMIN / ${escapeHtml(state.adminTab.toUpperCase())}</small><h2>${escapeHtml(info[0])}</h2><p>${escapeHtml(info[1])}</p></div><div class="admin-global-actions">${contextual}</div></header><div class="admin-panel admin-panel-v685">${panel}</div></section></div>`;
  }
