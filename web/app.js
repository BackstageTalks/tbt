(() => {
    ...
    try {
      ...
      if(runtime?.configured && runtime?.config?.schema===2){
        ...
        if(String(runtime.config.ui_revision||'')!==String(state.uiSource.ui_revision||'')){
          ...
          if(['6.7.0','6.7.3','6.7.4','6.7.5','6.7.6'].includes(String(state.uiSource.ui_revision||''))){
  ...
  function renderAccountModal(){
    ...
    return `<div class="account-modal-head"><div><small>BLINQ ÚČET</small><h2 id="accountDialogTitle">${escapeHtml(a.email||'BlinQ účet')}</h2>${a.telegram_nick?`<p>${adminTelegramIcon()} ${escapeHtml(a.telegram_nick)}</p>`:''}</div><span class="account-verified ${verified?'is-verified':'needs-verification'}">${verified?'✓ E-mail overený':'! E-mail neoverený'}</span></div>
      <form id="accountModalProfileForm" class="account-modal-main-card">
        ...
        <label class="account-inline-field account-inline-telegram"><span>${adminTelegramIcon()} Telegram</span><input id="accountModalTelegram" maxlength="33" value="${escapeHtml(a.telegram_nick||'')}" placeholder="@username"></label>
        <label class="account-inline-field"><span>Avatar</span><select id="accountModalAvatarVariant"><option value=""${!a.avatar_variant?' selected':''}>Predvolený</option><option value="m"${a.avatar_variant==='m'?' selected':''}>Muž</option><option value="w"${a.avatar_variant==='w'?' selected':''}>Žena</option></select></label>
      ...
      <div class="account-modal-security"><span>Účet a zabezpečenie</span><div><button class="btn btn-ghost" id="accountModalPassword" type="button">Obnoviť heslo</button><button class="btn btn-ghost account-signout-button" id="accountModalSignOut" type="button">Odhlásiť sa</button></div></div>
      <div class="account-modal-plans"><div class="account-modal-section-title"><small>BLINQ ČLENSTVO</small><h3>Vyber úroveň</h3><p>Názvy, popisy a odkazy plánov spravuješ cez membership JSON / Admin → Plány.</p></div>${plans}</div>`;
  }
  ...
  function renderAccountPage(){
    ...
    </section><section class="account-membership-page"><div class="account-membership-heading"><div><small>BLINQ ČLENSTVO</small><h2>Dostupné úrovne</h2></div><p>Vyber si rozsah prístupu. Ak je platobný odkaz nastavený, tlačidlo otvorí príslušnú ponuku.</p></div>${renderPlanCardsForAccount()}</section>`;
  }
