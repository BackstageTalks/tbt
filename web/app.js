(() => {
  ...
  function renderInsightBell(){
    ...
    const eligible=notificationAudienceConfig().enabled&&['rookie','pro','elite','legend','goat','admin'].includes(accountPlan());
  ...
  function openMatchPopout(){
    ...
    w.document.open();w.document.write(`<!doctype html><html lang="${escapeHtml(locale)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><base href="${escapeHtml(base)}"><title>BlinQ · Detail zápasu</title><link rel="stylesheet" href="/blinq.css?v=7010"><link rel="stylesheet" href="/redesign-680.css?v=7010"><link rel="stylesheet" href="/polish-681.css?v=7010"><link rel="stylesheet" href="/polish-683.css?v=7010"><link rel="stylesheet" href="/polish-684.css?v=7010"><link rel="stylesheet" href="/polish-685.css?v=7010"></head><body id="blinqPremium" class="blinq-detail-popout"><main class="match-popout-shell">${source.innerHTML}</main><script>document.addEventListener('click',function(e){var b=e.target.closest('[data-match-tab]');if(!b)return;var id=b.getAttribute('data-match-tab');document.querySelectorAll('[data-match-tab]').forEach(function(x){x.classList.toggle('active',x===b)});document.querySelectorAll('[data-match-panel]').forEach(function(p){var on=p.getAttribute('data-match-panel')===id;p.hidden=!on;p.classList.toggle('active',on)});});document.querySelectorAll('[data-match-popout]').forEach(function(x){x.remove()});<\/script></body></html>`);w.document.close();w.focus();
  }
  ...
  function renderAdminInsights(){
    ...
    const allowed=new Set(notificationAudienceConfig().editable_levels||['rookie','pro','elite','legend','goat']);
  ...
  function renderAdminRoute(){
    ...
    const info={accounts:['Účty','Správa levelu, platnosti a používateľského prístupu.'],banners:['Bannery','Jeden hero banner, mobilný podklad, pozadie a editovateľný HTML text.'],insights:['Správy & LIVE','Premium Info pre ľubovoľný level · automatický Comeback LIVE pre ELITE+.']}[state.adminTab];
    let contextual='';
  ...
  function wireAdmin(){
    ...
    host.onclick=async event=>{
      ...
      if(action==='insight-audience-elite'||action==='insight-audience-all'){const form=$('adminInsightForm');if(!form)return;const wanted=action==='insight-audience-all'?new Set(['rookie','pro','elite','legend','goat']):new Set(['elite','legend','goat']);form.querySelectorAll('input[name="insight_level"]').forEach(node=>{node.checked=wanted.has(node.value);});return;}
