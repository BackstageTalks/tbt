(() => {
  ...
  function isLiveInsight(item){return ['alert','live_watch'].includes(String(item?.type||'').toLowerCase());}
  function renderInsightBell(){
    ...
    if(shortcut){shortcut.hidden=!liveEligible;shortcut.classList.toggle('is-live-enabled',liveEligible);shortcut.classList.toggle('is-live-watching',watching&&!confirmed);shortcut.classList.toggle('is-live-confirmed',confirmed);shortcut.setAttribute('aria-expanded',state.insightDrawerOpen&&state.insightChannel==='live'?'true':'false');}
    ...
    if(shortcutLabel)shortcutLabel.textContent=confirmed?'CONFIRMED':watching?'WATCH':'RADAR';
  ...
  function renderInsightDrawer(){
    ...
    const channelRows=state.insights.filter(item=>channel==='live'?isLiveInsight(item):!isLiveInsight(item));
    ...
    if(title)title.textContent=channel==='live'?'Comeback LIVE':'Premium Info';
  ...
  function setupEvents(){
    ...
    if($('insightBell'))$('insightBell').onclick=()=>toggleInsightChannel('info');
    if($('insightShortcut'))$('insightShortcut').onclick=()=>toggleInsightChannel('live');
