(() => {
  ...
  function renderDailyHub(){
    ...
    const expand=$('dailyHubExpand'); if(expand){ const hasMore=allCount>preview; expand.hidden=!hasMore; if(tab==='board')expand.hidden=true; expand.title=state.dailyHubExpanded?lcopy('Show less','Zobraziť menej','Zobrazit méně'):lcopy('Show full offer','Zobraziť celú ponuku','Zobrazit celou nabídku'); expand.innerHTML=state.dailyHubExpanded?'<svg viewBox="0 0 20 20" aria-hidden="true"><path d="M8 8H4V4M12 8h4V4M8 12H4v4M12 12h4v4"></path></svg>':'<svg viewBox="0 0 20 20" aria-hidden="true"><path d="M7 4H4v3M13 4h3v3M7 16H4v-3M13 16h3v-3"></path></svg>'; expand.dataset.expanded=state.dailyHubExpanded?'1':'0'; expand.dataset.locked=ent.see_all===true?'0':'1'; expand.setAttribute('aria-expanded',state.dailyHubExpanded?'true':'false'); }
  }
  ...
  function renderResultsFilters(){
    ...
    const customLabel=lcopy('Custom range','Vlastné obdobie','Vlastní období');
    return `<div class="results-filter-bar results-filter-bar-v683">
      ...
      <label class="results-filter-field"><span>${escapeHtml(publicText('Period'))}</span><span class="select-shell"><select id="resultsWindow">${[['all',publicText('All time')],['1',publicText('24 hours')],['7',publicText('7 days')],['30',publicText('30 days')],['90',publicText('90 days')],['custom',customLabel]].map(([v,l])=>option(v,l,filters.window||'all')).join('')}</select><i aria-hidden="true"></i></span></label>
      <label class="results-filter-field results-date-field"><span>${escapeHtml(lcopy('From','Od','Od'))}</span><span class="date-shell"><input id="resultsDateFrom" type="date" value="${escapeHtml(filters.dateFrom||'')}" aria-label="${escapeHtml(lcopy('Date from','Dátum od','Datum od'))}"><i aria-hidden="true"></i></span></label>
      <label class="results-filter-field results-date-field"><span>${escapeHtml(lcopy('To','Do','Do'))}</span><span class="date-shell"><input id="resultsDateTo" type="date" value="${escapeHtml(filters.dateTo||'')}" aria-label="${escapeHtml(lcopy('Date to','Dátum do','Datum do'))}"><i aria-hidden="true"></i></span></label>
      <button id="resultsDateClear" class="results-date-clear" type="button" ${filters.dateFrom||filters.dateTo?'':'hidden'} aria-label="${escapeHtml(lcopy('Clear custom range','Vymazať vlastné obdobie','Vymazat vlastní období'))}" title="${escapeHtml(lcopy('Clear dates','Vymazať dátumy','Vymazat data'))}"><svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path d="M5 5l10 10M15 5 5 15"></path></svg></button>
    </div>`;
  }
  ...
  function wireResultsFilters(){
    [['resultsCategory','category'],['resultsTour','tour'],['resultsSurface','surface']].forEach(([id,key])=>{const el=$(id);if(el)el.onchange=()=>{state.resultsFilters[key]=el.value;renderRoute('results');};});
    const period=$('resultsWindow');if(period)period.onchange=()=>{state.resultsFilters.window=period.value||'all';if(state.resultsFilters.window!=='custom'){state.resultsFilters.dateFrom='';state.resultsFilters.dateTo='';}renderRoute('results');};
    const from=$('resultsDateFrom'),to=$('resultsDateTo');
    if(from)from.onchange=()=>{state.resultsFilters.dateFrom=from.value||'';if(state.resultsFilters.dateTo&&state.resultsFilters.dateFrom>state.resultsFilters.dateTo)state.resultsFilters.dateTo=state.resultsFilters.dateFrom;state.resultsFilters.window='custom';renderRoute('results');};
    if(to)to.onchange=()=>{state.resultsFilters.dateTo=to.value||'';if(state.resultsFilters.dateFrom&&state.resultsFilters.dateTo<state.resultsFilters.dateFrom)state.resultsFilters.dateFrom=state.resultsFilters.dateTo;state.resultsFilters.window='custom';renderRoute('results');};
    const clear=$('resultsDateClear');if(clear)clear.onclick=()=>{state.resultsFilters.dateFrom='';state.resultsFilters.dateTo='';state.resultsFilters.window='all';renderRoute('results');};
  }
  ...
  function renderAdminBanners(){
    ...
    return `<section class="admin-ux-section admin-banner-manager-v684"><div class="admin-ux-heading"><div><small>SPRÁVA OBSAHU</small><h2>Mapa bannerov</h2><p>Klikni priamo na pozíciu v náhľade hlavnej stránky. Vpravo upravíš obsah, podklad, odkaz a viditeľnosť bez zásahu do rozloženia.</p></div></div><div class="admin-banner-map-controls"><label>Horné CTA <select data-admin-header-count>${countOptions(headerCount,3)}</select></label><label>Hero slidy <select data-admin-hero-count>${countOptions(heroCfg.enabled===false?0:heroCount,5)}</select></label><label>Rotácia <span><input type="number" min="3" max="300" step="1" data-admin-hero-seconds value="${heroSeconds}"> s</span></label><label class="check"><input type="checkbox" data-admin-hero-rotate ${heroCfg.auto_rotate!==false?'checked':''}> Automaticky</label><label class="check"><input type="checkbox" data-admin-hero-dots ${heroCfg.show_dots!==false?'checked':''}> Bodky</label></div><div class="admin-banner-map-workspace">${renderAdminBannerMap()}${renderBannerEditor(selected)}</div></section>`;
  }
  ...
  function setupEvents(){
    ...
    if($('insightDrawerList'))$('insightDrawerList').onclick=event=>{
      ...
      const matchButton=event.target.closest('[data-insight-match]');if(matchButton){const found=findRowByEventId(matchButton.dataset.insightMatch);if(found){setInsightDrawer(false);setRoute('predictions');selectMatchInRail(found.row,found.tab);openMatch(normalize(found.row),found.tab,found.row);}else showStatus(lcopy('This match is not on the current board.','Tento zápas už nie je v aktuálnej ponuke.','Tento zápas už není v aktuální nabídce.'));}
    };
