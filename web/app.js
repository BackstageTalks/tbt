(() => {
  ...
  const PUBLIC_TRANSLATIONS = {
    sk: {
      ...
      'Dashboard':'Prehľad','Prime Predictions':'Short Odds','TOP Predictions':'TOP','Value Predictions':'Value','Doubles':'Štvorhra','Ace Predictions':'Esá','S/G Predictions':'Sety / hry','Results':'Výsledky','BTTS Bonus':'BTTS Bonus',
      ...
      'Prime Prediction':'Prime predikcia','Value Prediction':'Value predikcia','Ace / DF Prediction':'Predikcia es / dvojchýb','Set / Game Prediction':'Predikcia setu / hry','TOP Prediction':'TOP predikcia','Sets Projection':'Projekcia setov','Games Projection':'Projekcia hier','Ace / DF Projection':'Predikcia es / dvojchýb','Projection':'Predikcia','Projection only · no odds':'Iba projekcia · bez kurzu','Baseline':'Základ','Gap':'Rozdiel','Score':'Skóre','Opponent proj.':'Projekcia súpera','Data':'Dáta','games':'hier','proj.':'proj.','MODEL':'MODEL','VERY HIGH':'VEĽMI VYSOKÁ','HIGH':'VYSOKÁ','MEDIUM':'STREDNÁ','LOW':'NÍZKA','PROJECTION':'PREDIKCIA',
    ...
    cz: {
      'Skip to content':'Přeskočit na obsah','Dashboard':'Přehled','Prime Predictions':'Short Odds','TOP Predictions':'TOP','Value Predictions':'Value','Doubles':'Čtyřhra','Ace Predictions':'Esa','S/G Predictions':'Sety / hry','Results':'Výsledky','Plans':'Plány','Level':'Úroveň','Remaining':'Zbývá','Account':'Účet','Account & membership':'Účet a členství','Profile, access and plans':'Profil, přístup a plány','Sign out':'Odhlásit se','End this session':'Ukončit tuto relaci',
  ...
  const dashboardSectionFallback={
    prime:{label:'Prime Predictions',panel_id:'predictionsPanel',sidebar_element:'SIDEBAR_PRIME',sidebar_enabled:true,dashboard_enabled:true,dashboard_order:1,preview_limit:5},
  ...
  function renderVipRail(){
    ...
    const benefits=[1,2,3,4].map((n,i)=>{
      ...
      const iconText=c[`benefit_${n}_icon`]||defaults[i];
      const iconUrl=safeUiAsset(c[`benefit_${n}_icon_url`]||'')||safeExternalUrl(c[`benefit_${n}_icon_url`]||'');
      const icon=iconUrl?`<img src="${escapeHtml(iconUrl)}" alt="" loading="lazy">`:`<span aria-hidden="true">${escapeHtml(iconText)}</span>`;
  ...
  async function handleAuthSubmit(event){
    ...
    if(state.authMode==='signup'){const nick=$('authName').value.trim();if(!/^@?[A-Za-z0-9_]{5,32}$/.test(nick)){message.textContent='Telegram nickname musí mať 5–32 znakov: písmená, čísla alebo _.';$('authName').focus();return;}}
  ...
  function renderSidebarMatchDetail(row=state.railMatch){
    ...
    const overview=`<div class="rail-detail-panel" data-rail-panel="overview" ${active==='overview'?'':'hidden'}><div class="rail-player-pair">${insightPlayerCard(row,1)}${insightPlayerCard(row,2)}</div>${renderMotivationPanel(row)}</div>`;
  ...
  function renderInsightDrawer(){
    ...
    const rows=state.insights.filter(item=>filter==='unread'?!item.read:filter==='pinned'?Boolean(item.pinned):true);
  ...
  function dailyHubEntitlement(tab){
    const key=tab==='calendar'?'daily':tab==='daily'?'top_daily':tab==='top'?'prime':tab==='games'?'sg':tab==='board'?'board':tab;
  ...
  function dailyHubRows(tab){
    ...
    if(tab==='calendar'){
      const now=Date.now();
      return (Array.isArray(state.feed?.upcoming)?state.feed.upcoming:[]).filter(row=>{
    ...
    if(tab==='board'){
      const rows=state.boardMode==='results'?state.feed?.board_results:state.feed?.board_upcoming;
      return (Array.isArray(rows)?rows:[]).filter(offerSurfaceEligible).sort((a,b)=>state.boardMode==='results'?new Date(b?.scheduled_at||0)-new Date(a?.scheduled_at||0):(marketProbability(b)||0)-(marketProbability(a)||0));
    }
    return [];
  }
  function dailyHubTabLabel(tab){
    return {daily:lcopy('TOP predictions','TOP predikcie','TOP predikce'),calendar:'Preview',prime:lcopy('Short Odds','Short Odds','Short Odds'),top:lcopy('Short Odds','Short Odds','Short Odds'),value:lcopy('Value','Value','Value'),ace:lcopy('Aces','Esá','Esa'),games:lcopy('Sets & Games','Sety a gamy','Sety a gamy'),doubles:lcopy('Doubles','Štvorhra','Čtyřhra'),board:'BlinQ Board'}[tab]||tab;
  }
  function dashboardFilteredRows(rows){
    const q=String(state.dashboardSearch||'').trim().toLocaleLowerCase();
    const tournament=String(state.dailyHubTournament||'').trim();
    return (Array.isArray(rows)?rows:[]).filter(row=>{
      if(tournament&&String(row?.tournament||row?.competition||'')!==tournament)return false;
      if(!q)return true;
      const p1=row?.player1||{},p2=row?.player2||{};
      const hay=[row?.tournament,row?.competition,row?.tournament_city,row?.tournament_country_code,row?.tour,row?.surface,p1?.name,p2?.name,p1?.country_code,p2?.country_code,row?.player1_name,row?.player2_name].filter(Boolean).join(' ').toLocaleLowerCase();
  ...
  function dailyHubColumns(tab){
    ...
    if(tab==='board'&&state.boardMode==='results')return ['DÁTUM','TURNAJ','ZÁPAS','MODEL VÝBER','KURZ','BLINQ %','VÝSLEDOK'];
  ...
  function dailyHubMatch(row){
    const p1={...(row?.player1||{}),name:row?.player1?.name||row?.player1_name||'—',country_code:row?.player1?.country_code||row?.player1_country_code||row?.p1_country_code||'',rank:row?.player1?.rank??row?.player1_rank,photo_url:row?.player1?.photo_url||row?.player1_photo_url||''};
    const p2={...(row?.player2||{}),name:row?.player2?.name||row?.player2_name||'—',country_code:row?.player2?.country_code||row?.player2_country_code||row?.p2_country_code||'',rank:row?.player2?.rank??row?.player2_rank,photo_url:row?.player2?.photo_url||row?.player2_photo_url||''};
    const tour=row?.tour||'';
  ...
  function dailyHubRow(row,tab,active=false){
    ...
    if(tab==='board'&&state.boardMode==='results'){const result=row?.result||{};const raw=String(result?.status||result?.outcome||'').toLowerCase();const isVoid=result?.void===true||['void','cancelled','canceled','postponed','walkover','retired','abandoned','no_action'].includes(raw);const outcome=isVoid?'<b class="board-result board-void">VOID</b>':result?.correct===true?'<b class="board-result board-win">WIN</b>':result?.correct===false?'<b class="board-result board-loss">LOSS</b>':'<b class="board-result">—</b>';return `<tr${rowClass} data-hub-event="${key}"><td>${escapeHtml(fmtDate(row?.scheduled_at||row?.date))}</td><td>${tournament}</td><td>${dailyHubMatch(row)}</td><td><strong>${escapeHtml(pick)}</strong></td><td>${Number.isFinite(odds)?odds.toFixed(2):'—'}</td><td><b class="hub-prob">${probability==null?'—':pct(probability)}</b></td><td>${outcome}</td></tr>`;}
  ...
  function dailyHubLockedRow(tab,index){
    ...
    return `<tr class="hub-row-locked" data-upgrade-plan="${escapeHtml(firstUnlockPlan(tab==='daily'?'top_daily':tab==='top'?'top_daily':tab==='games'?'sg':tab,index))}" data-upgrade-section="${escapeHtml(dailyHubTabLabel(tab))}"><td colspan="${colspan}"><span>🔒</span><b>Prémiová predikcia</b><small>Dostupnosť závisí od úrovne členstva.</small></td></tr>`;
  }
  function renderDailyHub(){
    ...
    const tabs=['daily','calendar','top','value','ace','games','doubles','board'].filter(tab=>{const ent=dailyHubEntitlement(tab);const tc=cfg.tabs?.[tab]||{};return tc.enabled!==false&&ent.enabled!==false;});
    ...
    const settledResults=(state.feed?.results||[]).filter(row=>typeof row?.result?.correct==='boolean').length;
    $('dailyHubTabs').innerHTML=tabs.map(tab=>{const rows=dailyHubRows(tab),ent=dailyHubEntitlement(tab);const total=tab==='board'?rows.length:Number(ent.total);const count=Number.isFinite(total)?total:rows.length;return `<button type="button" role="tab" aria-selected="${tab===state.dailyHubTab?'true':'false'}" class="daily-hub-tab${tab===state.dailyHubTab?' active':''}${tab==='board'?' daily-hub-board-tab':''}" data-daily-hub-tab="${tab}"><span>${escapeHtml(dailyHubTabLabel(tab))}</span><b>${count}</b></button>`;}).join('')+`<button type="button" class="daily-hub-tab daily-hub-results" data-route="results"><span>${escapeHtml(lcopy('Results','Výsledky','Výsledky'))}</span>${settledResults?`<b>${settledResults}</b>`:''}</button>`;
    const tab=state.dailyHubTab,sourceRows=dailyHubRows(tab),rows=dashboardFilteredRows(sourceRows),ent=dailyHubEntitlement(tab);const total=tab==='board'?rows.length:Number(ent.total);const filtering=Boolean(String(state.dashboardSearch||'').trim()||String(state.dailyHubTournament||'').trim());const allCount=filtering?rows.length:(Number.isFinite(total)?Math.max(total,rows.length):rows.length);const preview=Math.max(1,Number(cfg.preview_rows)||10),canExpand=ent.see_all===true&&allCount>preview&&tab!=='board';let limit=(state.dailyHubExpanded&&canExpand)?allCount:preview;if(tab==='board')limit=allCount;const shown=Math.min(allCount,limit);
    ...
    const first=rows[0]||sourceRows[0]; const raw=first?.scheduled_at||first?.date||first?.start_time||first?.start_at||''; const d=raw?new Date(raw):new Date(); const dateText=Number.isNaN(d.getTime())?lcopy('Today selection','Dnešný výber','Dnešní výběr'):new Intl.DateTimeFormat(locale==='en'?'en-GB':locale==='cz'?'cs-CZ':'sk-SK',{weekday:'long',day:'numeric',month:'long'}).format(d);
    const metaDate=$('dailyHubMetaDate'); if(metaDate)metaDate.textContent=dateText; const toolbarDate=$('dailyHubToolbarDate');if(toolbarDate)toolbarDate.textContent=dateText;
    const tournamentSelect=$('dailyHubTournamentFilter');if(tournamentSelect){
      const options=[...new Set(dailyHubRows(tab).map(row=>String(row?.tournament||row?.competition||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
      const current=String(state.dailyHubTournament||'');
      tournamentSelect.innerHTML=`<option value="">${escapeHtml(lcopy('All tournaments','Všetky turnaje','Všechny turnaje'))}</option>`+options.map(name=>`<option value="${escapeHtml(name)}"${name===current?' selected':''}>${escapeHtml(name)}</option>`).join('');
      if(current&&!options.includes(current)){state.dailyHubTournament='';tournamentSelect.value='';}
    }
    const boardMode=$('dailyHubBoardMode'),boardNotice=$('dailyHubBoardNotice'),boardNoticeCopy=$('dailyHubBoardNoticeCopy');
    if(boardMode){boardMode.querySelectorAll('[data-board-mode]').forEach(button=>button.classList.toggle('active',button.dataset.boardMode===state.boardMode));}
    if(boardNotice){boardNotice.hidden=tab!=='board';}
    if(boardNoticeCopy){boardNoticeCopy.textContent=lcopy('Model shortlist · not an official published prediction','Modelový výber · nie je oficiálna publikovaná predikcia','Modelový výběr · není oficiální publikovaná predikce');}
    const expand=$('dailyHubExpand'); if(expand){ const hasMore=allCount>preview; expand.hidden=!hasMore; if(tab==='board')expand.hidden=true; expand.title=state.dailyHubExpanded?lcopy('Show less','Zobraziť menej','Zobrazit méně'):lcopy('Show full offer','Zobraziť celú ponuku','Zobrazit celou nabídku'); expand.textContent=state.dailyHubExpanded?'↙':'↗'; expand.dataset.expanded=state.dailyHubExpanded?'1':'0'; expand.dataset.locked=ent.see_all===true?'0':'1'; expand.setAttribute('aria-expanded',state.dailyHubExpanded?'true':'false'); }
  }
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    let badge='',mainValue='—',confidenceClass='low',pickLabel=publicText(key==='prime'?'Prime Prediction':key==='value'?'Value Prediction':key==='ace'?'Ace / DF Prediction':key==='sg'?'Set / Game Prediction':'TOP Prediction'),note='',metrics=[];
    if(projectionOnly){
  ...
  function renderDots(pageCount){ const host=$('carouselDots'); if(!host)return; host.innerHTML=''; if(pageCount<=1)return; for(let i=0;i<pageCount;i++){const b=document.createElement('button');b.type='button';b.className=i===state.page?'active':'';b.setAttribute('aria-label',`Show Prime page ${i+1}`);b.onclick=()=>{state.page=i;renderPredictions()};host.appendChild(b)} }
  function renderPredictions(){
    ...
    if(!previewCount)grid.innerHTML='<div class="state-card">No Prime predictions available yet. This board updates automatically when new predictions qualify.</div>';
  ...
  function wireDailyHub(){
    ...
    hub.addEventListener('change',event=>{
      if(event.target?.id==='dailyHubTournamentFilter'){state.dailyHubTournament=event.target.value||'';state.dailyHubExpanded=false;renderDailyHub();}
    });
    hub.addEventListener('click',event=>{
      ...
      if(event.target.closest('#dailyHubExpand')){ const ent=dailyHubEntitlement(state.dailyHubTab); if(ent.see_all!==true){ if(state.dailyHubTab==='board')showUpgradePrompt('legend','BlinQ Board');else showUpgradePrompt('elite',lcopy('Full daily offer','Celá denná ponuka','Celá denní nabídka')); return; } state.dailyHubExpanded=!state.dailyHubExpanded; renderDailyHub(); return; }
  ...
  function localResultMetrics(rows,category){
    ...
    const odds=graded.map(p=>Number(p.odds)).filter(Number.isFinite);
  ...
  function renderResults(){
    ...
    if(!entries.length)return '<div class="state-card">Zatiaľ nie sú dostupné vyhodnotené publikované predikcie pre tento filter.</div>';
    const body=entries.slice(0,250).map(({row:r,publication})=>{const p1=r.player1||{},p2=r.player2||{};const outcome=publicationOutcome(publication);const pickId=publication?.selection_id;const pickName=publication?.selection||(pickId===p1.id?p1.name:pickId===p2.id?p2.name:'—');const probability=Number(publication?.model_probability);const odds=Number(publication?.odds),rawUnits=Number(publication?.result?.profit_units),units=outcome.kind==='void'?0:rawUnits;const tag=String(publication?.section||'');const tags=`<span class="result-tag ${escapeHtml(tag)}">${escapeHtml(resultCategoryLabel(tag||'all'))}</span>`;const resultHtml=outcome.kind==='win'?'<b class="correct">✓ VÝHRA</b>':outcome.kind==='loss'?'<b class="wrong">× PREHRA</b>':`<b class="void">○ VOID</b>${outcome.reason?`<small class="void-reason">${escapeHtml(outcome.reason)}</small>`:''}`;return `<tr><td>${escapeHtml(fmtDate(r.scheduled_at))}<small>${escapeHtml(fmtTime(r.scheduled_at))}</small></td><td>${tags}</td><td><strong>${escapeHtml([flagEmoji(p1.country_code||p1.country_code2||p1.country_code3),p1.name||'Player 1'].filter(Boolean).join(' '))}</strong><small>vs ${escapeHtml([flagEmoji(p2.country_code||p2.country_code2||p2.country_code3),p2.name||'Player 2'].filter(Boolean).join(' '))} · ${escapeHtml(r.tournament||'')}</small></td><td>${escapeHtml(pickName)}</td><td>${Number.isFinite(probability)?pct(probability):'—'}</td><td>${Number.isFinite(odds)?odds.toFixed(2):'—'}</td><td>${resultHtml}</td><td class="${outcome.kind==='void'?'void':Number.isFinite(units)&&units>=0?'correct':'wrong'}">${outcome.kind==='void'?'0.00u':Number.isFinite(units)?`${units>0?'+':''}${units.toFixed(2)}u`:'—'}</td></tr>`}).join('');
    return `<div class="admin-table-wrap results-table-wrap"><table class="admin-analytics-table results-table"><thead><tr><th>Dátum</th><th>Kategória</th><th>Zápas</th><th>Predikcia</th><th>BlinQ %</th><th>Kurz</th><th>Výsledok</th><th>Jednotky</th></tr></thead><tbody>${body}</tbody></table></div>${entries.length>250?`<div class="results-limit-note">Zobrazených posledných 250 z ${entries.length} vyhodnotených publikovaných predikcií.</div>`:''}`;
  }
  ...
  function renderAdminBanners(){
    ...
    const selected=elements()?.[state.selectedElement]&&['header_slot','hero_banner','large_banner'].includes(elements()[state.selectedElement].kind)?state.selectedElement:'HEADER_BANNER_1';if(selected!==state.selectedElement)state.selectedElement=selected;
    return `<section class="admin-ux-section"><div class="admin-ux-heading"><div><small>SPRÁVA OBSAHU</small><h2>Bannery a odkazy</h2><p>Na jednom mieste spravuješ horné CTA, hlavný banner, obsahové bannery aj spodný VIP banner. Každý slot podporuje vlastný podklad, text a CTA.</p></div></div><div class="admin-banner-workspace"><aside class="admin-banner-library"><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Horné CTA</strong><span>1–3 rovnaké bannery</span></div><select data-admin-header-count>${countOptions(headerCount,3)}</select></div>${[1,2,3].map(i=>bannerLibraryButton(`HEADER_BANNER_${i}`,i,'header')).join('')}</div><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Hlavný banner</strong><span>Rotujúce bannery na celú šírku</span></div><select data-admin-hero-count>${countOptions(heroCfg.enabled===false?0:heroCount,5)}</select></div><div class="admin-rotation-controls"><label>Každých <input type="number" min="3" max="300" step="1" data-admin-hero-seconds value="${heroSeconds}"> s</label><label><input type="checkbox" data-admin-hero-rotate ${heroCfg.auto_rotate!==false?'checked':''}> Automaticky</label><label><input type="checkbox" data-admin-hero-dots ${heroCfg.show_dots!==false?'checked':''}> Bodky</label></div>${[1,2,3,4,5].map(i=>bannerLibraryButton(`HERO_BANNER_${i}`,i,'hero')).join('')}</div><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Obsahové bannery</strong><span>Podklad + vlastný text v každom slote</span></div></div>${['CONTENT_TOP_1','CONTENT_TOP_2','CONTENT_TOP_3','CONTENT_TOP_4','CONTENT_MID_1','CONTENT_MID_2','CONTENT_MID_3','CONTENT_MID_4','CONTENT_BOTTOM_1','CONTENT_BOTTOM_2','CONTENT_BOTTOM_3','CONTENT_BOTTOM_4'].map((id,i)=>bannerLibraryButton(id,i+1,'content')).join('')}</div><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Členstvo / VIP</strong><span>Spodný banner · obrázok + 3 texty + CTA</span></div></div>${bannerLibraryButton('VIP_RAIL',1,'vip')}</div></aside>${renderBannerEditor(selected)}</div></section>`;
  }
  ...
  function renderAdminRoute(){
    ...
    else if(diag?.accounts_ready&&diag?.content_storage_ready)diagText=`Backend OK · ${diag.admin_storage||'storage'}`;
    else if(diag?.accounts_ready)diagText='Účty OK · úložisko obsahu chýba';
    else if(diag)diagText='Problém s backendom';
    let diagNote='';
    const problems=Array.isArray(diag?.problems)?diag.problems:[];
    if(diag&&!diag.accounts_ready){
      const detail=problems.includes('firebase_admin_credentials_missing')
        ?'Na Azure Function App doplň serverové Firebase Admin credentials (project, service account e-mail a private key). Web Firebase konfigurácia sama nestačí na čítanie zoznamu používateľov.'
        :problems.includes('firebase_admin_users_unavailable')
          ?'Firebase Admin credentials sú nájdené, ale server nevie načítať používateľov. Skontroluj service-account oprávnenia a Firebase project ID.'
          :'Správa účtov nie je pripravená. Klikni Diagnostika a skopíruj iba bezpečný diagnostický výstup.';
      diagNote=`<div class="admin-runtime-note is-error"><strong>Účty: backend vyžaduje zásah</strong><span>${escapeHtml(detail)}</span></div>`;
    }else if(diag&&!diag.content_storage_ready){
      const storage=diag.storage||{};
      const detail=storage.azure_configured
        ?'Účty fungujú cez Firebase. Pre globálne bannery, runtime konfiguráciu, interné poznámky a analytiku oprav Azure Table prístup alebo zapni Firestore.'
        :'Účty fungujú cez Firebase. Pre globálne bannery a runtime konfiguráciu zapni Firestore v rovnakom Firebase projekte alebo nastav admin storage connection string alebo AzureWebJobsStorage.';
  ...
  function renderAccountModal(){
    ...
    const expiry=status==='lifetime'?'Doživotne':a.expires_at?`${remainingLabel(a.expires_at)} zostáva`:(status==='active'?'Aktívny prístup':'Bez aktívneho prístupu');
    ...
    return `<div class="account-modal-head"><div><small>BLINQ ÚČET</small><h2 id="accountDialogTitle">Tvoj BlinQ účet</h2></div><span class="account-verified ${verified?'is-verified':'needs-verification'}">${verified?'✓ E-mail overený':'! E-mail neoverený'}</span></div>
      <form id="accountModalProfileForm" class="account-modal-main-card account-modal-main-card-v2">
        <div class="account-modal-access"><div class="account-modal-avatar" id="accountModalAvatar">${escapeHtml(accountAvatarFallback(a))}</div><div class="account-access-copy"><small>AKTUÁLNY PRÍSTUP</small><strong>${escapeHtml(planLabel)}</strong><span class="account-access-status"><i></i>${escapeHtml(expiry)}</span></div></div>
        <div class="account-profile-facts"><span><small>Registrovaný e-mail</small><strong>${escapeHtml(a.email||'—')}</strong></span><span><small>Telegram nick</small><strong>${escapeHtml(tg||'Nenastavený')}</strong></span><span><small>Úroveň</small><strong>${escapeHtml(planLabel)} <em>· ${escapeHtml(expiry)}</em></strong></span></div>
      ...
      <div class="account-modal-plans"><div class="account-modal-section-title"><small>BLINQ ČLENSTVO</small><span>Vyber úroveň podľa rozsahu dát a prístupu.</span></div>${plans}</div>`;
  }
  ...
  function renderRoute(route){
    ...
    if(route==='admin'){host.innerHTML=renderAdminRoute();wireAdmin();loadAdminDiagnostics();if(state.adminTab==='accounts')loadAdminUsers();if(state.adminTab==='analytics')loadBannerAnalytics();return;}
    if(route==='prime'){
      ...
      const desc=lcopy(`BlinQ publication day ${day} · adaptive confidence cascade 80% → 78% → 76% → 74% → 72% → 70% → 68% until at least ${Number(r.target_count||10)} remaining picks qualify · depth ${Math.round(Number(r.min_data_depth||.80)*100)}%+ · surface ${Number(r.min_surface_matches||5)}+/player · odds floor ${Number(r.min_odds||1.20).toFixed(2)}. Edge/EV are diagnostic only.`,`Publikačný deň BlinQ ${day} · adaptívny rebrík istoty 80% → 78% → 76% → 74% → 72% → 70% → 68%, kým sa nekvalifikuje aspoň ${Number(r.target_count||10)} zostávajúcich predikcií · hĺbka dát ${Math.round(Number(r.min_data_depth||.80)*100)}%+ · povrch ${Number(r.min_surface_matches||5)}+ zápasov/hráč · minimálny kurz ${Number(r.min_odds||1.20).toFixed(2)}. Edge/EV sú iba diagnostické.`,`Publikační den BlinQ ${day} · adaptivní žebřík jistoty 80% → 78% → 76% → 74% → 72% → 70% → 68%, dokud se nekvalifikuje alespoň ${Number(r.target_count||10)} zbývajících predikcí · hloubka dat ${Math.round(Number(r.min_data_depth||.80)*100)}%+ · povrch ${Number(r.min_surface_matches||5)}+ zápasů/hráč · minimální kurz ${Number(r.min_odds||1.20).toFixed(2)}. Edge/EV jsou pouze diagnostické.`);
  ...
  function setupEvents(){
    ...
    document.querySelectorAll('[data-open-insights]').forEach(node=>node.addEventListener('click',()=>setInsightDrawer(true)));
    ...
    if($('insightDrawerToolbar'))$('insightDrawerToolbar').onclick=async event=>{const filter=event.target.closest('[data-insight-filter]');if(filter){state.insightFilter=filter.dataset.insightFilter||'all';renderInsightDrawer();return;}if(event.target.closest('[data-insight-read-all]')){const unread=state.insights.filter(item=>!item.read);for(const item of unread){await markInsightRead(item.id);}renderInsightDrawer();}};
