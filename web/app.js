(() => {
  ...
  function tournamentFallbackBadge(row){
    ...
    const number=(raw.match(/(?:MASTERS\s*)?(1000|500|250|175|125|100|75|50|35|25|15)\b/)||[])[1]||'';
    let family=tour.includes('WTA')?'WTA':tour.includes('ATP')?'ATP':tour.includes('ITF')?'ITF':'';
    if(/CHALLENGER|\bCH\b/.test(raw)) family='CH';
    const itfCode=(raw.match(/\b([MW](?:15|25|35|50|75|100))\b/)||[])[1]||'';
    if(itfCode)return {top:'ITF',bottom:itfCode};
    if(family==='CH')return {top:'CH',bottom:number||'TOUR'};
    if(family==='ATP'&&/MASTERS/.test(raw)&&!number)return {top:'ATP',bottom:'M'};
    if(family==='ATP'||family==='WTA')return {top:family,bottom:number||'TOUR'};
    if(family==='ITF')return {top:'ITF',bottom:number||'TOUR'};
    return {top:'TENNIS',bottom:number||'TOUR'};
  }
  function tournamentFallbackHtml(row){
    const badge=tournamentFallbackBadge(row);
    return '<span class="hub-logo-code"><b>'+escapeHtml(badge.top)+'</b><small>'+escapeHtml(badge.bottom)+'</small></span>';
  }
  ...
  function adminSectionLabelSk(key, fallback){
    const labels={prime:'Prime tipy',top_daily:'Top tipy',ace:'Esá',value:'Value tipy',doubles:'Štvorhra',sg:'Sety / hry',results:'Výsledky',btts:'BTTS Bonus'};
  ...
  function renderAccountModal(){
    ...
    const plans=membershipHierarchy.filter(id=>state.ui?.plans?.[id]?.enabled!==false).map(id=>{const p=state.ui.plans[id]||{},url=safeExternalUrl(p.url||'');return `<article class="account-modal-plan${id===plan?' is-current':''}">${planAvatarHtml(id,p)}<div><small>${escapeHtml(id.toUpperCase())}</small><strong>${escapeHtml(p.label||id.toUpperCase())}</strong><span>${escapeHtml(p.description||p.note||'')}</span></div>${id===plan?'<b>AKTUÁLNY</b>':url?`<a href="${escapeHtml(url)}" target="_blank" rel="noopener">Vybrať →</a>`:`<em>Čoskoro</em>`}</article>`}).join('');
    return `<div class="account-modal-head"><div><small>BLINQ ÚČET</small><h2 id="accountDialogTitle">${escapeHtml(a.name||'BlinQ Member')}</h2><p>${escapeHtml(a.email||'—')}</p></div><span class="account-verified ${verified?'is-verified':'needs-verification'}">${verified?'✓ E-mail overený':'! E-mail neoverený'}</span></div><div class="account-modal-summary"><div class="account-modal-avatar" id="accountModalAvatar">${escapeHtml(accountAvatarFallback(a))}</div><div><small>AKTUÁLNY PRÍSTUP</small><strong>${escapeHtml(planLabel)}</strong><span>${escapeHtml(expiry)}</span></div><button class="btn btn-ghost" id="accountModalPassword" type="button">Obnoviť heslo</button><button class="btn btn-ghost" id="accountModalSignOut" type="button">Odhlásiť sa</button></div><details class="account-modal-profile"><summary>Profil a Telegram</summary><form id="accountModalProfileForm"><label>Telegram nick<input id="accountModalTelegram" maxlength="33" value="${escapeHtml(a.telegram_nick||'')}" placeholder="@username"></label><label>Avatar<select id="accountModalAvatarVariant"><option value=""${!a.avatar_variant?' selected':''}>Predvolený</option><option value="m"${a.avatar_variant==='m'?' selected':''}>Muž</option><option value="w"${a.avatar_variant==='w'?' selected':''}>Žena</option></select></label><button class="btn btn-primary" type="submit">Uložiť profil</button><p id="accountModalMessage" class="form-message"></p></form></details><div class="account-modal-plans"><div class="account-modal-section-title"><small>BLINQ ČLENSTVO</small><h3>Plány</h3></div>${plans}</div>`;
  }
