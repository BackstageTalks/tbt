(() => {
  ...
  function applyV6525NavigationAndOverviewDefaults(){
    if(!state.ui)return;
    const navOrder={SIDEBAR_PREDICTIONS:1,SIDEBAR_PRIME:2,SIDEBAR_TOP_DAILY:3,SIDEBAR_VALUE:4,SIDEBAR_ACE:5,SIDEBAR_DOUBLES:6,SIDEBAR_SG:7,SIDEBAR_RESULTS:8,SIDEBAR_OVERVIEW:9,SIDEBAR_BTTS:10};
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    if(projectionOnly){
      ...
      if(key==='sg'&&unit==='probability'){mainValue=`${pct(projection)}<small> proj.</small>`;metrics=[[lcopy('Baseline','Základ','Základ'),Number.isFinite(reference)?pct(reference):'—'],[lcopy('Gap','Rozdiel','Rozdíl'),Number.isFinite(projectionGap)?`${(projectionGap*100).toFixed(1)} pp`:'—'],[lcopy('Score','Skóre','Skóre'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];pickLabel=lcopy('Sets projection','Projekcia setov','Projekce setů');}
        ...
        const rawLine=row?.line??row?.market_line??row?.threshold??row?.over_under_line??row?.bookmaker_line??row?.total_line;
        const line=Number(rawLine);
        const hasLine=Number.isFinite(line);
        const side=hasLine?(projection>=line?'OVER':'UNDER'):'';
        mainValue=`${projection.toFixed(1)}<small>${hasLine?lcopy(' projection',' projekcia',' projekce'):' proj.'}</small>`;
        if(key==='ace'&&hasLine){
          const lineGap=projection-line;
          metrics=[[lcopy('Line','Hranica','Hranice'),line.toFixed(1)],[lcopy('vs line','Rozdiel','Rozdíl'),`${lineGap>=0?'+':''}${lineGap.toFixed(1)}`],[lcopy('Data','Dáta','Data'),Number.isFinite(Number(samples.player1))&&Number.isFinite(Number(samples.player2))?`${samples.player1}/${samples.player2}`:'—']];
          pickLabel=lcopy('Ace / DF O/U','Esá / dvojchyby O/U','Esa / dvojchyby O/U');
          if(side) pick=`${pick} · ${side} ${line.toFixed(1)}`;
        }else{
          metrics=[[lcopy('Opponent','Súper','Soupeř'),Number.isFinite(opponentProjection)?opponentProjection.toFixed(1):'—'],[lcopy('Gap','Rozdiel','Rozdíl'),Number.isFinite(projectionGap)?`${projectionGap>=0?'+':''}${projectionGap.toFixed(1)}`:'—'],[lcopy('Score','Skóre','Skóre'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];
          pickLabel=lcopy('Ace / DF projection','Projekcia es / dvojchýb','Projekce es / dvojchyb');
        }
      }
  ...
  function renderMarketSection(key,hostId,emptyText){
    ...
    host.innerHTML=visible.length?visible.map((row,localIndex)=>{const absoluteIndex=start+localIndex;const locked=ent.blur_remaining!==false&&absoluteIndex>=unlocked;return marketPreviewCard(row,key,absoluteIndex,locked,true)}).join(''):`<div class="state-card market-empty">${escapeHtml(emptyText)}</div>`;
