(() => {
  ...
  function renderDashboardKpis(){
    ...
    const accuracy=Number(bestModel?.accuracy??perf?.accuracy);
    const accuracySample=Number(bestModel?.n??perf?.n);
    ...
    const windowNote=Number.isFinite(bestDays)&&Number.isFinite(accuracySample)
      ?lcopy(`best of 3/7/10/14/30 days · ${bestDays}d · n=${Math.trunc(accuracySample)}`,`najlepšie z 3/7/10/14/30 dní · ${bestDays} dní · n=${Math.trunc(accuracySample)}`,`nejlepší z 3/7/10/14/30 dní · ${bestDays} dní · n=${Math.trunc(accuracySample)}`)
      :lcopy('3/7/10/14/30-day history','história 3/7/10/14/30 dní','historie 3/7/10/14/30 dní');
    const cards=[
      [icons.board,lcopy('TODAY PREDICTIONS','DNEŠNÉ PREDIKCIE','DNEŠNÍ PREDIKCE'),String(totalToday),lcopy('matches','zápasov','zápasů'),''],
      [icons.target,lcopy('MODEL SUCCESS','MODEL ÚSPEŠNOSŤ','ÚSPĚŠNOST MODELU'),Number.isFinite(accuracy)?pct(accuracy):'—',windowNote,''],
  ...
  function notificationAudienceConfig(){
    ...
    const infoDefault=Array.isArray(cfg.info_default_levels)?cfg.info_default_levels.filter(level=>membershipHierarchy.includes(String(level))):[...membershipHierarchy];
    return {enabled:cfg.enabled!==false,one_way:cfg.one_way!==false,live_min_level:liveMin,default_min_level:liveMin,default_levels:Array.isArray(cfg.default_levels)?cfg.default_levels:membershipLevelsFrom(liveMin),info_default_levels:infoDefault.length?infoDefault:[...membershipHierarchy],editable_levels:editable.length?editable:[...membershipHierarchy]};
  }
  ...
  function renderInsightBell(){
    ...
    if(shortcut){
      ...
      if(eligible&&!liveEligible){shortcut.dataset.upgradePlan=liveMin;shortcut.dataset.upgradeSection='Comeback LIVE';shortcut.setAttribute('aria-label',`Comeback LIVE · dostupné od ${liveAccessLabel}`);}
    ...
    if(shortcutLabel)shortcutLabel.textContent=liveEligible?(confirmed?'CONFIRMED':watching?'WATCH':'RADAR'):liveAccessLabel;
  ...
  function performanceCategoryForTab(tab){
    return ({daily:'top_daily',prime:'prime',value:'value',ace:'ace',double_faults:'double_faults',doubles:'doubles',games:'games',sets:'sets'})[String(tab||'')]||'';
  }
  function bestPerformanceForTab(tab){
    const category=performanceCategoryForTab(tab);if(!category)return null;
    const item=state.feed?.performance_window_summary?.categories?.[category];
    const days=Number(item?.best_days),rate=Number(item?.best_hit_rate),n=Number(item?.best_n);
    if(!Number.isFinite(days)||!Number.isFinite(rate)||!Number.isFinite(n)||n<=0)return null;
    return {days,rate,n,mode:String(item?.selection_mode||'')};
  }
  function dailyHubPerformanceText(tab,compact=false){
    const perf=bestPerformanceForTab(tab);if(!perf)return '';
    const rate=`${(perf.rate*100).toFixed(1)}%`;
    if(compact)return `${rate} · ${perf.days}d · n=${Math.trunc(perf.n)}`;
    return lcopy(
      `Best result: ${rate} · ${perf.days} days · n=${Math.trunc(perf.n)} · all periods remain available in Results`,
      `Najlepší výsledok: ${rate} · ${perf.days} dní · n=${Math.trunc(perf.n)} · všetky obdobia zostávajú vo Výsledkoch`,
      `Nejlepší výsledek: ${rate} · ${perf.days} dní · n=${Math.trunc(perf.n)} · všechna období zůstávají ve Výsledcích`
    );
  }
  ...
  function dailyHubColumnKeys(tab){
    ...
    if(tab==='ace'||tab==='double_faults'||tab==='games'||tab==='sets')return ['rank','time','tournament','match','pick','number','confidence'];
  ...
  function renderDailyHub(){
    ...
    $('dailyHubTabs').innerHTML=visibleTabs.map(tab=>{
      ...
      const performance=coming?'':dailyHubPerformanceText(tab,true);
      return `<button type="button" role="tab" aria-selected="${tab===state.dailyHubTab?'true':'false'}" class="daily-hub-tab${tab===state.dailyHubTab?' active':''}${coming?' is-coming':''}" data-daily-hub-tab="${tab}"><span class="daily-hub-tab-copy"><span>${escapeHtml(dailyHubTabLabel(tab))}</span>${performance?`<small>${escapeHtml(performance)}</small>`:''}</span>${count}</button>`;
    }).join('');
    ...
    const boardNotice=$('dailyHubBoardNotice'),boardNoticeCopy=$('dailyHubBoardNoticeCopy');
    const performanceText=dailyHubPerformanceText(tab,false);
    if(boardNotice){boardNotice.hidden=!performanceText;if(boardNoticeCopy)boardNoticeCopy.textContent=performanceText;}
  }
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    const rowMarket=String(row?.market||row?.market_key||'').toLowerCase();
    const performanceTab=key==='top_daily'?'daily':key==='ace'?(rowMarket==='double_faults'?'double_faults':'ace'):key==='sg'?(rowMarket==='games'?'games':'sets'):key;
    const performanceText=dailyHubPerformanceText(performanceTab,true);
    const performanceHtml=performanceText?`<div class="market-card-performance"><small>${escapeHtml(lcopy('Best 3/7/10/14/30d','Najlepšie z 3/7/10/14/30d','Nejlepší z 3/7/10/14/30d'))}</small><strong>${escapeHtml(performanceText)}</strong></div>`:'';
  ...
  function renderResultsFilters(){
    ...
    const periodOptions=hours?[[String(hours/24),hours===24?lcopy('Last 24 hours','Posledných 24 hodín','Posledních 24 hodin'):lcopy('Last 48 hours','Posledných 48 hodín','Posledních 48 hodin')]]:[['all',publicText('All time')],['1',publicText('24 hours')],['3',lcopy('3 days','3 dni','3 dny')],['7',publicText('7 days')],['10',lcopy('10 days','10 dní','10 dní')],['14',lcopy('14 days','14 dní','14 dní')],['30',publicText('30 days')],['90',publicText('90 days')],['custom',lcopy('Custom range','Vlastné obdobie','Vlastní období')]];
  ...
  function renderResults(){
    ...
    const head=projectionOnly?'<tr><th>Dátum</th><th>Typ</th><th>Turnaj</th><th>Zápas</th><th>Projekcia pre</th><th>Projekcia</th><th>Skutočne</th><th>Výsledok</th><th>DATA DEPTH</th></tr>':'<tr><th>Dátum</th><th>Kategória</th><th>Turnaj</th><th>Zápas</th><th>Predikcia / projekcia</th><th>BlinQ % / projekcia</th><th>Kurz / skutočne</th><th>Výsledok</th><th>Jednotky / DATA DEPTH</th></tr>';
  ...
  function syncAdminInsightAudienceForType(form,reset=false){
    if(!form)return;const live=String(form.querySelector('#adminInsightType')?.value||'vip')==='alert';const liveLevels=new Set(membershipLevelsFrom(notificationAudienceConfig().live_min_level));
    const nodes=[...form.querySelectorAll('input[name="insight_level"]')];
    nodes.forEach(node=>{node.disabled=live&&!liveLevels.has(node.value);if(node.disabled)node.checked=false;});
    ...
    const hint=form.querySelector('[data-insight-audience-hint]');if(hint){const label=String(upgradePlanLabel(notificationAudienceConfig().live_min_level)||notificationAudienceConfig().live_min_level).replace(/^BlinQ\s+/i,'').toUpperCase();hint.textContent=live?`LIVE rešpektuje globálne minimum ${label}. Pre konkrétnu správu môžeš publikum iba zúžiť.`:'INFO môže ísť všetkým alebo ľubovoľnej kombinácii levelov.';}
  }
  function renderAdminInsights(){
    ...
    const notificationCfg=notificationAudienceConfig(),liveMin=notificationCfg.live_min_level,liveLevels=membershipLevelsFrom(liveMin),liveLabel=String(upgradePlanLabel(liveMin)||liveMin).replace(/^BlinQ\s+/i,'').toUpperCase();
  ...
  function renderAdminLevels(){
    ...
    const inactivity=state.ui.account_inactivity=mergeConfig({enabled:true,inactive_days:90,warning_days:7,notify_admin:true,notify_user:false,auto_expire_rookie:false},state.ui.account_inactivity||{});
    const inactivityPanel=`<div class="admin-form-section admin-inactivity-policy"><div class="admin-form-section-title"><strong>Kontrola neaktivity účtov</strong><span>ROOKIE zostáva bez pevnej expirácie. Toto je samostatná voliteľná ochrana pre dlhodobo nepoužívané účty.</span></div><div class="admin-membership-switches"><label><input type="checkbox" data-admin-inactivity-field="enabled" ${inactivity.enabled!==false?'checked':''}><span><strong>Kontrolovať posledné prihlásenie</strong><small>Denný worker vyhodnotí Firebase last_sign_in_at.</small></span></label><label><input type="checkbox" data-admin-inactivity-field="notify_admin" ${inactivity.notify_admin!==false?'checked':''}><span><strong>Poslať admin súhrn e-mailom</strong><small>Vyžaduje SMTP nastavenie v Azure.</small></span></label><label><input type="checkbox" data-admin-inactivity-field="notify_user" ${inactivity.notify_user?'checked':''}><span><strong>Upozorniť používateľa e-mailom</strong><small>Pošle upozornenie pred hranicou neaktivity.</small></span></label></div><div class="admin-membership-fields"><label><span>Neaktívny po (dni)</span><input type="number" min="14" max="3650" data-admin-inactivity-field="inactive_days" value="${escapeHtml(String(inactivity.inactive_days||90))}"></label><label><span>Upozorniť vopred (dni)</span><input type="number" min="1" max="365" data-admin-inactivity-field="warning_days" value="${escapeHtml(String(inactivity.warning_days||7))}"></label><div class="span-2 admin-inactivity-danger"><span>Automatická expirácia FREE ROOKIE</span><label class="admin-inline-check"><input type="checkbox" data-admin-inactivity-field="auto_expire_rookie" ${inactivity.auto_expire_rookie?'checked':''}> Po prekročení limitu označiť dlhodobo neaktívny ROOKIE účet ako expirovaný</label><small>Predvolene vypnuté. Kým je vypnuté, FREE ROOKIE je bez časového obmedzenia a kontrola iba upozorňuje.</small></div></div><div class="admin-runtime-note"><strong>E-mail je pripravený, ale heslá sa neukladajú do webu</strong><span>SMTP údaje zostávajú iba v Azure Environment variables. Diagnostika ukáže, či je mailer a account worker nakonfigurovaný.</span></div></div>`;
  ...
  function wireAdmin(){
    ...
    host.onchange=event=>{
      ...
      if(t.dataset.adminInactivityField){const cfg=state.ui.account_inactivity=state.ui.account_inactivity||{enabled:true,inactive_days:90,warning_days:7,notify_admin:true,notify_user:false,auto_expire_rookie:false};const field=t.dataset.adminInactivityField;let value=t.type==='checkbox'?t.checked:Number(t.value);if(field==='inactive_days')value=Math.max(14,Math.min(3650,Number(value)||90));if(field==='warning_days')value=Math.max(1,Math.min(Math.max(1,Number(cfg.inactive_days||90)-1),Number(value)||7));cfg[field]=value;showStatus('Kontrola neaktivity · zmena je v koncepte. Klikni Publikovať.');return;}
  ...
  function renderRoute(route){
    ...
    else if(route==='results') body=renderResultsFilters()+resultsSummary()+`<div class="route-sub results-section-head results-section-clean">${renderResults()}</div>`;
