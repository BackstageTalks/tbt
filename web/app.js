(() => {
  ...
  const dashboardCardsPerPanel = () => 1; // v6.5.15: dashboard is a lightweight one-pick preview; See more opens 3–5 picks.
  ...
  const avatarAssetSrc = value => { const url=safeUiAsset(value); return url ? `${url}?v=v6515` : ''; };
    ...
    try {
      ...
      if(runtime?.configured && runtime?.config?.schema===2){
        ...
        if(String(runtime.config.ui_revision||'')!==String(state.uiSource.ui_revision||'')){
          ...
          ['CONTENT_TOP_1','CONTENT_TOP_2','CONTENT_TOP_3','CONTENT_TOP_4','HEADER_BANNER_1','HEADER_BANNER_2','HEADER_BANNER_3','HEADER_BANNER_4','HERO_BANNER_1','HERO_BANNER_2','HERO_BANNER_3','HERO_BANNER_4','HERO_BANNER_5'].forEach(id=>{
  ...
  function loadAdminDraft(){
    ...
    try{
      ...
      if(saved?.schema===2 && saved?.elements && saved?.plans){state.ui=saved;applyV6514AdminCleanup();}
    }catch{}
  }
  ...
  function marketingAvatarUrl(planId){
    ...
    // v6.5.15: membership artwork is image-only. Plan labels are rendered beside
  ...
  function renderAdminRoute(){
    const tabs=[['layout','Layout & access','Sections, order, top CTA banners and main rotating banner'],['plans','Plans','Membership catalogue and access terms'],['accounts','Accounts','Member access and roles']];
    if(!['layout','plans','accounts'].includes(state.adminTab))state.adminTab='layout';
    const panel=state.adminTab==='layout'?renderAdminLayout():state.adminTab==='plans'?renderAdminPlans():renderAdminAccounts();
