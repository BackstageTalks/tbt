(() => {
  ...
  function renderAdminLevels(){
    const plans=membershipHierarchy.map((id,index)=>{
      ...
      const duration=p.lifetime?'':(Number.isFinite(Number(p.duration_days))?String(Number(p.duration_days)):'');
      const external=safeExternalUrl(p.invite_only?(inviteUrl||mainUrl):mainUrl);
      return `<details class="admin-membership-card plan-${escapeHtml(id)}" data-admin-level-plan="${escapeHtml(id)}" ${id==='rookie'?'open':''}><summary><div>${planAvatarPairHtml(id,p)}<span><small>${escapeHtml(id.toUpperCase())}</small><strong>${escapeHtml(p.label||id.toUpperCase())}</strong><em>${p.enabled===false?'VYPNUTÝ':'AKTÍVNY'}</em></span></div><b>${external?'ODKAZ OK':'BEZ ODKAZU'}</b></summary><div class="admin-membership-card-body"><div class="admin-membership-switches"><label><input type="checkbox" data-admin-level-field="enabled" ${p.enabled!==false?'checked':''}><span><strong>Level aktívny</strong><small>Zobrazí sa v členských kartách a upgrade ponuke.</small></span></label><label><input type="checkbox" data-admin-level-field="invite_only" ${p.invite_only?'checked':''}><span><strong>Len na pozvánku</strong><small>Použije pozvánkový odkaz namiesto bežného CTA.</small></span></label><label class="is-static"><span class="admin-membership-static-mark">${id==='goat'?'∞':'30'}</span><span><strong>${id==='goat'?'Doživotný level':'Časovo obmedzený'}</strong><small>${id==='goat'?'GOAT zostáva bez expirácie.':'Platnosť nastavuješ počtom dní nižšie.'}</small></span></label></div><div class="admin-membership-fields"><label><span>Zobrazený názov</span><input data-admin-level-field="label" value="${escapeHtml(p.label||'')}"></label><label><span>Názov karty</span><input data-admin-level-field="card_title" value="${escapeHtml(p.card_title||'')}"></label><label class="span-2"><span>Krátky popis</span><input data-admin-level-field="short_description" value="${escapeHtml(p.short_description||p.description||'')}"></label><label class="span-2"><span>Detailný popis</span><textarea rows="3" data-admin-level-field="description">${escapeHtml(p.description||'')}</textarea></label><label><span>Text tlačidla</span><input data-admin-level-field="cta_label" value="${escapeHtml(p.cta_label||'')}"></label><label><span>Úroveň v poradí</span><input value="${escapeHtml(String(index+1))} · ${escapeHtml(id.toUpperCase())}" disabled title="Poradie levelov je pevné: ROOKIE → PRO → ELITE → LEGEND → GOAT"></label><label class="span-2"><span>Hlavný odkaz / platba</span><div class="admin-membership-link-row"><input data-admin-level-field="url" value="${escapeHtml(mainUrl)}" placeholder="https://..."><span>${safeExternalUrl(mainUrl)?`<a href="${escapeHtml(safeExternalUrl(mainUrl))}" target="_blank" rel="noopener">Otestovať ↗</a>`:'<em>Nie je nastavený</em>'}</span></div></label><label class="span-2"><span>Pozvánkový odkaz <small>(iba ak je „Len na pozvánku“)</small></span><div class="admin-membership-link-row"><input data-admin-level-field="invite_url" value="${escapeHtml(inviteUrl)}" placeholder="https://..."><span>${safeExternalUrl(inviteUrl)?`<a href="${escapeHtml(safeExternalUrl(inviteUrl))}" target="_blank" rel="noopener">Otestovať ↗</a>`:'<em>Voliteľné</em>'}</span></div></label><label><span>Predvolená platnosť (dni)</span><input type="number" min="1" max="3650" data-admin-level-field="duration_days" value="${escapeHtml(duration)}" ${p.lifetime?'disabled':''}></label><label><span>Interná poznámka</span><input data-admin-level-field="note" value="${escapeHtml(p.note||'')}"></label><label class="span-2"><span>Výhody na karte <small>(1 riadok = 1 bod)</small></span><textarea rows="5" data-admin-level-field="features">${escapeHtml(features)}</textarea></label></div><div class="admin-membership-note"><strong>Publikovanie</strong><span>Zmeny sú súčasťou UI konfigurácie. Klikni hore na <b>Publikovať</b>; JSON súbory už nemusíš ručne meniť.</span></div></div></details>`;
    }).join('');
    return `<section class="admin-ux-section admin-membership-manager"><div class="admin-ux-heading"><div><small>ČLENSTVÁ</small><h2>Levely a upgrade karty</h2><p>Spravuj názvy, popisy, odkazy, dostupnosť a predvolenú platnosť bez ručnej úpravy JSON.</p></div></div><div class="admin-runtime-note"><strong>Prístupové pravidlá zostávajú oddelené</strong><span>Táto sekcia upravuje samotné levely a ich marketingové karty. Čo ktorý level vidí nastavuješ v <b>Zobrazenie</b>; LIVE minimum v <b>Info & LIVE</b>.</span></div><div class="admin-membership-list">${plans}</div></section>`;
  }
