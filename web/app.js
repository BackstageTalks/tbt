(() => {
  ...
  function dailyHubTabLabel(tab){
    return {daily:lcopy('TOP predictions','TOP predikcie','TOP predikce'),prime:lcopy('Short Odds','Short Odds','Short Odds'),top:lcopy('Short Odds','Short Odds','Short Odds'),value:lcopy('Value','Value','Value'),ace:lcopy('Aces','Esá','Esa'),games:lcopy('Sets & Games','Sety a gamy','Sety a gamy'),doubles:lcopy('Doubles','Štvorhra','Čtyřhra'),board:'BlinQ Board'}[tab]||tab;
  }
  function dashboardFilteredRows(rows){
    ...
    const from=String(state.dailyHubDateFrom||'').trim();
    const to=String(state.dailyHubDateTo||'').trim();
    const dayStart=value=>{if(!value)return null;const d=new Date(`${value}T00:00:00`);return Number.isNaN(d.getTime())?null:d.getTime();};
    const dayEnd=value=>{if(!value)return null;const d=new Date(`${value}T23:59:59.999`);return Number.isNaN(d.getTime())?null:d.getTime();};
    const fromTs=dayStart(from),toTs=dayEnd(to);
    return (Array.isArray(rows)?rows:[]).filter(row=>{
      if(tournament&&String(row?.tournament||row?.competition||'')!==tournament)return false;
      const raw=row?.scheduled_at||row?.date||row?.start_time||row?.start_at||'';
  ...
  function renderDailyHub(){
    ...
    const tabs=['daily','top','value','ace','games','doubles','board'].filter(tab=>{const ent=dailyHubEntitlement(tab);const tc=cfg.tabs?.[tab]||{};return tc.enabled!==false&&ent.enabled!==false;});
    ...
    const tab=state.dailyHubTab; host.dataset.tab=tab; host.dataset.boardMode=state.boardMode||'live'; const sourceRows=dailyHubRows(tab),rows=dashboardFilteredRows(sourceRows),ent=dailyHubEntitlement(tab);const total=tab==='board'?rows.length:Number(ent.total);const filtering=Boolean(String(state.dashboardSearch||'').trim()||String(state.dailyHubTournament||'').trim()||state.dailyHubDateFrom||state.dailyHubDateTo);const allCount=filtering?rows.length:(Number.isFinite(total)?Math.max(total,rows.length):rows.length);const preview=Math.max(1,Number(cfg.preview_rows)||10),canExpand=ent.see_all===true&&allCount>preview&&tab!=='board';let limit=(state.dailyHubExpanded&&canExpand)?allCount:preview;if(tab==='board')limit=allCount;const shown=Math.min(allCount,limit);
    ...
    const dateFrom=$('dailyHubDateFrom'),dateTo=$('dailyHubDateTo'),dateClear=$('dailyHubDateClear');
    if(dateFrom&&dateFrom.value!==String(state.dailyHubDateFrom||''))dateFrom.value=String(state.dailyHubDateFrom||'');
    if(dateTo&&dateTo.value!==String(state.dailyHubDateTo||''))dateTo.value=String(state.dailyHubDateTo||'');
    if(dateClear)dateClear.hidden=!(state.dailyHubDateFrom||state.dailyHubDateTo);
  ...
  function wireDailyHub(){
    ...
    hub.addEventListener('change',event=>{
      ...
      if(event.target?.id==='dailyHubDateFrom'){state.dailyHubDateFrom=event.target.value||'';if(state.dailyHubDateTo&&state.dailyHubDateFrom>state.dailyHubDateTo)state.dailyHubDateTo=state.dailyHubDateFrom;state.dailyHubExpanded=false;renderDailyHub();return;}
      if(event.target?.id==='dailyHubDateTo'){state.dailyHubDateTo=event.target.value||'';if(state.dailyHubDateFrom&&state.dailyHubDateTo<state.dailyHubDateFrom)state.dailyHubDateFrom=state.dailyHubDateTo;state.dailyHubExpanded=false;renderDailyHub();return;}
    });
    hub.addEventListener('click',event=>{
      ...
      if(event.target.closest('#dailyHubDateClear')){state.dailyHubDateFrom='';state.dailyHubDateTo='';state.dailyHubExpanded=false;renderDailyHub();return;}
  ...
  function renderAccountModal(){
    ...
    return `<div class="account-modal-head"><div><small>BLINQ ÚČET</small><h2 id="accountDialogTitle">Tvoj BlinQ účet</h2><p>Spravuj profil, prístup a členskú úroveň na jednom mieste.</p></div></div>
      <form id="accountModalProfileForm" class="account-modal-main-card account-modal-main-card-v3">
        <div class="account-modal-access"><div class="account-modal-avatar" id="accountModalAvatar">${escapeHtml(accountAvatarFallback(a))}</div><div class="account-access-copy"><small>AKTUÁLNY PRÍSTUP</small><strong>${escapeHtml(planLabel)}</strong><span class="account-access-status"><i></i>${escapeHtml(status==='lifetime'?'Aktívny · doživotne':expiry)}</span></div></div>
