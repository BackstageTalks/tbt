(() => {
  ...
  const PUBLIC_TRANSLATIONS = {
    sk: {
      ...
      'Dashboard':'Prehľad','Prime Picks':'Prime','Top Bets':'TOP','Value Picks':'Value','Doubles':'Štvorhra','Ace Picks':'Esá','S/G Picks':'Sety / hry','Results':'Výsledky','BTTS Bonus':'BTTS Bonus',
    ...
    try {
      ...
      if(runtime?.configured && runtime?.config?.schema===2){
        ...
        if(String(runtime.config.ui_revision||'')!==String(state.uiSource.ui_revision||'')){
          ...
          // 6.7.0 product decision: Prime Picks ship OFF. Apply this once when an
          ...
          if(['6.7.0','6.7.3','6.7.4'].includes(String(state.uiSource.ui_revision||''))){
  ...
  function contentEditor(item){
    ...
    if(!['header_slot','hero_banner','large_banner','sidebar_promo'].includes(item.kind)) return '<p class="admin-muted">Funkčný prvok dashboardu. Verejné umiestnenie a viditeľnosť tipov nastav v bloku zobrazenia dashboardu nižšie.</p>';
  ...
  function dashboardDisplayEditor(elementId){
    ...
    const rows=accessContexts.map(plan=>{const ent=cfg.plans?.[plan]||cfg.plans?.rookie||{},options=choices.map(v=>`<option value="${escapeHtml(v)}"${String(ent.visible_picks).toUpperCase()===String(v).toUpperCase()?' selected':''}>${escapeHtml(v)}</option>`).join(''),order=Number(ent.order||cfg.dashboard_order||99);return `<div class="admin-plan-rule" data-dashboard-plan-row="${escapeHtml(plan)}"><b>${escapeHtml(state.ui?.plans?.[plan]?.label||plan.toUpperCase())}</b><label>Viditeľné tipy<select data-dashboard-matrix-field="visible_picks">${options}</select></label><label>Poradie<input type="number" min="1" max="20" data-dashboard-matrix-field="order" value="${order}"></label><label class="check-field"><input type="checkbox" data-dashboard-matrix-field="blur_remaining" ${ent.blur_remaining!==false?'checked':''}> Rozmazať zvyšok</label><label class="check-field"><input type="checkbox" data-dashboard-matrix-field="see_all" ${ent.see_all?'checked':''}> Zobraziť viac</label></div>`;}).join('');
    return `<div class="admin-section dashboard-inspector-section" data-dashboard-section="${escapeHtml(key)}"><div class="admin-section-title"><strong>Pravidlá sekcie · ${escapeHtml(adminSectionLabelSk(key,cfg.label||key))}</strong><span>Viditeľnosť, počet tipov a poradie nastavíš samostatne pre každú úroveň členstva.</span></div><div class="admin-field-grid"><label class="check-field"><input type="checkbox" data-dashboard-field="sidebar_enabled" ${cfg.sidebar_enabled!==false?'checked':''}> Zobraziť v hornej navigácii</label><label class="check-field"><input type="checkbox" data-dashboard-field="dashboard_enabled" ${cfg.dashboard_enabled!==false?'checked':''}> Sekcia zapnutá</label><label>Limit náhľadu<select data-dashboard-field="preview_limit">${preview}</select></label></div><div class="admin-plan-rules">${rows}</div><small class="field-hint">Viditeľné tipy = 0 + Rozmazať zvyšok znamená, že sekcia ostane viditeľná, ale všetky tipy budú rozmazané. Poradie môže byť pre každú úroveň odlišné.</small></div>`;
  }
  ...
  function adminSectionRuleRow(key,plan,compact=false){
    ...
    return `<div class="admin-section-rule-row${compact?' compact':''}" data-dashboard-plan-row="${escapeHtml(plan)}"><b>${escapeHtml(state.ui?.plans?.[plan]?.label||plan.toUpperCase())}</b><label><span>Prístup na stránku</span><select data-dashboard-route-access>${adminSectionAccessOptions(routeAccess)}</select></label><label><span>Viditeľné tipy</span><select data-dashboard-matrix-field="visible_picks">${adminVisiblePickOptions(ent.visible_picks)}</select></label><label><span>Poradie</span><input type="number" min="1" max="20" data-dashboard-matrix-field="order" value="${order}"></label><label class="admin-toggle-line"><input type="checkbox" data-dashboard-matrix-field="blur_remaining" ${ent.blur_remaining!==false?'checked':''}><span>Rozmazať zvyšok</span></label><label class="admin-toggle-line"><input type="checkbox" data-dashboard-matrix-field="see_all" ${ent.see_all?'checked':''}><span>Zobraziť viac</span></label></div>`;
  }
  ...
  function renderSimpleBannerCard(id,index,kind='header'){
    ...
    return `<article class="admin-simple-banner-card" data-simple-banner="${escapeHtml(id)}">
      ...
      <div class="admin-simple-banner-fields">
        ...
        <label class="span-2">Podnadpis<input data-simple-banner-field="text" value="${escapeHtml(c.text||'')}" placeholder="News · Picks · Discussions"></label>
        <label>Text tlačidla<input data-simple-banner-field="button_text" value="${escapeHtml(c.button_text||'')}" placeholder="JOIN"></label>
  ...
  function bannerLibraryButton(id,index,kind){
    ...
    return `<button type="button" class="admin-banner-library-card${selected?' selected':''}" data-admin-element="${escapeHtml(id)}"><span class="admin-banner-index">${index}</span><span><small>${kind==='hero'?'MAIN SLIDE':kind==='content'?'CONTENT':'TOP CTA'}</small><strong>${escapeHtml(c.headline||item.label||id)}</strong><em>${active?'ACTIVE':'OFF'}${link?' · LINKED':''}</em></span><b>›</b></button>`;
  }
  ...
  function renderAdminBanners(){
    ...
    return `<section class="admin-ux-section"><div class="admin-ux-heading"><div><small>SPRÁVA OBSAHU</small><h2>Bannery a odkazy</h2><p>Na jednom mieste spravuješ tri horné CTA bannery a hlavný banner. Kampaňové kreatívy podporujú aj delené rozloženia pre väčšie reklamné zóny.</p></div></div><div class="admin-banner-workspace"><aside class="admin-banner-library"><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Horné CTA</strong><span>1–3 rovnaké bannery</span></div><select data-admin-header-count>${countOptions(headerCount,3)}</select></div>${[1,2,3].map(i=>bannerLibraryButton(`HEADER_BANNER_${i}`,i,'header')).join('')}</div><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Hlavný banner</strong><span>Rotujúce bannery na celú šírku</span></div><select data-admin-hero-count>${countOptions(heroCfg.enabled===false?0:heroCount,5)}</select></div><div class="admin-rotation-controls"><label>Každých <input type="number" min="3" max="300" step="1" data-admin-hero-seconds value="${heroSeconds}"> s</label><label><input type="checkbox" data-admin-hero-rotate ${heroCfg.auto_rotate!==false?'checked':''}> Automaticky</label><label><input type="checkbox" data-admin-hero-dots ${heroCfg.show_dots!==false?'checked':''}> Bodky</label></div>${[1,2,3,4,5].map(i=>bannerLibraryButton(`HERO_BANNER_${i}`,i,'hero')).join('')}</div><div class="admin-banner-group"><div class="admin-banner-group-head"><div><strong>Obsahové bannery</strong><span>Podklad + vlastný text v každom slote</span></div></div>${['CONTENT_TOP_1','CONTENT_TOP_2','CONTENT_TOP_3','CONTENT_TOP_4','CONTENT_MID_1','CONTENT_MID_2','CONTENT_MID_3','CONTENT_MID_4','CONTENT_BOTTOM_1','CONTENT_BOTTOM_2','CONTENT_BOTTOM_3','CONTENT_BOTTOM_4'].map((id,i)=>bannerLibraryButton(id,i+1,'content')).join('')}</div></aside>${renderBannerEditor(selected)}</div></section>`;
  }
  ...
  function renderAdminRoute(){
    const tabs=[['accounts','Účty','Levely · expirácie · Telegram'],['layout','Denná ponuka','Kategórie · predikcie · prístupy'],['banners','Bannery','Horné CTA · hlavný banner'],['plans','Plány','Levely · platobné odkazy'],['campaigns','Kampane','Reklamné kreatívy'],['analytics','Analytika','Zobrazenia · kliky · CTR'],['performance','Model','Kvalita modelu']];
    if(!['layout','banners','campaigns','plans','accounts','analytics','performance'].includes(state.adminTab))state.adminTab='accounts';
    ...
    const configActions=state.adminTab==='accounts'?`<div class="admin-account-direct-note"><span>●</span> Zmeny účtov sa aplikujú okamžite</div>`:`${state.adminTab==='layout'?'<button class="btn btn-ghost" type="button" data-admin-action="preview-demo">Demo náhľad</button>':''}<button class="btn btn-ghost" type="button" data-admin-action="save-draft">Uložiť koncept</button><button class="btn btn-primary" type="button" data-admin-action="publish-config">Publikovať zmeny</button><details><summary>Viac</summary><button type="button" data-admin-action="export">Exportovať JSON</button><button type="button" data-admin-action="reset">Zrušiť koncept</button></details>`;
    const diag=state.adminDiagnostics,diagText=state.adminDiagnosticsLoading?'Kontrolujem…':diag?.ok?`API OK · ${diag.admin_storage||'storage'}`:diag?`API problém · ${(diag.problems||[]).join(', ')||diag.error||'diagnostics'}`:'Diagnostika';
    return `<div class="admin-console admin-console-v6522"><header class="admin-control-header"><div><small>BLINQ CONTROL</small><h1>Admin centrum</h1><p>${state.adminTab==='accounts'?'Spravuj prístupy členov priamo.':'Jednoduchá správa živého webu. Zmeny zostanú lokálne, kým ich nepublikuješ.'}</p></div><div class="admin-global-actions"><button class="btn btn-ghost admin-diagnostics-button ${diag?.ok?'is-ok':diag?'is-error':''}" type="button" data-admin-action="diagnostics">${escapeHtml(diagText)}</button>${configActions}</div></header><nav class="admin-tabs admin-tabs-v6522">${tabs.map(([id,label,hint])=>`<button type="button" class="${state.adminTab===id?'active':''}" data-admin-tab="${id}"><strong>${label}</strong><small>${hint}</small></button>`).join('')}</nav><div class="admin-panel admin-panel-v6522">${panel}</div></div>`;
  }
  ...
  async function loadAdminUsers(force=false){
    ...
    try{
      ...
      while(page<=25){const data=await BlinqAuth.adminUsers(page,perPage);const batch=Array.isArray(data?.users)?data.users:[];all.push(...batch);if(batch.length<perPage)break;page+=1;}
    ...
    }catch(error){state.adminUsers=[];state.adminUsersError=error.status===503?'Admin API nie je dostupné. Klikni na Diagnostika; backend podporuje Azure Table aj Firebase Firestore fallback.':error.message;showStatus(state.adminUsersError);}
  ...
  function renderAccountModal(){
    ...
    const expiry=status==='lifetime'?'Doživotný prístup':a.expires_at?`${remainingLabel(a.expires_at)} zostáva`:(status==='active'?'Aktívne':'Bez aktívneho prístupu');
    const plans=membershipHierarchy.filter(id=>state.ui?.plans?.[id]?.enabled!==false).map(id=>{const p=state.ui.plans[id]||{},url=safeExternalUrl(p.url||'');return `<article class="account-modal-plan${id===plan?' is-current':''}">${planAvatarPairHtml(id,p)}<div><small>${escapeHtml(id.toUpperCase())}</small><strong>${escapeHtml(p.label||id.toUpperCase())}</strong><span>${escapeHtml(p.description||p.note||'')}</span></div>${id===plan?'<b>AKTUÁLNY</b>':url?`<a href="${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(p.cta_label||'Upgrade')} →</a>`:`<em>Čoskoro</em>`}</article>`}).join('');
    return `<div class="account-modal-head"><div><small>BLINQ ÚČET</small><h2 id="accountDialogTitle">${escapeHtml(a.name||'BlinQ Member')}</h2><p>${escapeHtml(a.email||'—')}</p></div><span class="account-verified ${verified?'is-verified':'needs-verification'}">${verified?'✓ E-mail overený':'! E-mail neoverený'}</span></div><div class="account-modal-summary"><div class="account-modal-avatar" id="accountModalAvatar">${escapeHtml(accountAvatarFallback(a))}</div><div><small>AKTUÁLNY PRÍSTUP</small><strong>${escapeHtml(planLabel)}</strong><span>${escapeHtml(expiry)}</span></div><button class="btn btn-primary" form="accountModalProfileForm" type="submit">Uložiť TG profil</button><button class="btn btn-ghost" id="accountModalPassword" type="button">Obnoviť heslo</button><button class="btn btn-ghost" id="accountModalSignOut" type="button">Odhlásiť sa</button></div><details class="account-modal-profile" open><summary>Telegram a avatar</summary><form id="accountModalProfileForm"><label>Telegram nick<input id="accountModalTelegram" maxlength="33" value="${escapeHtml(a.telegram_nick||'')}" placeholder="@username"></label><label>Avatar<select id="accountModalAvatarVariant"><option value=""${!a.avatar_variant?' selected':''}>Predvolený</option><option value="m"${a.avatar_variant==='m'?' selected':''}>Muž</option><option value="w"${a.avatar_variant==='w'?' selected':''}>Žena</option></select></label><p id="accountModalMessage" class="form-message"></p></form></details><div class="account-modal-plans"><div class="account-modal-section-title"><small>BLINQ ČLENSTVO</small><h3>Plány</h3></div>${plans}</div>`;
  }
  ...
  function renderAccountPage(){
    ...
    return `<section class="account-overview">
      <article class="account-profile-card">
        <div class="account-profile-head"><span class="avatar account-page-avatar" id="accountPageAvatar">${escapeHtml(accountAvatarFallback(a))}</span><div><span class="account-card-kicker">YOUR ACCOUNT</span><h2>${escapeHtml(a.name||'BlinQ Member')}</h2><p>${escapeHtml(a.email||'—')}</p></div><div class="account-profile-head-actions"><span class="account-verified ${verified?'is-verified':'needs-verification'}">${escapeHtml(publicText(verified?'✓ Email verified':'! Email not verified'))}</span><div class="account-head-buttons"><button class="btn btn-primary" form="accountProfileForm" type="submit">Uložiť TG profil</button><button class="btn btn-ghost" id="accountPasswordReset" type="button">Obnoviť heslo</button></div></div></div>
        <form id="accountProfileForm" class="account-profile-form"><label><span class="telegram-label"><span aria-hidden="true">✈</span> Telegram nick</span><input id="accountTelegramNick" maxlength="33" value="${escapeHtml(a.telegram_nick||'')}" placeholder="@username"></label><label>Avatar style<select id="accountAvatarVariant"><option value=""${!a.avatar_variant?' selected':''}>Default</option><option value="m"${a.avatar_variant==='m'?' selected':''}>Male</option><option value="w"${a.avatar_variant==='w'?' selected':''}>Female</option></select></label><p class="form-message" id="accountProfileMessage"></p></form>
      </article>
