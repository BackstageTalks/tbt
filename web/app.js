(() => {
  ...
  function dailyHubConfig(){
    return state.ui?.dashboard?.daily_hub||{enabled:true,default_tab:'daily',preview_rows:5,expand_rows:10,tabs:{}};
  }
  ...
  function dailyHubMatch(row){
    const p1=row?.player1?.name||row?.player1_name||'—',p2=row?.player2?.name||row?.player2_name||'—';
    return `<span class="hub-match"><b>${escapeHtml(p1)}</b><i>vs</i><b>${escapeHtml(p2)}</b></span>`;
  }
  function dailyHubRow(row,tab){
    const probability=marketProbability(row),odds=Number(row?.odds??row?.betting?.odds),pick=row?.pick||row?.selection||row?.prediction||'—';
    const time=fmtTime(row?.scheduled_at||row?.date),tournament=row?.tournament||row?.competition||'—';
    const data=dataDepthMetric(row);
    if(tab==='value'){const ev=Number(row?.expected_value??row?.betting?.expected_value);return `<tr data-hub-event="${escapeHtml(row?.event_id||row?.id||'')}"><td>${escapeHtml(time)}</td><td>${escapeHtml(tournament)}</td><td>${dailyHubMatch(row)}</td><td><strong>${escapeHtml(pick)}</strong></td><td>${Number.isFinite(odds)?odds.toFixed(2):'—'}</td><td><b class="hub-prob">${probability==null?'—':pct(probability)}</b></td><td>${escapeHtml(closeMarketLabel(row))}</td><td class="metric-positive">${Number.isFinite(ev)?`${ev>0?'+':''}${(ev*(Math.abs(ev)<=1?100:1)).toFixed(1)}%`:'—'}</td><td><button class="hub-detail" type="button" data-hub-detail>Detail →</button></td></tr>`;}
    if(tab==='ace'||tab==='games'){const projection=Number(row?.projection),line=apiMarketLine(row),side=tab==='ace'?aceLineSide(row):String(row?.side||row?.prediction_side||'');return `<tr data-hub-event="${escapeHtml(row?.event_id||row?.id||'')}"><td>${escapeHtml(time)}</td><td>${escapeHtml(tournament)}</td><td>${dailyHubMatch(row)}</td><td><strong>${escapeHtml(pick)}</strong></td><td>${Number.isFinite(line)?`${escapeHtml(side)} ${line.toFixed(1)}`.trim():'—'}</td><td><b class="hub-prob">${Number.isFinite(projection)?(tab==='games'?projection.toFixed(1):projection.toFixed(1)):'—'}</b></td><td>${escapeHtml(data)}</td><td><button class="hub-detail" type="button" data-hub-detail>Detail →</button></td></tr>`;}
    return `<tr data-hub-event="${escapeHtml(row?.event_id||row?.id||'')}"><td>${escapeHtml(time)}</td><td>${escapeHtml(tournament)}</td><td>${dailyHubMatch(row)}</td><td><strong>${escapeHtml(pick)}</strong></td><td>${Number.isFinite(odds)?odds.toFixed(2):'—'}</td><td><b class="hub-prob">${probability==null?'—':pct(probability)}</b></td><td>${escapeHtml(data)}</td><td><button class="hub-detail" type="button" data-hub-detail>Detail →</button></td></tr>`;
  }
  ...
  function renderDailyHub(){
    const host=$('dailyHub');if(!host)return;wireDailyHub();const cfg=dailyHubConfig();host.hidden=cfg.enabled===false; if(host.hidden)return;
    ...
    const tab=state.dailyHubTab,rows=dailyHubRows(tab),ent=dailyHubEntitlement(tab);const total=Number(ent.total);const allCount=Number.isFinite(total)?Math.max(total,rows.length):rows.length;const preview=Math.max(1,Number(cfg.preview_rows)||5),expanded=Math.max(preview,Number(cfg.expand_rows)||10),limit=state.dailyHubExpanded?expanded:preview;const shown=Math.min(allCount,limit);
    $('dailyHubHead').innerHTML=`<tr>${dailyHubColumns(tab).map(c=>`<th>${escapeHtml(c)}</th>`).join('')}</tr>`;
    const out=[];for(let i=0;i<shown;i++){if(i<rows.length)out.push(dailyHubRow(rows[i],tab));else if(ent.blur_remaining!==false)out.push(dailyHubLockedRow(tab,i));}
    $('dailyHubBody').innerHTML=out.join('');$('dailyHubEmpty').hidden=Boolean(out.length);
    const expand=$('dailyHubExpand');if(expand){expand.hidden=allCount<=preview;expand.textContent=state.dailyHubExpanded?lcopy('Show less','Zobraziť menej','Zobrazit méně'):`${lcopy('Show all','Zobraziť všetky','Zobrazit všechny')} (${allCount})`;expand.dataset.expanded=state.dailyHubExpanded?'1':'0';}
  }
  ...
  function openMatch(m){ const signalRows=m.signals.length?m.signals.map(s=>{const meta=signalMeta(s,m);const favoursId=String(s?.player_id??s?.favours_player_id??'');const favours=favoursId===String(m.p1Id)?m.p1:favoursId===String(m.p2Id)?m.p2:'—';return `<div class="dialog-signal"><span>${escapeHtml(meta.label)}</span><strong>${escapeHtml(favours)}</strong><small>${meta.favours?'supports pick':'counter-signal'}</small></div>`}).join(''):'<p class="signal-empty">No secondary signals are available.</p>'; const betting=Number.isFinite(m.odds)&&m.odds>1?`<div class="dialog-section dialog-market"><h3>${escapeHtml(lcopy('Pick context','Kontext tipu','Kontext tipu'))}</h3><div class="dialog-market-grid"><span>${escapeHtml(publicText('Odds'))} <strong>${m.odds.toFixed(2)}</strong></span><span>${escapeHtml(lcopy('Data','Dáta','Data'))} <strong>${escapeHtml(dataDepthMetric(m.raw||m))}</strong></span><span>${escapeHtml(publicText('Surface'))} <strong>${escapeHtml(surfaceSampleLabel(m.raw||m))}</strong></span><span>${escapeHtml(lcopy('Form','Forma','Forma'))} <strong>${escapeHtml(publicFormLabel(m.raw||m,m.pickId))}</strong></span></div></div>`:''; $('dialogContent').innerHTML=`<div class="dialog-eyebrow">${escapeHtml(m.tour)} · ${escapeHtml(m.tournament)}</div><h2>${escapeHtml(m.p1)} <span>vs</span> ${escapeHtml(m.p2)}</h2><div class="dialog-pick"><div><small>BlinQ Pick</small><strong>${escapeHtml(m.pick)}</strong></div><div class="dialog-prob">${pct(m.probability)} <span class="confidence ${m.confidence}">${m.confidence==='very-high'?'VERY HIGH':m.confidence.toUpperCase()}</span></div></div>${betting}<div class="dialog-section"><h3>Model signals</h3>${signalRows}</div><div class="dialog-meta"><span>${escapeHtml(String(m.surface).replaceAll('_',' '))}</span><span>${fmtDate(m.date)} · ${fmtTime(m.date)}</span><span>Model ${escapeHtml(m.model||'—')}</span></div>`; $('matchDialog').showModal(); }
  ...
  function wireDailyHub(){
    const hub=$('dailyHub');if(!hub||hub.dataset.wired==='1')return;hub.dataset.wired='1';
    hub.addEventListener('click',event=>{const tab=event.target.closest('[data-daily-hub-tab]');if(tab){state.dailyHubTab=tab.dataset.dailyHubTab;state.dailyHubExpanded=false;renderDailyHub();return;}if(event.target.closest('#dailyHubExpand')){state.dailyHubExpanded=!state.dailyHubExpanded;renderDailyHub();return;}const row=event.target.closest('tr[data-hub-event]');if(row&&event.target.closest('[data-hub-detail]')){const current=dailyHubRows(state.dailyHubTab).find(r=>String(r?.event_id||r?.id||'')===String(row.dataset.hubEvent||''));if(current){if(state.dailyHubTab==='daily'||state.dailyHubTab==='value')openMatch(normalize(current));else setRoute(state.dailyHubTab==='games'?'sg':state.dailyHubTab);}}});
  }
  ...
  function wireAdmin(){
    ...
    host.onchange=event=>{
      ...
      if(t.dataset.adminHubField){const tab=t.dataset.adminHubTab,hub=state.ui.dashboard.daily_hub=state.ui.dashboard.daily_hub||{enabled:true,default_tab:'daily',preview_rows:5,expand_rows:10,tabs:{}};hub.tabs=hub.tabs||{};const tc=hub.tabs[tab]=hub.tabs[tab]||{enabled:true,plans:{}};tc.plans=tc.plans||{};const rule=tc.plans[state.adminPlan]=tc.plans[state.adminPlan]||{visible_rows:0,blur_remaining:true,tab_enabled:true};let value=t.type==='checkbox'?t.checked:t.value;if(t.dataset.adminHubField==='visible_rows'&&String(value).toUpperCase()!=='ALL')value=Number(value);rule[t.dataset.adminHubField]=value;if(state.adminPlan==='rookie')tc.plans.trial=clone(rule);rerenderAdmin();return;}
