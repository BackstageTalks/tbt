(() => {
  ...
  function renderVipRail(){
    ...
    host.innerHTML=`${picture}<div class="vip-rail-content vip-links-only"><div class="vip-benefits">${benefits}</div><a class="vip-cta${clickable?'':' is-link-locked'}" href="${escapeHtml(finalHref)}" ${clickable&&route?`data-route="${escapeHtml(route)}"`:''} ${!clickable?`data-upgrade-plan="${escapeHtml(lockedPlan)}" data-upgrade-section="${escapeHtml(c.headline||'BlinQ VIP')}"`:''}>${!clickable?lockIcon()+escapeHtml(upgradePlanLabel(lockedPlan).replace(/^BlinQ\s+/i,'')):escapeHtml(c.button_text||'Zobraziť plány')+' →'}</a></div>`;
  }
  ...
  function renderAdminRoute(){
    ...
    }[state.adminTab]||['Admin centrum','Správa BlinQ'];
    const configTabs=['layout','banners','campaigns','plans'];
    let contextual='';
    ...
    else if(diag?.accounts_ready&&diag?.content_storage_ready)diagText='Systém OK';
    else if(diag?.accounts_ready)diagText='Systém · upozornenie';
    else if(diag)diagText='Systém · chyba';
    let diagNote='';
    const problems=Array.isArray(diag?.problems)?diag.problems:[];
    if(diag&&!diag.accounts_ready){
      const detail=problems.includes('firebase_admin_credentials_missing')?'Na backend doplň Firebase Admin credentials. Web Firebase konfigurácia nestačí na serverovú správu účtov.':problems.includes('firebase_admin_users_unavailable')?'Server nevie načítať Firebase používateľov. Skontroluj service-account oprávnenia a project ID.':'Správa účtov nie je pripravená. Otvor System pre presný stav.';
      diagNote=`<div class="admin-runtime-note is-error"><strong>Backend účtov vyžaduje zásah</strong><span>${escapeHtml(detail)}</span></div>`;
  ...
  function renderRoute(route){
    ...
    if(route==='admin'){host.innerHTML=renderAdminRoute();wireAdmin();loadAdminDiagnostics();if(state.adminTab==='accounts')loadAdminUsers();if(state.adminTab==='analytics')loadBannerAnalytics();if(state.adminTab==='support')loadAdminSupport();if(state.adminTab==='audit')loadAdminAudit();return;}
    if(route==='prime'){
  ...
  async function loadFeed(showLoading=true){
    ...
    try{
      ...
      if(state.route==='admin'&&!isAdminAccount())state.route='predictions'; setRoute(state.route,false); applyAccessStates();renderDashboardKpis();if(state.railMatch)renderMatchRail();else renderDashboardRightRailDefault();loadInsights(true);renderFooterConfig();window.BlinqUI.sync(feed.stale?'stale':'ready',feed.generated_at);
