(() => {
  ...
  const dashboardSectionFallback={
    overview:{label:'Overview',panel_id:'',sidebar_element:'SIDEBAR_OVERVIEW',sidebar_enabled:true,dashboard_enabled:false,dashboard_order:1,preview_limit:'ALL'},
  ...
  function applyV6525NavigationAndOverviewDefaults(){
    ...
    cfg.sidebar_element='SIDEBAR_OVERVIEW';cfg.sidebar_enabled=cfg.sidebar_enabled!==false;cfg.dashboard_enabled=false;cfg.dashboard_order=9;cfg.preview_limit='ALL';cfg.plans=cfg.plans||{};
    accessContexts.forEach(plan=>{const source=cfg.plans[plan]||cfg.plans.rookie||{};cfg.plans[plan]={visible_picks:source.visible_picks??'ALL',blur_remaining:source.blur_remaining===true,see_all:source.see_all!==false,order:Number(source.order||9)};});
  ...
  function renderDashboardComposition(){
    ...
    const top=$('bannerTop'),mid=$('bannerMid'),bottom=$('bannerBottom');
    // v6.6 public composition is intentionally strict: top navigation → three managed CTA blocks → hero → adaptive pick grid.
    // Legacy content rows stay manageable in Admin but do not interrupt the homepage composition.
    [top,mid,bottom].forEach(row=>{if(row){row.hidden=true;row.setAttribute('aria-hidden','true');}});
    grid.style.order='20';
  ...
  function heroSlideHtml(item,index){
    ...
    const proof=`<div class="hero-proof-row" aria-label="BlinQ analytical principles"><span><i aria-hidden="true">★</i>${escapeHtml(lcopy('Model predictions','Modelové predikcie','Modelové predikce'))}</span><span><i aria-hidden="true">▥</i>${escapeHtml(lcopy('Real statistics','Reálne štatistiky','Reálné statistiky'))}</span><span><i aria-hidden="true">◇</i>${escapeHtml(lcopy('Transparent results','Transparentné výsledky','Transparentní výsledky'))}</span></div>`;
    const copy=showCopy?`<div class="dashboard-hero-copy"><small>${escapeHtml(c.eyebrow||'BLINQ')}</small>${titleHtml}<p>${escapeHtml(c.text||'')}</p>${proof}${c.button_text?`<b class="hero-slide-cta">${escapeHtml(c.button_text)} →</b>`:''}</div>`:'';
    const clickable=bannerClickAllowed(item),lockedPlan=clickable?'':firstBannerClickPlan(item),finalHref=clickable?href:'#';
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    const rawLine=row?.line??row?.market_line??row?.over_under_line??row?.total_line??row?.threshold;const marketLine=Number(rawLine);const rawSide=String(row?.side??row?.selection_side??row?.over_under??row?.ou_side??'').trim().toUpperCase();const ouSide=/^(OVER|UNDER|O|U)$/.test(rawSide)?(rawSide.startsWith('O')?'OVER':'UNDER'):'';
    let badge='',mainValue='—',scoreLabel=publicText('Model'),confidenceClass='low',pickLabel=publicText(key==='prime'?'Prime Pick':key==='value'?'Value Pick':key==='ace'?'Ace / DF Pick':key==='sg'?'Set / Game Pick':'Top Bet'),note='',metrics=[];
    if(projectionOnly){
      const marketLabel=row?.market_type||String(row?.market||'Projection').replaceAll('_',' ');const sampleText=Number.isFinite(Number(samples.player1))&&Number.isFinite(Number(samples.player2))?`${publicText('Data')} ${samples.player1}/${samples.player2}`:'';const unit=String(row?.projection_unit||'count');const reference=Number(row?.reference_projection??row?.baseline_projection);scoreLabel=key==='ace'?lcopy('Player projection','Predikcia hráča','Predikce hráče'):publicText('Projection');
      if(key==='ace'&&Number.isFinite(marketLine)){
        const inferredSide=ouSide||(projection>=marketLine?'OVER':'UNDER');
        pickLabel=lcopy('Aces / double-faults O/U','Esá / dvojchyby O/U','Esa / dvojchyby O/U');
        mainValue=`${inferredSide} ${marketLine.toFixed(marketLine%1?1:0)}<small> line</small>`;
        metrics=[[lcopy('Projection','Predikcia','Predikce'),projection.toFixed(1)],[lcopy('Opponent','Súper','Soupeř'),Number.isFinite(opponentProjection)?opponentProjection.toFixed(1):'—'],[lcopy('Confidence','Istota','Jistota'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];
        note=[marketLabel,sampleText,lcopy('API line available','API hranica dostupná','API hranice dostupná')].filter(Boolean).join(' · ');
      } else 
      ...
      badge=key==='ace'&&Number.isFinite(marketLine)?'O/U':publicText('PROJECTION');confidenceClass=Number.isFinite(projectionConfidence)?confidenceBand(projectionConfidence):'medium';if(!(key==='ace'&&Number.isFinite(marketLine)))note=[marketLabel,sampleText,publicText('Projection only · no odds')].filter(Boolean).join(' · ');
    }else{
  ...
  function renderMarketSection(key,hostId,emptyText){
    ...
    const emptyMeta={
      doubles:{icon:'◎',title:lcopy('No doubles picks yet','Zatiaľ žiadne tipy na štvorhru','Zatím žádné tipy na čtyřhru'),copy:lcopy('Qualified doubles picks will appear here automatically.','Kvalifikované tipy na štvorhru sa tu zobrazia automaticky.','Kvalifikované tipy na čtyřhru se zde zobrazí automaticky.')},
      sg:{icon:'▦',title:lcopy('No sets / games picks yet','Zatiaľ žiadne tipy na sety / hry','Zatím žádné tipy na sety / hry'),copy:lcopy('New model projections will appear here when available.','Nové modelové projekcie sa zobrazia, keď budú dostupné.','Nové modelové projekce se zobrazí, až budou dostupné.')},
      ace:{icon:'●',title:lcopy('No ace markets yet','Zatiaľ žiadne predikcie es / dvojchýb','Zatím žádné predikce es / dvojchyb'),copy:lcopy('Player O/U information will appear when qualified data and lines are available.','O/U informácie pre hráčov sa zobrazia, keď budú dostupné kvalitné dáta a hranice.','O/U informace pro hráče se zobrazí, až budou dostupná kvalitní data a hranice.')}
    }[key];
    host.innerHTML=visible.length?visible.map((row,localIndex)=>{const absoluteIndex=start+localIndex;const locked=ent.blur_remaining!==false&&absoluteIndex>=unlocked;return marketPreviewCard(row,key,absoluteIndex,locked,true)}).join(''):(emptyMeta?`<div class="state-card market-empty premium-empty"><span class="empty-icon" aria-hidden="true">${emptyMeta.icon}</span><strong>${escapeHtml(emptyMeta.title)}</strong><small>${escapeHtml(emptyMeta.copy)}</small></div>`:`<div class="state-card market-empty premium-empty"><span class="empty-icon" aria-hidden="true">◇</span><strong>${escapeHtml(emptyText)}</strong><small>${escapeHtml(lcopy('This section updates automatically.','Sekcia sa aktualizuje automaticky.','Sekce se aktualizuje automaticky.'))}</small></div>`);
