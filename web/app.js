(() => {
    ...
    try {
      ...
      if(runtime?.configured && runtime?.config?.schema===2){
        ...
        if(String(runtime.config.ui_revision||'')!==String(state.uiSource.ui_revision||'')){
          ...
          if(String(state.uiSource.ui_revision||'')==='6.7.0'){
  ...
  function renderAdminAnalytics(){
    ...
    if(!data?.available)return `<div class="admin-note"><strong>Analytika bannerov nie je dostupná</strong><span>Azure Table Storage is not available in the current runtime. Configure BLINQ_ADMIN_STORAGE_CONNECTION_STRING or use AzureWebJobsStorage.</span></div><button class="btn btn-ghost" type="button" data-admin-action="refresh-analytics">Skúsiť znova</button>`;
  ...
  function renderAdminRoute(){
    const tabs=[['accounts','Účty','Levely · expirácie · Telegram'],['layout','Denná ponuka','Kategórie · tipy · prístupy'],['banners','Bannery','Horné CTA · hlavný banner'],['plans','Plány','Levely · platobné odkazy'],['campaigns','Kampane','Reklamné kreatívy'],['analytics','Analytika','Zobrazenia · kliky · CTR'],['performance','Model','Kvalita modelu']];
  ...
  async function loadAdminUsers(force=false){
    ...
    }catch(error){state.adminUsers=[];state.adminUsersError=error.status===503?'API účtov nie je nakonfigurované. Skontroluj Firebase Admin prihlasovacie údaje a nastavenie Azure Storage.':error.message;showStatus(state.adminUsersError);}
