(() => {
  ...
  const dashboardSectionKeys=['prime','top_daily','value','ace','sg','doubles','results','btts'];
  const dashboardSectionFallback={
    ...
    ace:{label:'Ace Picks',panel_id:'acePanel',sidebar_element:'SIDEBAR_ACE',sidebar_enabled:true,dashboard_enabled:false,dashboard_order:4,preview_limit:5},
    sg:{label:'S/G Picks',panel_id:'sgPanel',sidebar_element:'SIDEBAR_SG',sidebar_enabled:true,dashboard_enabled:false,dashboard_order:5,preview_limit:5},
    doubles:{label:'Doubles',panel_id:'doublesPanel',sidebar_element:'SIDEBAR_DOUBLES',sidebar_enabled:true,dashboard_enabled:false,dashboard_order:6,preview_limit:5},
    results:{label:'Results',panel_id:'resultsPreviewPanel',sidebar_element:'SIDEBAR_RESULTS',sidebar_enabled:true,dashboard_enabled:true,dashboard_order:7,preview_limit:5},
  ...
  function renderDashboardComposition(){
    ...
    const top=$('bannerTop'),mid=$('bannerMid'),bottom=$('bannerBottom');if(top)top.style.order='5';if(mid)mid.style.order='900';if(bottom)bottom.style.order='910';
    dashboardSectionKeys.forEach(key=>{
      const cfg=dashboardSectionConfig(key),panel=$(cfg.panel_id);if(!panel)return;
      panel.hidden=cfg.dashboard_enabled===false;panel.style.order=String(20+Number(cfg.dashboard_order||99)*10);
      const see=sectionSeeAllNode(key),ent=dashboardPlanEntitlement(key);
      if(see){see.hidden=false;see.classList.toggle('section-see-all-locked',!ent.see_all);see.dataset.dashboardSection=key;
        if(ent.see_all){see.dataset.route=key;see.href=key==='btts'?'/btts':`#${key}`;delete see.dataset.upgradePlan;}
        else{delete see.dataset.route;see.href='#';see.dataset.upgradePlan=firstUnlockPlan(key,0,true);const b=see.querySelector('b');if(b)b.textContent=`Unlock with ${upgradePlanLabel(see.dataset.upgradePlan).replace(/^BlinQ\s+/i,'')} →`;}
      }
    });
  }
  ...
  function renderMarketSection(key,hostId,emptyText){
    ...
    const perPage=Math.min(5,pageSize()),pageCount=Math.max(1,Math.ceil(preview.length/perPage));
  ...
  function renderPredictions(){
    const allRows=rankedPredictions(),limit=dashboardPreviewLimit('prime',allRows.length||1),rows=allRows.slice(0,limit),grid=$('predictionGrid'),size=Math.min(5,pageSize());if(!grid)return;
  ...
  function resultCategoryLabel(value){return ({all:'All published',prime:'Prime',top_daily:'Top Bet',value:'Value',ace:'Aces',double_faults:'Double Faults',sets:'Sets',games:'Games'})[value]||String(value||'').replaceAll('_',' ');}
  ...
  function wireAccount(){
    ...
    $('logoutButton').onclick=signOutCurrentSession;
  }
