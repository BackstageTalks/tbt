(() => {
  ...
  const routeMeta={
    ...
    top_daily:['BALANCED EDGE','Top Bets','Best combination of model probability, price, edge and expected value.'],
  ...
  function renderRoute(route){
    ...
    else if(route==='top_daily'){const r=state.ui?.market_rules?.top_daily||{};const day=state.feed?.market_selection?.odds_report?.betting_day||'current';body=`<div class="route-sub route-rules"><strong>Top Bets rule</strong><span>BlinQ betting day ${escapeHtml(day)} · model ${Math.round(Number(r.min_probability||.72)*100)}%+ · odds ${Number(r.min_odds||1.50).toFixed(2)}+ · edge ${Math.round(Number(r.min_edge||.02)*100)} pp+ · EV ${Math.round(Number(r.min_expected_value||.03)*100)}%+ · max ${Number(r.limit||10)} picks.</span></div>${genericTable(marketRows('top_daily'),'top_daily')}`;}
