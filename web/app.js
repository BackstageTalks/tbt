(() => {
  ...
  function marketPreviewCard(row,key,index=0,locked=false,compact=false){
    ...
    if(projectionOnly){
      ...
      if(key==='sg'&&unit==='probability'){mainValue=`${pct(projection)}<small> proj.</small>`;metrics=[[lcopy('Baseline','Základ','Základ'),Number.isFinite(reference)?pct(reference):'—'],[lcopy('Gap','Rozdiel','Rozdíl'),Number.isFinite(projectionGap)?`${(projectionGap*100).toFixed(1)} pp`:'—'],[lcopy('Score','Skóre','Skóre'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];pickLabel=publicText('Sets Projection');}
      else if(key==='sg'&&unit==='games'){mainValue=`${projection.toFixed(1)}<small> games</small>`;metrics=[[lcopy('Baseline','Základ','Základ'),Number.isFinite(reference)?reference.toFixed(1):'—'],[lcopy('Gap','Rozdiel','Rozdíl'),Number.isFinite(projectionGap)?`${projectionGap.toFixed(1)}`:'—'],[lcopy('Score','Skóre','Skóre'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];pickLabel=publicText('Games Projection');}
      else{mainValue=`${projection.toFixed(1)}<small> proj.</small>`;metrics=[[lcopy('Opponent','Súper','Soupeř'),Number.isFinite(opponentProjection)?opponentProjection.toFixed(1):'—'],[lcopy('Gap','Rozdiel','Rozdíl'),Number.isFinite(projectionGap)?`+${projectionGap.toFixed(1)}`:'—'],[lcopy('Score','Skóre','Skóre'),Number.isFinite(projectionConfidence)?`${Math.round(projectionConfidence*100)}/100`:'—']];pickLabel=publicText('Ace / DF Projection');}
  ...
  function renderAdminLayout(){
    ...
    const headerCount=Math.max(0,Math.min(3,Number(state.ui?.header_cta?.slot_count??3)||0));
    const countOptions=(selected,allowOff=true,maxCount=4)=>[...(allowOff?[0]:[]),...Array.from({length:maxCount},(_,i)=>i+1)].map(v=>`<option value="${v}"${Number(selected)===v?' selected':''}>${v===0?'OFF':`${v} block${v===1?'':'s'}`}</option>`).join('');
    ...
    return `<div class="admin-toolbar"><label>Editing access for<select id="adminPlanSelect">${planOptions}</select></label><label>Copy all access from<select id="adminCopyFrom">${copyOptions}</select></label><button class="btn btn-ghost" type="button" data-admin-action="copy-plan">Copy → ${escapeHtml(accessLabel(state.adminPlan))}</button><span class="admin-toolbar-spacer"></span><button class="btn btn-ghost" type="button" data-admin-action="preview-demo">Preview full board</button><button class="btn btn-ghost" type="button" data-admin-action="preview">Preview as ${escapeHtml(accessLabel(state.adminPlan))}</button><button class="btn btn-ghost" type="button" data-admin-action="save-draft">Save browser draft</button><button class="btn btn-primary" type="button" data-admin-action="publish-config">Publish changes</button><button class="btn btn-ghost" type="button" data-admin-action="export">Export JSON</button></div><div class="admin-note admin-banner-note"><strong>Simple BlinQ control</strong><span>Configure only the three top CTA banners, the main rotating banner and section access/order. The public page structure stays fixed.</span></div><div class="admin-zone-controls"><div class="admin-row-control header-row-control"><label>HEADER CTA / TELEGRAM<select data-admin-header-count>${countOptions(headerCount,true,3)}</select></label><small>Use 1 banner across the full CTA area, or split the fixed area into 2–3 top banners.</small></div><div class="admin-row-control hero-row-control"><label>MAIN BANNER SLIDES<select data-admin-hero-count>${countOptions(heroCfg.enabled===false?0:heroCount,true,5)}</select></label><label class="admin-inline-setting">Rotation <input type="number" min="3" max="300" step="1" data-admin-hero-seconds value="${heroSeconds}"> sec</label><label class="check-field"><input type="checkbox" data-admin-hero-rotate ${heroCfg.auto_rotate!==false?'checked':''}> Auto rotate</label><label class="check-field"><input type="checkbox" data-admin-hero-dots ${heroCfg.show_dots!==false?'checked':''}> Dots</label><small>One full-width banner is visible at a time. Configure up to 5 creatives and set the change interval from 3–300 seconds.</small></div></div><div class="admin-editor-grid"><div>${renderAdminCanvas()}</div>${renderAdminInspector()}</div>`;
  }
  ...
  function renderAdminBanners(){
    ...
    return `<div class="admin-toolbar admin-toolbar-clean"><div><strong>Banner manager</strong><small>Fixed slots, simple content, links and per-level access.</small></div><span class="admin-toolbar-spacer"></span><button class="btn btn-ghost" type="button" data-admin-action="save-draft">Save draft</button><button class="btn btn-primary" type="button" data-admin-action="publish-config">Publish changes</button></div>
      <section class="admin-banner-manager-section"><div class="admin-section-title"><strong>Top CTA banners</strong><span>Use 1, 2 or 3 equal blocks. A single banner automatically uses the whole CTA area.</span></div><div class="admin-banner-global"><label>Active top banners<select data-admin-header-count>${countOptions(headerCount,3)}</select></label></div><div class="admin-simple-banner-grid top-cta-grid">${[1,2,3].map(i=>renderSimpleBannerCard(`HEADER_BANNER_${i}`,i,'header')).join('')}</div></section>
      <section class="admin-banner-manager-section"><div class="admin-section-title"><strong>Main rotating banner</strong><span>Up to 5 slides. One full-width creative is shown at a time.</span></div><div class="admin-banner-global hero-banner-global"><label>Active slides<select data-admin-hero-count>${countOptions(heroCfg.enabled===false?0:heroCount,5)}</select></label><label>Rotate every<input type="number" min="3" max="300" step="1" data-admin-hero-seconds value="${heroSeconds}"><span>sec</span></label><label class="admin-switch"><input type="checkbox" data-admin-hero-rotate ${heroCfg.auto_rotate!==false?'checked':''}><span>Auto rotate</span></label><label class="admin-switch"><input type="checkbox" data-admin-hero-dots ${heroCfg.show_dots!==false?'checked':''}><span>Dots</span></label></div><div class="admin-simple-banner-grid hero-admin-grid">${[1,2,3,4,5].map(i=>renderSimpleBannerCard(`HERO_BANNER_${i}`,i,'hero')).join('')}</div></section>`;
  }
  ...
  function renderAdminRoute(){
    const tabs=[['layout','Layout & access','Section visibility, pick counts and per-level ordering'],['banners','Banners & links','Top CTA banners, Telegram links and main rotating banner'],['plans','Plans','Membership catalogue and access terms'],['accounts','Accounts','Member access and roles']];
    ...
    return `<div class="admin-console"><div class="admin-tabs">${tabs.map(([id,label,hint])=>`<button type="button" class="${state.adminTab===id?'active':''}" data-admin-tab="${id}" title="${escapeHtml(hint||'')}">${label}</button>`).join('')}</div><div class="admin-panel">${panel}</div></div>`;
  }
