(() => {
  ...
  function renderInsightBell(){
    ...
    const infoEligible=['elite','legend','goat','admin'].includes(plan);
    const liveEligible=infoEligible;
    bell.hidden=!eligible;
    bell.classList.toggle('is-access-locked',eligible&&!infoEligible);
    if(eligible&&!infoEligible){bell.dataset.upgradePlan='elite';bell.dataset.upgradeSection='Premium Info';bell.setAttribute('aria-label','Premium Info · dostupné od ELITE');}
    else{delete bell.dataset.upgradePlan;delete bell.dataset.upgradeSection;bell.setAttribute('aria-label','Premium Info');}
  ...
  async function loadInsights(force=false){
    const plan=accountPlan();
    if(!['elite','legend','goat','admin'].includes(plan)){state.insights=[];state.insightsUnread=0;state.insightsStorageUnavailable=false;renderInsightBell();return;}
  ...
  async function refreshPrivateUpdates(force=false){
    if(state.privateUpdatesBusy||document.hidden||$('appShell')?.hidden)return;const plan=accountPlan();if(!['elite','legend','goat','admin'].includes(plan))return;const now=Date.now();if(!force&&now-Number(state.privateUpdatesLastPoll||0)<25000)return;state.privateUpdatesBusy=true;state.privateUpdatesLastPoll=now;try{await loadInsights(true);await loadUserLiveRadarStatus(false);}finally{state.privateUpdatesBusy=false;}
  }
  ...
  function renderResults(){
    ...
    if(!entries.length){state.resultsPage=0;return '<div class="state-card">Zatiaľ nie sú dostupné vyhodnotené publikované predikcie pre tento filter.</div>';}
    ...
    return `<div class="admin-table-wrap results-table-wrap"><table class="admin-analytics-table results-table"><thead><tr><th>Dátum</th><th>Kategória</th><th>Zápas</th><th>Predikcia</th><th>BlinQ %</th><th>Kurz</th><th>Výsledok</th><th>Jednotky</th></tr></thead><tbody>${body}</tbody></table></div>${pager}`;
  }
