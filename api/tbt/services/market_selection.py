...
PRIME_DAILY_FLOOR = 0.68
PRIME_PREFERRED_PROBABILITY = 0.85
PRIME_FALLBACK_PROBABILITIES = (0.83, 0.81, 0.79, 0.77, 0.75, 0.73, 0.71, 0.69)
...
# confidence-first policy: start at 80%, then relax in 2 percentage-point
# steps only when fewer than ten remaining picks qualify. The 68% floor is
# absolute; data-depth, surface-sample and real-odds guardrails never relax.
# Once a tier reaches the target, ALL qualifying picks in that tier are kept.
TOP_PREFERRED_PROBABILITY = 0.80
TOP_SECONDARY_PROBABILITY = 0.78
TOP_STANDARD_PROBABILITY = 0.76
TOP_FALLBACK_PROBABILITIES = (0.74, 0.72, 0.70)
...
VALUE_MIN_COUNT = 5
# Value fallback tiers keep real odds and positive model/market disagreement, but
# progressively widen the price/edge/EV window so the daily board is useful.
...
def select_market_sections(
    ...
    *,
    ...
    balanced_daily_fill = (
        len(cards) >= (PRIME_MIN_COUNT + TOP_MIN_COUNT + VALUE_MIN_COUNT)
        and float(prime_min_probability) == float(PRIME_MIN_PROBABILITY)
        and float(top_min_probability) == float(TOP_MIN_PROBABILITY)
        and float(value_min_probability) == float(VALUE_MIN_PROBABILITY)
    )
    effective_prime_floor = (
        PRIME_DAILY_FLOOR
        if balanced_daily_fill and float(prime_min_probability) == float(PRIME_MIN_PROBABILITY)
        else float(prime_min_probability)
    )
    ...
    for card in cards:
        if _passes_candidate_gate(
            card,
            min_probability=effective_prime_floor,
    ...
    if balanced_daily_fill:
        # Daily allocator: guarantee the minimum board density before any section
        # is allowed to expand toward its preferred target. This prevents Prime
        # or Top from consuming every shared candidate while another public
        # section is left with one or two cards. Quality/sample/real-price gates
        # above remain hard; only the ranking threshold adapts.
        value_tier_pool, value_tier_counts, value_applied_tier = _value_tier_candidates(value_qualified)
        value_tier_pool.sort(
            key=lambda card: (
                _number(card.get("expected_value")) or -999.0,
                _number(card.get("edge")) or -999.0,
                _number(card.get("probability")) or 0.0,
        ...
        # Reserve Value cards that are least likely to be needed by the
        # confidence-first sections, then reserve the five strongest Prime and
        # five strongest remaining Top cards.
        value_reserve_order = [c for c in value_tier_pool if (_number(c.get("probability")) or 0.0) < TOP_PREFERRED_PROBABILITY]
        value_reserve_order += [c for c in value_tier_pool if (_number(c.get("probability")) or 0.0) >= TOP_PREFERRED_PROBABILITY]
        value = value_reserve_order[:VALUE_MIN_COUNT]
        claimed = {_selection_identity(card) for card in value}

        prime_remaining = [card for card in prime_qualified if _selection_identity(card) not in claimed]
        prime_min_pool, _, _ = _confidence_cascade_select(
            prime_remaining,
            thresholds=(PRIME_PREFERRED_PROBABILITY, *PRIME_FALLBACK_PROBABILITIES),
            minimum_probability=effective_prime_floor,
            target_count=PRIME_MIN_COUNT,
        )
        prime = prime_min_pool[:PRIME_MIN_COUNT]
        claimed.update(_selection_identity(card) for card in prime)
        ...
        top_min_pool, _, _ = _top_cascade_select(
            top_remaining, minimum_probability=top_min_probability, target_count=TOP_MIN_COUNT
        )
        top = top_min_pool[:TOP_MIN_COUNT]
        claimed.update(_selection_identity(card) for card in top)

        # Once every feasible section has its five-card dashboard reserve, expand
        # toward ten in product order without reusing an underlying selection.
        for card in prime_qualified:
            if len(prime) >= PRIME_TARGET_COUNT:
                break
            identity = _selection_identity(card)
            if identity in claimed:
                continue
            prime.append(card); claimed.add(identity)

        # Re-run the Top confidence cascade after the protected Value minimum
        # and Prime allocation are fixed. This preserves the established
        # product rule: when the first threshold that reaches the preferred
        # target contains more than ten picks, keep the whole confidence tier.
        value_prime_ids={_selection_identity(card) for card in (value+prime)}
        top_available=[card for card in top_qualified if _selection_identity(card) not in value_prime_ids]
        top_cascade, top_applied_floor, top_tier_counts = _top_cascade_select(
            top_available, minimum_probability=top_min_probability, target_count=TOP_TARGET_COUNT
        )
        top=list(top_cascade)
        claimed=value_prime_ids|{_selection_identity(card) for card in top}

        reserved_ids = {_selection_identity(v) for v in value}
        for card in value_tier_pool:
            if len(value) >= VALUE_TARGET_COUNT:
                break
            identity = _selection_identity(card)
            if identity in claimed or identity in reserved_ids:
                continue
            value.append(card); claimed.add(identity)

        # Metadata still reports the effective adaptive floors selected by the
        # final board, rather than pretending a fixed threshold was used.
        prime_applied_floor = min((_number(card.get("probability")) or 0.0 for card in prime), default=None)
        prime_tier_counts = {f"{threshold:.2f}": sum(1 for card in prime_qualified if (_number(card.get("probability")) or 0.0) >= threshold) for threshold in sorted({PRIME_PREFERRED_PROBABILITY, *PRIME_FALLBACK_PROBABILITIES, effective_prime_floor}, reverse=True)}
        prime_cascade = list(prime)
        top_cascade_excluded = max(0, len(top_available) - len(top_cascade))
        top_remaining = top_available

        if prime_limit is not None and len(prime) > max(0, int(prime_limit)):
            before=len(prime); prime=prime[:max(0,int(prime_limit))]; prime_limited_out=before-len(prime)
        else:
            prime_limited_out=0
        if top_limit is not None and len(top) > max(0, int(top_limit)):
            before=len(top); top=top[:max(0,int(top_limit))]; top_limited_out=before-len(top)
        else:
            top_limited_out=0
        if value_limit is not None and len(value) > max(0, int(value_limit)):
            before=len(value); value=value[:max(0,int(value_limit))]; value_limited_out=before-len(value)
        else:
            value_limited_out=0

        value_ids={_selection_identity(card) for card in value}
        prime_ids={_selection_identity(card) for card in prime}
        top_ids={_selection_identity(card) for card in top}
        duplicate_removed = {
            "prime": sum(1 for card in prime_qualified if _selection_identity(card) in value_ids),
            "top_daily": sum(1 for card in top_qualified if _selection_identity(card) in (value_ids|prime_ids)),
            "value": sum(1 for card in value_tier_pool if _selection_identity(card) in (prime_ids|top_ids)),
        }
        limited_out = {
            "prime": prime_limited_out,
            "top_daily": top_limited_out,
            "value": value_limited_out,
        }
    else:
        # Preserve legacy semantics for thin candidate pools and focused tests.
        value_tier_pool = []
        value_tier_counts = {}
        value_applied_tier = None
        prime_tier_counts = {f"{float(prime_min_probability):.2f}": len(prime_qualified)}
        prime_applied_floor = float(prime_min_probability)
        prime_cascade = list(prime_qualified)
        prime_exclusive, prime_removed, prime_limited = _exclusive_section_assignment(
            {"prime": prime_qualified}, priority=("prime",), limits={"prime": prime_limit}
        )
        ...
        top_duplicate_removed = len(top_qualified) - len(top_remaining)
        top_cascade, top_applied_floor, top_tier_counts = _top_cascade_select(
            top_remaining, minimum_probability=top_min_probability, target_count=TOP_TARGET_COUNT
        )
    ...
    return {
        ...
        "market_selection": {
            "schema": 10,
            "selection_policy": "prime_top_value_v7_min5_before_expand",
            "selection_counts": {
                ...
                "balanced_daily_fill": bool(balanced_daily_fill),
                "minimum_fill_threshold_rows": int(PRIME_MIN_COUNT + TOP_MIN_COUNT + VALUE_MIN_COUNT),
                "minimum_shortfall": {"prime": max(0, PRIME_MIN_COUNT-len(prime)), "top_daily": max(0, TOP_MIN_COUNT-len(top)), "value": max(0, VALUE_MIN_COUNT-len(value))},
            },
            ...
            "prime_cascade": {
                "target_count": int(PRIME_TARGET_COUNT),
                "minimum_dashboard_count": int(PRIME_MIN_COUNT),
                "tiers": [float(PRIME_PREFERRED_PROBABILITY), *[float(v) for v in PRIME_FALLBACK_PROBABILITIES], float(effective_prime_floor)],
                "applied_floor": None if prime_applied_floor is None else float(prime_applied_floor),
                "tier_counts": prime_tier_counts,
                "selected_before_optional_hard_limit": len(prime_cascade),
                "presentation_preview_limit": 10,
            },
            "top_cascade": {
                "target_count": int(TOP_TARGET_COUNT),
                "minimum_dashboard_count": int(TOP_MIN_COUNT),
                "tiers": [
                    float(TOP_PREFERRED_PROBABILITY),
                    float(TOP_SECONDARY_PROBABILITY),
                    float(TOP_STANDARD_PROBABILITY),
                    *[float(value) for value in TOP_FALLBACK_PROBABILITIES],
            ...
            "value_fill": {
                "target_count": int(VALUE_TARGET_COUNT),
                "minimum_dashboard_count": int(VALUE_MIN_COUNT),
                "tiers": [
                    {"min_odds": float(o), "min_edge": float(e), "min_expected_value": float(ev)}
                    for o, e, ev in VALUE_FALLBACK_TIERS
                ],
                "applied_tier": None if value_applied_tier is None else {
                    "min_odds": float(value_applied_tier[0]),
                    "min_edge": float(value_applied_tier[1]),
                    "min_expected_value": float(value_applied_tier[2]),
                },
                "tier_counts": value_tier_counts,
                "reserved_before_confidence_sections": min(len(value_tier_pool), int(VALUE_MIN_COUNT)),
                "presentation_preview_limit": 10,
            },
            "current_outputs": (
                ["match_winner"]
            ...
            "main_candidate_rule": {
                "min_probability": float(effective_prime_floor if balanced_daily_fill else prime_min_probability),
                "preferred_probability": float(PRIME_PREFERRED_PROBABILITY),
                "target_count": int(PRIME_TARGET_COUNT),
                "minimum_dashboard_count": int(PRIME_MIN_COUNT),
                "confidence_cascade": "85_then_83_then_81_then_79_then_77_then_75_then_73_then_71_then_69_then_68",
            ...
            "top_daily_rule": {
                ...
                "limit": top_limit,
                "presentation_preview_limit": 10,
                "cascade": "80_then_78_then_76_then_74_then_72_then_70_then_68_until_at_least_10_keep_full_tier",
            ...
            "value_rule": {
                ...
                "target_count": int(VALUE_TARGET_COUNT),
                "minimum_dashboard_count": int(VALUE_MIN_COUNT),
                "fill_policy": "reserve_5_value_style_then_expand_nonnegative_ev_tiers_to_10",
                "limit": value_limit,
