...
# Prime/Top may not relax below 65% BlinQ Probability; Value may not relax below 60%.
#
...
PRIME_TOP_MIN_PROBABILITY = 0.65

# PRIME: any decimal price below 1.50 (no lower odds bound).
PRIME_MIN_PROBABILITY = PRIME_TOP_MIN_PROBABILITY
PRIME_DAILY_FLOOR = PRIME_TOP_MIN_PROBABILITY
PRIME_PREFERRED_PROBABILITY = 0.80
PRIME_FALLBACK_PROBABILITIES = (0.78, 0.76, 0.74, 0.72, 0.70, 0.68, 0.65)
...
TOP_FALLBACK_PROBABILITIES = (0.74, 0.72, 0.70, 0.68, 0.65)
TOP_MIN_PROBABILITY = PRIME_TOP_MIN_PROBABILITY
TOP_TARGET_COUNT = 10
TOP_MIN_COUNT = 5
...
def select_market_sections(
    ...
    *,
    ...
    """Split priced Match Winner predictions by probability + price only.

    Hard invariant: Prime/Top require at least 65% BlinQ Probability; Value
    requires at least 60%. Low-evidence rows fail closed through data-depth/surface
    gates. EV/edge are retained for diagnostics but never qualify/disqualify a
    row. Value is assigned first, then Prime, then Top.
    """
    cards = [card for row in predictions if (card := _market_card(row)) is not None]

    # Product floors are applied to evidence-adjusted BlinQ Probability.
    # Prime/Top never fall below 65%; Value never falls below 60%.
    prime_floor = max(PRIME_TOP_MIN_PROBABILITY, float(prime_min_probability))
    top_floor = max(PRIME_TOP_MIN_PROBABILITY, float(top_min_probability))
    value_floor = max(PUBLICATION_MIN_PROBABILITY, float(value_min_probability))

    prime_qualified: list[dict[str, Any]] = []
    top_qualified: list[dict[str, Any]] = []
    ...
    for card in cards:
        ...
        # VALUE first by product definition: >=1.80 + close two-way prices.
        if (
        ...
        # PRIME is exclusively the short-price branch.
        if (
            odds < PRIME_MAX_ODDS_EXCLUSIVE
            and _passes_candidate_gate(
                card, min_probability=prime_floor,
        ...
        # TOP is all sufficiently strong remaining prices from 1.50 upward.
        if (
            odds >= float(top_min_odds or TOP_MIN_ODDS or 1.50)
            and _passes_candidate_gate(
                card, min_probability=top_floor,
    ...
    prime_qualified.sort(key=rank, reverse=True)
    top_qualified.sort(key=rank, reverse=True)
    value_qualified.sort(key=rank, reverse=True)

    # Explicit assignment rather than generic SECTION_PRIORITY so Value always
    # wins overlaps even if a legacy caller passes an older priority tuple.
    ...
    if any((_number(card.get("probability")) or 0.0) + 1e-12 < PRIME_TOP_MIN_PROBABILITY for card in (prime + top)):
        raise ValueError("Prime/Top probability floor invariant failed")
    ...
    return {
        ...
        "market_selection": {
            "schema": 12,
            "selection_policy": "probability_first_odds_buckets_v9",
            "selection_counts": {
                ...
                "balanced_daily_fill": False,
                "minimum_fill_threshold_rows": 0,
                "minimum_shortfall": {
            ...
            "main_candidate_rule": {
                "min_probability": PUBLICATION_MIN_PROBABILITY,
            ...
            "prime_rule": {
                ...
                "min_probability": prime_floor,
                "min_data_depth": float(prime_min_data_depth),
                "min_surface_matches_each": int(prime_min_surface_matches),
                "min_odds": None,
                "max_odds_exclusive": PRIME_MAX_ODDS_EXCLUSIVE,
                "display_max_odds": PRIME_PREFERRED_MAX_ODDS,
