...
def validate_ui_config(payload: object) -> dict:
    ...
    if dashboard.get("visible_slots") != 4:
        raise ValueError("Dashboard must expose exactly four active pick windows")
    if dashboard.get("user_switches") is not True:
        raise ValueError("Dashboard user section switches must remain enabled")
    if not isinstance(dashboard.get("show_disabled_strip"), bool):
        raise ValueError("Invalid dashboard disabled-section strip setting")
