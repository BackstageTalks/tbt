...
class RapidTennisClient:
    ...
    def tournament_logo(self, tournament_id: str | int) -> tuple[bytes, str] | None:
        """Current presentation logo for a tournament id.

        Prefer TennisApi's dark logo on the BlinQ dark UI. If a tournament does
        not expose a dark variant (204/404), fall back to the regular logo.
        Tournament artwork is presentation-only metadata and must never enter
        model features.
        """
        dark = self._get_bytes(
            f"/api/tennis/tournament/{tournament_id}/image/dark",
            enrichment=True,
        )
        if dark is not None:
            return dark
        return self._get_bytes(
            f"/api/tennis/tournament/{tournament_id}/image",
            enrichment=True,
        )
