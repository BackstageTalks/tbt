...
def test_prime_top_and_value_follow_distinct_working_policy():
    ...
    assert sections['market_selection']['top_daily_rule']['preferred_probability'] == .80
    assert sections['market_selection']['top_daily_rule']['secondary_probability'] == .78
    assert sections['market_selection']['top_daily_rule']['standard_probability'] == .76
...
def test_top_bets_keep_entire_tier_when_target_is_reached_without_default_hard_cap():
    ...
    assert len(sections['top_daily_picks']) == 11
    cascade = sections['market_selection']['top_cascade']
    assert cascade['target_count'] == 10
    assert cascade['applied_floor'] == .78
    assert cascade['selected_before_optional_hard_limit'] == 11
...
def test_large_daily_inventory_balances_prime_top_and_value_to_useful_board_density():
    rows = []
    for i in range(60):
        probability = .90 - (i % 23) * .01
        odds = 1.25 + (i % 10) * .10
        implied = max(.25, min(.80, 1 / odds - .04 - (i % 4) * .01))
        rows.append(row(f'daily-{i}', probability, odds, implied, depth=.90, surface1=10, surface2=10))

    sections = select_market_sections(rows)
    assert len(sections['prime_picks']) >= 5
    assert len(sections['top_daily_picks']) >= 5
    assert len(sections['value_picks']) >= 5
    identities = [x['event_id'] for x in sections['prime_picks'] + sections['top_daily_picks'] + sections['value_picks']]
    assert len(identities) == len(set(identities))
    meta = sections['market_selection']
    assert meta['schema'] == 10
    assert meta['selection_counts']['balanced_daily_fill'] is True
    assert meta['selection_counts']['daily_minimum_target'] == 5
    assert meta['selection_counts']['daily_preferred_target'] == 10
    assert meta['prime_cascade']['minimum_dashboard_count'] == 5
    assert meta['top_cascade']['minimum_dashboard_count'] == 5
    assert meta['value_fill']['minimum_dashboard_count'] == 5


def test_minimum_five_allocator_activates_as_soon_as_fifteen_priced_rows_make_it_possible():
    rows=[]
    for i in range(18):
        probability=.86-(i%10)*.015
        odds=1.35+(i%6)*.12
        implied=max(.30,min(.78,1/odds-.05))
        rows.append(row(f'mid-{i}', probability, odds, implied, depth=.92, surface1=12, surface2=12))
    sections=select_market_sections(rows)
    assert sections['market_selection']['selection_counts']['balanced_daily_fill'] is True
    assert len(sections['prime_picks']) >= 5
    assert len(sections['top_daily_picks']) >= 5
    assert len(sections['value_picks']) >= 5
    ids=[x['event_id'] for x in sections['prime_picks']+sections['top_daily_picks']+sections['value_picks']]
    assert len(ids)==len(set(ids))
    assert sections['market_selection']['selection_counts']['minimum_fill_threshold_rows']==15
    assert sections['market_selection']['selection_counts']['minimum_shortfall']=={'prime':0,'top_daily':0,'value':0}
