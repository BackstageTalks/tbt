...
def test_r32_identity_background_and_loader_contract():
    ...
    assert release["patch"] == UI["ui_patch"] == "736-r46"
    assert 'content="736-r46"' in INDEX
    ...
    assert "/assets/blinq_loading_r29.svg?v=7360&p=46" in INDEX
...
def test_r32_custom_email_uses_real_blinq_logo_and_csp_safe_action_page():
    ...
    assert '<script src="/auth/action/action.js?v=7360&p=46"></script>' in action_html
