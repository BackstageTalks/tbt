...
def _identity_repo() -> tuple[SupabaseRepository, dict]:
    ...
    def fake_select_all(table, filters=None, order=None, **kwargs):
        ...
        return [
            {"match_id": "a", "scheduled_at": "2026-09-01T00:00:00+00:00"},
            {"match_id": "b", "scheduled_at": "2026-09-02T00:00:00+00:00"},
        ]

    repo.select_all = fake_select_all
    repo._match_from_row = lambda row: row
...
def test_completed_match_read_is_rolling_hot_tier_not_full_history():
    """Regression guard for V20.7+: completed reads must be time-bounded in Supabase."""
    ...
    assert [row["match_id"] for row in matches] == ["a", "b"]
    ...
    cutoff = datetime.fromisoformat(filters["scheduled_at"].removeprefix("gte."))
    # Default policy is rolling and evaluated at runtime. Allow a broad range so this
    # test does not hard-code a calendar year or a particular retention setting.
...
def test_completed_matches_since_uses_explicit_delta_window():
    ...
    before = datetime(2026, 9, 1, 12, 0, tzinfo=timezone.utc)

    matches = repo.get_completed_matches_since(after=after, before=before)

    assert [row["match_id"] for row in matches] == ["a", "b"]
    filters = captured["filters"]
    assert filters["winner_id"] == "not.is.null"
    assert filters["scheduled_at"] == f"gte.{after.isoformat()}"
    assert filters["and"] == f"(scheduled_at.lt.{before.isoformat()})"
