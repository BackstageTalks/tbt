...
def test_rebuild_keeps_v64_dashboard_contract_and_complete_assets():
    cfg = json.loads((ROOT / "web" / "ui-config.json").read_text(encoding="utf-8"))
    assert cfg["ui_revision"] == "6.4.0"
    for name in (
        "blinq_logo.svg",
...
def test_rebuild_keeps_v62_top_bets_adaptive_cascade():
    source = (ROOT / "api" / "tbt" / "services" / "market_selection.py").read_text(encoding="utf-8")
    assert "TOP_PREFERRED_PROBABILITY = 0.75" in source
    assert "TOP_SECONDARY_PROBABILITY = 0.72" in source
    assert "TOP_STANDARD_PROBABILITY = 0.70" in source
    assert "TOP_TARGET_COUNT = 10" in source
    assert "prime_top_value_v4_adaptive_top_cascade" in source
