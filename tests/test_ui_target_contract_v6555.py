...
def test_release_and_cache_are_6600():
    ...
    assert cfg["ui_revision"] == "6.6.7"
    assert cfg["revision"] == "6.6.7"
    assert 'RELEASE = "6.6.7"' in read("api/function_app.py")
    html = read("web/index.html")
    assert '/blinq.css?v=667' in html
    for asset in ("auth.js", "responsive.js", "app.js"):
        assert f'/{asset}?v=667' in html
