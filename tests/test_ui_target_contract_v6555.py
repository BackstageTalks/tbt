...
def test_release_and_cache_are_6555():
    cfg = json.loads(read("web/ui-config.json"))
    assert cfg["ui_revision"] == "6.5.55"
    assert cfg["revision"] == "6.5.55"
    assert 'RELEASE = "6.5.55"' in read("api/function_app.py")
    html = read("web/index.html")
    for asset in ("styles.css", "responsive.css", "premium.css", "premium-v2.css", "final-ui.css", "auth.js", "responsive.js", "app.js"):
        assert f'/{asset}?v=6555' in html


def test_final_ui_is_loaded_last_and_beats_legacy_specificity():
    html = read("web/index.html")
    assert html.index("premium-v2.css?v=6555") < html.index("final-ui.css?v=6555")
