...
def test_date_range_controls_exist_and_filter_rows():
    assert 'id="dailyHubDateFrom"' in HTML
    assert 'id="dailyHubDateTo"' in HTML
    assert 'id="dailyHubDateClear"' in HTML
    filtered = APP.split("function dashboardFilteredRows(rows){",1)[1].split("function dailyHubColumns",1)[0]
    assert "state.dailyHubDateFrom" in filtered
    assert "state.dailyHubDateTo" in filtered
    assert "ts<fromTs" in filtered
    assert "ts>toTs" in filtered
    assert ".daily-date-range" in CSS
