...
## 6.5.11 legacy admin login compatibility
- Firebase sign-in now preserves the valid session and lets the API enforce email verification.
- Normal unverified members remain blocked.
- Accounts listed in `BLINQ_ADMIN_EMAILS` may enter as legacy bootstrap admins even if they pre-date verification; Account visibly marks that email as still unverified.


## v6.5.14 admin cleanup
- RSS editor removed from Admin and RSS fallback disabled.
- Model Performance renamed to Model Quality with a clear diagnostics explanation.
- Campaigns renamed to Banner campaigns and marked optional for reusable/scheduled ads.
- Membership artwork now uses fresh text-free v6514 asset paths to avoid stale browser cache.
