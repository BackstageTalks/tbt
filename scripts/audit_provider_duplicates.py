...
def _count_match_rows(repo: SupabaseRepository) -> int:
    """
    Count rows without downloading the whole matches table.

    PostgREST returns the exact total in Content-Range when Prefer=count=exact
    is used. Only one tiny row is requested.
    """
    ...
    if "/" not in content_range:
        raise RuntimeError(
            "Supabase count query did not return Content-Range: "
            f"{content_range!r}"
        )
    ...
    if not total_text.isdigit():
        raise RuntimeError(
            "Supabase count query returned an invalid total: "
            f"{content_range!r}"
        )
    ...
    while True:
        ...
        if last_match_id is not None and current_last <= last_match_id:
            raise RuntimeError(
                ...
                f"previous={last_match_id!r}, current={current_last!r}"
            )
...
def _build_report(
    *,
            ...
            detailed_groups.append(
                {
                    ...
                    "count": len(rows),
                    "same_real_match_signature": same_real_match,
                    "distinct_match_ids": len(match_ids),
                    "distinct_tournament_ids": len(tournament_ids),
                    "match_ids": sorted(match_ids),
                    "tournament_ids": sorted(tournament_ids),
                    "tournament_names": sorted(tournament_names),
                    "round_names": sorted(round_names),
                    "rows": [
                        _compact_row(row)
                        for row in rows
                    ],
    ...
    return {
        ...
        "duplicates": {
            "provider_event_duplicate_groups": len(duplicate_groups),
            "duplicate_rows_total": duplicate_rows_total,
            "extra_rows_if_deduped": extra_rows,
            "extra_row_ratio": extra_ratio,
            "extra_row_ratio_percent": extra_ratio * 100.0,
        ...
        "duplicate_rows_by_tour": dict(
            sorted(duplicate_rows_by_tour.items())
        ),
        "duplicate_rows_by_year": dict(
            sorted(duplicate_rows_by_year.items())
        ),
        "top_duplicate_groups": detailed_groups,
        "diagnostic_notes": {
            "same_real_match_signature_groups": (
...
def main() -> None:
    ...
    print("READ ONLY: no DB rows were modified.")

    # Duplicates are exactly what this diagnostic workflow investigates,
    # therefore the script intentionally does NOT fail merely because
