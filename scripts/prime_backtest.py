...
def _selection_stats(full_frame: pd.DataFrame, subset: pd.DataFrame) -> dict[str, Any]:
    stats = _summary(subset)
    total = int(len(full_frame))
    days_total = int(full_frame["day"].nunique()) if not full_frame.empty else 0
    days_selected = int(subset["day"].nunique()) if not subset.empty else 0
    counts = subset.groupby("day").size() if not subset.empty else pd.Series(dtype=float)
    mean_probability = stats.get("mean_model_probability")
    ...
    stats.update(
        {
            ...
            "day_coverage": float(days_selected / days_total) if days_total else 0.0,
            "mean_picks_per_day": float(len(subset) / days_selected) if days_selected else 0.0,
            "median_picks_per_day": float(counts.median()) if len(counts) else 0.0,
            "min_picks_per_day": int(counts.min()) if len(counts) else 0,
            "max_picks_per_day": int(counts.max()) if len(counts) else 0,
            "calibration_gap": (
                float(accuracy - mean_probability)
...
def _segment_summary(full_frame: pd.DataFrame, subset: pd.DataFrame) -> dict[str, Any]:
    result: dict[str, Any] = {
        "overall": _selection_stats(full_frame, subset),
        "by_tour": {},
        "by_surface": {},
        "by_tour_surface": {},
        "by_month": {},
    ...
    for value, group in subset.groupby("tour", dropna=False):
        key = str(value or "unknown").lower()
        result["by_tour"][key] = _selection_stats(full_frame, group)

    for value, group in subset.groupby("surface", dropna=False):
        key = str(value or "unknown").lower()
        result["by_surface"][key] = _selection_stats(full_frame, group)

    if not subset.empty:
        temp = subset.copy()
        temp["tour_surface"] = (
            temp["tour"].fillna("unknown").astype(str).str.lower()
            + ":"
            + temp["surface"].fillna("unknown").astype(str).str.lower()
        )
        for value, group in temp.groupby("tour_surface"):
            result["by_tour_surface"][str(value)] = _selection_stats(full_frame, group)

        temp["month"] = pd.to_datetime(temp["scheduled_at"], utc=True).dt.strftime("%Y-%m")
        for value, group in temp.groupby("month"):
            result["by_month"][str(value)] = _selection_stats(full_frame, group)

    return result


def _detailed_top_n_report(frame: pd.DataFrame, score_column: str) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for n in (1, 3, 5, 8, 10):
        chosen = _top_n(frame, score_column, n)
        result[str(n)] = _segment_summary(frame, chosen)
    return result


def _probability_thresholds_report(frame: pd.DataFrame) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for threshold in (0.75, 0.775, 0.80, 0.825, 0.85, 0.875, 0.90, 0.925, 0.95):
        subset = frame[frame["winner_probability"] >= threshold].copy()
        result[f"{threshold * 100:.1f}"] = _segment_summary(frame, subset)
    return result


def _high_probability_bands(frame: pd.DataFrame) -> dict[str, Any]:
    edges = [0.50, 0.60, 0.70, 0.75, 0.80, 0.825, 0.85, 0.875, 0.90, 0.925, 0.95, 1.000001]
    result: dict[str, Any] = {}
    for lo, hi in zip(edges[:-1], edges[1:]):
        subset = frame[(frame["winner_probability"] >= lo) & (frame["winner_probability"] < hi)].copy()
        label = f"{lo * 100:.1f}-{min(hi, 1.0) * 100:.1f}"
        result[label] = _selection_stats(frame, subset)
    return result


def _flatten_selection_rows(
    dataset: str,
    selection_type: str,
    selection_value: str,
    report: dict[str, Any],
    ...
    def add(segment_type: str, segment_value: str, stats: dict[str, Any]) -> None:
        ...
        rows.append(
            {
                ...
                "days": stats.get("days"),
                "day_coverage": stats.get("day_coverage"),
                "mean_picks_per_day": stats.get("mean_picks_per_day"),
                "mean_data_depth": stats.get("mean_data_depth"),
                "mean_factor_agreement": stats.get("mean_factor_agreement"),
            }
        )

    add("overall", "all", report["overall"])
    for segment_type in ("by_tour", "by_surface", "by_tour_surface", "by_month"):
        for key, stats in report.get(segment_type, {}).items():
            add(segment_type.removeprefix("by_"), key, stats)
    return rows
...
def _write_csv(path: Path, report: dict[str, Any]) -> None:
    ...
    for dataset in ("tuning", "validation"):
        section = report[dataset]["production_prime_audit"]
        for n, selection in section["top_n_per_day"].items():
            rows.extend(_flatten_selection_rows(dataset, "top_n_per_day", n, selection))
        for threshold, selection in section["probability_thresholds"].items():
            rows.extend(_flatten_selection_rows(dataset, "probability_threshold", threshold, selection))

    path.parent.mkdir(parents=True, exist_ok=True)
    fields = [
        "dataset", "selection_type", "selection_value", "segment_type", "segment_value",
        "n", "correct", "accuracy", "ci95_low", "ci95_high", "mean_probability",
        "calibration_gap", "coverage", "days", "day_coverage", "mean_picks_per_day",
        "mean_data_depth", "mean_factor_agreement",
    ]
...
def _markdown_table_for_topn(section: dict[str, Any]) -> list[str]:
    lines = [
        "| Top/day | Picks | Accuracy | 95% CI | Mean model p | Gap | Days |",
        "|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for n, item in section["top_n_per_day"].items():
        s = item["overall"]
        ci = s.get("accuracy_ci95") or [None, None]
        lines.append(
            f"| {n} | {s.get('n', 0):,} | {_pct(s.get('accuracy'))} | "
            f"{_pct(ci[0])} – {_pct(ci[1])} | {_pct(s.get('mean_model_probability'))} | "
            f"{_pct(s.get('calibration_gap'))} | {s.get('days', 0)} |"
        )
    return lines


def _markdown_table_for_thresholds(section: dict[str, Any]) -> list[str]:
    lines = [
        "| Min p | Picks | Accuracy | 95% CI | Mean model p | Coverage | Picks/day |",
        "|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for threshold, item in section["probability_thresholds"].items():
        s = item["overall"]
        ci = s.get("accuracy_ci95") or [None, None]
        lines.append(
            f"| ≥{threshold}% | {s.get('n', 0):,} | {_pct(s.get('accuracy'))} | "
            f"{_pct(ci[0])} – {_pct(ci[1])} | {_pct(s.get('mean_model_probability'))} | "
            f"{_pct(s.get('coverage'))} | {s.get('mean_picks_per_day', 0):.2f} |"
        )
    return lines
...
def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# BlinQ Prime Picks — full out-of-time audit",
        "",
        "Production candidate ranking: **calibrated winner probability descending**.",
        "Composite Prime score is retained only as an experimental comparison because it did not beat probability-only ranking on untouched validation.",
        "",
        "## Decision",
        "",
        f"- Validation year: **{report['data']['validation_year']}**",
        f"- Probability-only Top {decision['selected_top_n']}/day accuracy: **{_pct(decision['validation_probability_only_accuracy'])}**",
        f"- Composite Top {decision['selected_top_n']}/day accuracy: **{_pct(decision['validation_prime_accuracy'])}**",
        f"- Composite delta: **{_pct(decision['validation_accuracy_delta'])}**",
        "",
        "## Validation — Top N per day",
        "",
    ]
    lines.extend(_markdown_table_for_topn(report["validation"]["production_prime_audit"]))
    lines.extend(["", "## Validation — probability thresholds", ""])
    lines.extend(_markdown_table_for_thresholds(report["validation"]["production_prime_audit"]))

    top10 = report["validation"]["production_prime_audit"]["top_n_per_day"]["10"]
    lines.extend(["", "## Validation Top 10/day — ATP/WTA", ""])
    lines.extend(["| Tour | Picks | Accuracy | Mean p |", "|---|---:|---:|---:|"])
    for key, s in top10["by_tour"].items():
        lines.append(f"| {key.upper()} | {s.get('n',0):,} | {_pct(s.get('accuracy'))} | {_pct(s.get('mean_model_probability'))} |")

    lines.extend(["", "## Validation Top 10/day — surfaces", ""])
    lines.extend(["| Surface | Picks | Accuracy | Mean p |", "|---|---:|---:|---:|"])
    for key, s in sorted(top10["by_surface"].items()):
        lines.append(f"| {key} | {s.get('n',0):,} | {_pct(s.get('accuracy'))} | {_pct(s.get('mean_model_probability'))} |")

    lines.extend(["", "## High-probability calibration bands", ""])
    lines.extend(["| Band | Picks | Actual win rate | Mean p | Gap |", "|---|---:|---:|---:|---:|"])
    for band, s in report["validation"]["production_prime_audit"]["probability_bands"].items():
        lines.append(
            f"| {band}% | {s.get('n',0):,} | {_pct(s.get('accuracy'))} | "
            f"{_pct(s.get('mean_model_probability'))} | {_pct(s.get('calibration_gap'))} |"
        )
    ...
    lines.extend([
        ...
        "## Notes",
        "",
        "- All reported selection accuracy is out-of-time: the evaluated year is never used to train its model.",
        "- 95% confidence intervals use the Wilson binomial interval.",
        "- `calibration_gap = actual win rate - mean model probability`; values near zero are desirable.",
        "- This audit is read-only and does not call Tennis API.",
    ])
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    tuning_production_audit = {
        "ranking": "winner_probability_desc",
        "top_n_per_day": _detailed_top_n_report(tuning_scored, "winner_probability"),
        "probability_thresholds": _probability_thresholds_report(tuning_scored),
        "probability_bands": _high_probability_bands(tuning_scored),
    }
    validation_production_audit = {
        "ranking": "winner_probability_desc",
        "top_n_per_day": _detailed_top_n_report(validation_scored, "winner_probability"),
        "probability_thresholds": _probability_thresholds_report(validation_scored),
        "probability_bands": _high_probability_bands(validation_scored),
    }
    ...
    report = {
        ...
        "tuning": {
            **tuning_meta,
            ...
            "production_prime_audit": tuning_production_audit,
        },
        "validation": {
            **validation_meta,
            ...
            "production_prime_audit": validation_production_audit,
        },
        "decision": {
    ...
    print(f"CSV detail written to: {csv_path}")
